...
**Architectural mechanism specification — v0.5**

Filename: SysEngage_Row_3_Mechanism_CCI_Construction_v0_5.md
...
**Status.** v0.5 — four targeted corrections to v0.4 surfaced by Replit Agent plan-mode review. (1) Stage 4b: explicit group-size cap added — groups exceeding 50 members are split into sub-groups of 50 before the cluster review AI call. (2) §7 AnalysisPass outputs: `execution_warnings` array added to `cci_data` schema — home for `step4_read_failure` and other runtime execution conditions distinct from data integrity violations. (3) §11.2 OQ-3b-03: RA-1 stale question retired and replaced with resolved group-size cap question. No other sections changed.

**Changes from v0.4.** Stage 4b: group-size cap of 50 specified; sub-group splitting rule added. §7 cci_data schema: `execution_warnings` array added. §11.2: OQ-3b-03 updated to record group-size cap resolution.
...
| `execution_warnings` | Array of runtime execution conditions distinct from data integrity violations: `{warning_type, detail}`. Includes: `step4_read_failure` (SSL/connection drop during existing CCI read, with `cell_id`); `step4_nonetype_excluded` (malformed CCI excluded from cluster review, with `cell_id` and `ci_id_or_ref`); `step4_sub_group_split` (group size exceeded cap and was split, with `cell_id`, `classification_type`, `group_size`, `sub_group_count`). |
...
End of SysEngage Row 3 Mechanism: CellContentItem Construction v0.5.

Changes from v0.4: group-size cap of 50 added to Stage 4b; `execution_warnings` array added to §7 cci_data schema; OQ-3b-03 updated to record group-size cap resolution.

Companion artefact: SysEngage_Row_4_Mechanism_CCI_Construction_v0_5.md
