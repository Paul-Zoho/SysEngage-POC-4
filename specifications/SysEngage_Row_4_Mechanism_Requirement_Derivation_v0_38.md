...
Filename: SysEngage_Row_4_Mechanism_Requirement_Derivation_v0_38.md

Version: 0.38 (Supersedes v0.37 — **object_refs `formed` is now defined as a per-segment verification**, closing a spec gap that licensed the resolver to commit unverified attribute paths. §4.4.3a step 4 previously defined the dangling outcome only for an absent `.value` (`value_absent_from_class_model_domain`) and an unresolved leading entity — it never required the `.attr` segment to be verified, so a two-segment `<Entity>.<attr>` path with no value passed vacuously (the leading entity resolved, there was no value to miss) and was reported `formed` regardless of whether the attribute existed. v0.38 makes resolution a **verification, not a pass-through**: a path is `formed` only if EVERY segment positively resolves — the leading entity to a canonical with a defining `class_model`, the `.attr` to an attribute of that model **by `attr_name`**, the `.value` (where present) to that attribute's domain; a miss at any segment is a dangling binding with the segment-specific reason, and the new `attribute_not_in_class_model` reason is defined for an `.attr` that names no attribute of the resolved model. This is the spec-side root of the hollow-`formed` finding: the resolver was not violating the spec on `.attr` because the spec never told it to check `.attr`. MD-8 + §5.5 aligned; VER-3d-28 extended; §7 dangling-reason enumeration; §12.19. Version-stamp quadruple swept. No ledger/DD-schema change. Realises Row 3 RD v0.22.)

Version: 0.37 (Supersedes v0.36 — **[G] attribute well-formedness**, from the Run-28/29 finding that the CHK-3d-13 repair emitted attributes with `attr_name=null` and `semantic_type` set to a part-of-speech tag (`'Noun'`/`'Qualifier'`), which passed CHK-3d-11 because the Row-2 clause checked only that a semantic type was *present*. Run 29 fixed the POS-tag value but `attr_name` is still null — and an anonymous attribute cannot be bound by an `object_ref` (`<Entity>.<attr>` matches by name), which is why `object_refs formed` stays near-zero. [G] adds, to CHK-3d-11 (Stage-3 structural, HARD): every attribute has a **non-empty `attr_name`** (`detail: attr_name_empty`); every `semantic_type` is **shape-valid** — a single lowercase snake_case token `^[a-z][a-z0-9_]*$` (`semantic_type_malformed`) and **not a part-of-speech tag** (a closed, domain-independent POS set; `semantic_type_pos_tag`). **This is deliberately a *shape* gate, not an inclusion vocabulary** — it constrains the *form* of a kind-of-value descriptor, so a novel domain term (`systolic_pressure`, `iso_country_code`) passes exactly as `money` does; the only closed list is the universal POS set, which never grows with project type. The **consistency** problem (one concept spelled three ways) is handled on the open side by a soft **accreting semantic-type registry** (`core/semantic_type_registry.py`, §4.4.3c) — exact-match-or-mint like the DD's entity canonicals, flagging near-duplicates (`semantic_type_near_duplicate`) for Practitioner consolidation, **never rejecting** a novel term and never pre-populated. Semantic *aptness* (is `money` the right descriptor) remains a PLB-3d-07 judgement, not a hard check. VER-3d-26 extended ([G] negative fixtures); `core/semantic_type_registry.py`; `mechanism_data.semantic_type_registry`; §12.18. Version-stamp quadruple swept. No ledger/DD-schema change. Realises Row 3 RD v0.21.)

Version: 0.36 (Supersedes v0.35 — two cross-reference residuals from PLB-3d-07's v0.29 reassignment to `class_model` fidelity, + a fingerprint-format harmonisation; no logic change. **R2** ADVC-3d-02 (interrogative slot completeness) pointed at PLB-3d-07, which has been `class_model` fidelity since v0.29 — it had no live PLB home. Added **PLB-3d-08 — interrogative slot completeness**; the two stale ADVC-3d-02 → PLB-3d-07 references (the §4.3 body and the §8.1 parenthetical) now point to PLB-3d-08. **A1** the CHK-3d-13 fingerprint label was `_R{nnn}`-suffixed in the body but bare in the §6/§4.3 tables — harmonised to the bare stage form `stage3_chk3d13_judge`/`_repair` everywhere (consistent with the sibling `chk3d12_repair`/`stage3_repair`; the per-call `requirement_id` lives in the fingerprint payload, which is where call→requirement traceability belongs); also removed a duplicate detector sentence the v0.35 A2/A3 edit left in the CHK-3d-13 body. **R1 (agent-reported) did not reproduce** — the §8.1 parenthetical already reviews CHK-3d-08 via PLB-3d-02 in v0.35. §12.17. No A–F logic change. Realises Row 3 RD v0.20.)

Version: 0.35 (Supersedes v0.34 — SW-agent v0.34 review: doc residuals + two clarifications + prompt additions, no logic change to A–F. **R1** the v0.33→v0.34 change-detail was mislabelled `§12.13` (a duplicate) → **§12.15** (the split-pair label slip again). **R2** PLB-3d-07 referenced the retired `class_model_profile_advisory` → `profile_violation_row{n}:{field}`. **R3** §7 `execution_warnings` listed `class_model_profile_advisory` as soft → removed (it is now a HARD `validation_failures` detail under [C]); `class_model_statement_divergence` stays. **R4** §6 mode table gains the CHK-3d-13 judge + repair IM sub-acts. **R5/A2** §4.3 budget table gains the CHK-3d-13 judge + repair calls; **A2/A3** CHK-3d-13 fingerprints (`stage3_chk3d13_judge`/`_repair`) and repair prompt (`requirement_derivation_prompt.py`, `class_model`-only, scoped to one entity, one call per failing Structural) specified. **[C] clarification:** the logical `type` at Rows 2–3 is a **closed enum** {String, Integer, DateTime, Boolean, Decimal, Enum, Reference, JSON}; a physical DB type (`VARCHAR(255)`) at Row 2/3 is a profile violation (`type_not_logical`); the enum widens to free-form physical at Row 4+. **[A] clarification:** the strict entity binding is a `resolve_and_record(..., strict=True)` contract that skips the AI-judge step (DD-spec reciprocal flagged). **P1** §5.4 Row 1 guidance gains an explicit "Row 1 Structurals carry no `class_model`" instruction; **P2** §5.4 Row 2 gains the explicit profile restriction (suppress over-generation at the prompt, not only at the hard reject). **A4** `class_model_binding.by_tier` keys are tier-name strings (noted). **A1** (`model_coverage_gaps` full shape) and **A4** are impl-aligns — the spec shape stands. §12.16 change detail. Version-stamp quadruple swept. No ledger/DD-schema change; DD-spec strict-mode note flagged for Row 4 DD v0.2.)

Version: 0.34 (Supersedes v0.33 — Run-26 integrity findings → six checks/rules (A–F), the first substantive F105/F107 logic change since v0.29; realises Row 3 RD v0.20 / ledger v2.17 unchanged. Run 26 ran the F105/F107 build at v2.17 but produced low-integrity `class_model`s: the entity-name resolver's AI-judge fuzzy-bound `entity_ref` to semantically-near canonicals (`ChildEarnings`→`monetary value`); two entities collided on one `entity_ref`; the same entity was modelled twice with divergent attributes; every Row-2 model carried Row-4 physical detail; and prose Structurals duplicated `class_model` content — none caught, because the §4.4.3b audit blocks were written by stage4 but never read (now fixed by the SW agent). **[A] Strict `entity_ref` (§4.4.3a):** a `class_model`'s own entity name is bound by **exact (normalised) canonical match or mint-new** — never the AI-judge similarity path used for behavioural synonymy; entity identity ≠ semantic relatedness. **[B] CHK-3d-11 uniqueness (Stage-4 referential):** no two surviving `(entity, row_target)` Structurals share an `entity_ref`/`entity`; divergent duplicates are a HARD finding (the `(statement, cci_refs)` dedup key, CHK-3d-07, cannot see entity identity). **[C] CHK-3d-11 attribute-profile by tier (Stage-3, HARD — replaces the soft §4.4.3c advisory):** each attribute's populated fields MUST be ⊆ the row's profile field-set (§5.4) — Row 2 forbids physical `type`/`key`/`domain`/`target_ref`; a field outside the row's set is rejected. **[D] Row 1 authors no `class_model`** (`tier`∈{2..5}; a `tier=1` model is rejected; Row 1 Structurals are scope prose). **[E] New CHK-3d-13 — a data-asserting Structural MUST carry a `class_model`** (a *have/comprise/define/contain* predicate over an entity's attribute/relationship/key/domain); a non-data structural constraint MAY stay prose; a data-assertion-without-`class_model` is repaired then HARD-failed (kills the R023/R069/R072 prose duplicates). **[F] Drop the bare-`<Entity>` `object_refs` form** — `<Entity>.<attr>` minimum (a bare entity names nothing; the resolver already enforces `malformed_path_min_2_segments`). VER-3d-26 extended + new VER-3d-29 (CHK-3d-13); §12.13 change detail. Version-stamp quadruple swept. No ledger/DD-schema change.)

Version: 0.33 (Supersedes v0.32 — Issue H: one residual of the version-locked-forward-sentence pattern. Fixture 9 (§9.9) still read "the first **v0.31** Pass 3d run … builds its `class_model` domain" — the paired companion of VER-3d-28, which was made version-agnostic in v0.32 (Issue E/G) while the fixture sentence was not. Same split-pair failure as v0.30-A→v0.31-E and v0.32-G→here: a pattern fixed in one of two paired locations. Corrected to "the first Pass 3d run that derives …", matching the §12.3 / VER-3d-28 wording. **Sweep:** an exhaustive audit confirmed this is the *only* remaining forward-looking sentence locked to a version number; the other `v0.NN` references are stable history ("the v0.24 build …" narrative; baseline-absence notes like "no chunking at v0.1") that do not re-stale. §12.14. No ledger/DD/Row-3 change; no normative logic change. Realises Row 3 RD v0.19.)

Version: 0.32 (Supersedes v0.31 — third SW-review pass: three same-pattern residuals from the v0.31 edits + a proactive sweep of every instance, no design change. **Issue E (§8.1 VER-3d-28):** the test description still asserted the v0.29 "migration promotes the value-set into the `class_model` domain" (the same survival pattern as v0.30 Issue A — companion fixture updated, verification row missed). Corrected to the two-step flow: the migration **strips** (no `class_model` write); the **first Pass 3d run** builds the `Task.status` domain from the Structural requirement content. **Issue F (§4.4.3c Step 3):** the FK/target-resolution attribution "§4.4.3a Stage-4 referential check" was stale — Issue C moved that check to §4.4.3c sub-pass 2; re-pointed. **Issue G (§12.3 ¶3):** "first **v0.30** Pass 3d run" was a missed body-text version residual — made version-agnostic ("first run under this spec"). **Proactive sweep (same patterns, not separately flagged):** the §3.1 module comment for `promote_dd_to_class_model` still read "DD value-sets → class_model domains" (the old staging behaviour) → corrected to strip-and-defer; the CHK-3d-11 intro and its rejection-location both pointed the *referential* check at §4.4.3a → re-pointed to §4.4.3c sub-pass 2 (`entity_ref` is *set* in §4.4.3a; the referential *check* runs in sub-pass 2). §12.13 change detail. Version-stamp quadruple swept. No ledger/DD/Row-3 change; no normative logic change. Realises Row 3 RD v0.19.)

Version: 0.31 (Supersedes v0.30 — second SW-agent spec-review pass: four residuals from the v0.30 edits, no design change. **Issue A (§9.9):** Fixture 9 still asserted the v0.29 staging/promotion behaviour (migration promotes value-sets into `class_model` domains) — contradicting the v0.30 §12.3 strip-then-defer. Rewritten to the two-step flow: the **migration** strips DD value-sets/relationships (`orphan_value_set_on_migration` for any unbacked), and the **first Pass 3d run** (not the migration) derives the `Task` Structural, builds its `class_model` domain from the Structural requirement content, and materialises `object_refs`. **Issue B (§4.4.3a step 1):** the Structural sub-clause still carried the pre-F107 relationship-triple wording ("present the relationship `(entity_a, relation, entity_b)`") — the retired DD `record_relationship` operation — and the "value referenced by name → attribute value" DD presentation. Corrected: for a `class_model`-bearing Structural the DD receives **names only** — `class_model.entity` plus each `class_model.relationships[].target` entity name (so referenced entities register and FK/relationship targets can resolve); the relationship *structure* is `class_model.relationships` (§4.4.3c), and a named value is an `object_refs` path (step 4), neither presented to the DD. **Issue C (CHK-3d-11 Stage-4 referential, §4.4.3a/c):** the referential check ran inline per-Structural, so a same-row cross-entity target (e.g. `Task`→`Child`, both derived in one pass) would false-fail because the target's `class_model` is still in-flight. Resolved by **two sub-passes** in Stage 4 (option (c)): sub-pass 1 sets every `entity_ref` and builds the row's working `class_model` set; sub-pass 2 runs the referential resolution once the whole row exists (working set ∪ prior rows), so same-row targets resolve and an unresolved target is a genuine reject. **Issue D (§7):** `orphan_value_set_on_migration` is a one-time *migration-script* log, not a Pass 3d AnalysisPass warning — removed from the §7 `execution_warnings` enumeration and kept solely as the §12.3 migration output. VER-3d-26 (b) updated to the sub-pass; §12.12 change detail. Version-stamp quadruple swept. No ledger/DD/Row-3 change. Realises Row 3 RD v0.19.)
...
mechanisms/requirement_derivation/
  ...
  core/
    ...
    structural_data_assertion.py               # CHK-3d-13: data-asserting Structural must carry a class_model ([E], §4.3)
    semantic_type_registry.py                  # [G]: accreting semantic_type registry — exact-match-or-mint + near-duplicate flag (soft, §4.4.3c)
  ...
  migrations/
    ...
    promote_dd_to_class_model.py               # one-time data migration: STRIP DD value-sets/relationships (DD → names+synonyms); orphan_value_set_on_migration for any unbacked; class_model domains/object_refs are built by the FIRST run, not the migration (F107-T1(d), §12.3)
```
...
**MD-8 — `object_refs` is DM-materialised at Stage 4; the DD is names-only (realises Row 3 §5.5; F107).** A behavioural proposal carries candidate `object_refs` (the dotted paths the statement names — `<Entity>.<attr>` / `<Entity>.<attr>.<value>`, **≥2 segments — [F]/C2**; a bare `<Entity>` is NOT an object_ref, naming an entity is the DD's job, and the resolver rejects it `malformed_path_min_2_segments`) as an IM output; Stage 4 §4.4.3a **materialises** each by resolving the leading `<Entity>` through the DD canonical name and **verifying** the trailing `.attr` (by `attr_name`) and `.value` (by domain) against the owning `class_model`; a path is committed to `requirement.object_refs` only if **every** segment resolves, and a miss at any segment (`entity_not_in_working_set` / `attribute_not_in_class_model` / `value_absent_from_class_model_domain`) is a **dangling binding** for the Quality pass (NOT a derivation reject) — a `<Entity>.<attr>` path is never formed without verifying `.attr`. Post-F107 the DD is a pure naming / synonym index: the §4.4.3a relationship-record (DD §4.3) and value-record (DD §4.4) operations are **removed** — relationships live in `class_model.relationships`, named values in `object_refs`. The DD service answers "what is this called and what does the name resolve to"; `class_model` answers "what is the thing."
...
| Stage 3 CHK-3d-13 data-assertion judge (ambiguous only) | single-boolean judge (no template) | One statement; a boolean — trivially small |
| Stage 3 CHK-3d-13 repair (conditional) | `requirement_derivation_prompt.py` (`class_model`-only, scoped to one entity) | One Structural per call (not batched); inherently small |
...
**CHK-3d-11 — `class_model` structural validity (decidable, HARD; split structural / referential — F105 / Issue 2).** Realises Row 3 §4.1.1(g) / CHK-3d-11. **Structural clauses run in Stage 3 against the AI proposal alone** (`core/class_model_validity.py`); **referential clauses are deferred to Stage 4** (§4.4.3c sub-pass 2 — `entity_ref` is *set* in §4.4.3a, but the referential *check* runs after the whole row's `class_model`s exist, Issue C), because the proposal carries the entity *name* but not its `DD###` `entity_ref` — that id is allocated by the DD service in Stage 4 (on a FirstRun the DD is empty, so no `entity_ref` could exist at proposal time, Issue 2).

*Stage 3 — structural (against the proposal):*
- `entity` (the canonical name) present and non-empty; `tier` ∈ {2,3,4,5} and matches `row_target` (2→`conceptual`, 3→`logical`, 4→`physical`, 5→`detailed`). **[D] A `class_model` at `row_target=1` (tier 1) is rejected** (`detail: tier1_class_model`): `class_model` authoring begins at Row 2 (entity existence) — Row 1 Structurals are scope-level prose and carry no `class_model` (Run-26: 6 tier-1 models authored, all rejected as noise; the IM should not author them, §5.4);
...
- **[G] attribute well-formedness (HARD).** Every attribute MUST have a **non-empty `attr_name`** (`detail: attr_name_empty`) — an anonymous attribute cannot be bound by an `object_ref` (`<Entity>.<attr>` matches by name) and is not a model element. Every `semantic_type`, where present, MUST be **shape-valid**: a single lowercase snake_case token matching `^[a-z][a-z0-9_]*$` (≤64 chars; `detail: semantic_type_malformed`) and **not a part-of-speech tag** — the closed, domain-independent set {`noun`,`verb`,`adjective`,`adverb`,`pronoun`,`preposition`,`conjunction`,`determiner`,`article`,`interjection`,`qualifier`,`modifier`,`participle`,`gerund`}, case-insensitive (`detail: semantic_type_pos_tag`). **This is a *shape* gate, NOT an inclusion vocabulary:** it constrains the *form* of a kind-of-value descriptor, so a novel domain term (`systolic_pressure`, `iso_country_code`) passes exactly as `money` does, while a grammar tag (`Noun` — caps fail shape; `noun` — POS set), a physical type (`VARCHAR(255)` — parens fail shape), or a sentence fragment (spaces fail shape) is rejected. The only closed list is the universal POS set, which never grows with project type — no per-project curation. (Run-28: `attr_name=null` + `semantic_type='Noun'`/`'Qualifier'` passed the present-only check; Run-29 fixed the value but not the null name. [G] rejects both.) The *consistency* of well-formed terms is handled softly by the semantic-type registry (§4.4.3c);
...
- **[C] attribute profile by tier (HARD — replaces the soft §4.4.3c advisory).** Each attribute's *populated* fields MUST be a subset of the row's profile field-set (§5.4): **Row 2 (conceptual)** — `name`, `semantic_type`, `origin` only; relationships limited to a plain `association` + `target`; the physical fields `type`, `key` (PK/FK), `domain`, `target_ref`, `null`, `unique`, `default` are FORBIDDEN. **Row 3 (logical)** — adds logical `type`, `domain`, business `keys`; forbids physical `type`, surrogate/FK columns, `null`. The logical `type` at Rows 2–3 is drawn from a **closed enum** `{String, Integer, DateTime, Boolean, Decimal, Enum, Reference, JSON}`; a `type` value outside that enum (a physical DB type — `VARCHAR(255)`, `DECIMAL(10,2)`) at Row 2/3 is a profile violation (`detail: profile_violation_row{n}:type_not_logical`). The enum widens to a free-form physical DB type at **Row 4+** (so the logical/physical distinction is decidable from the `type` value, not a separate field). **Row 4 (physical)** — adds physical `type`, `null`, `PK`/`FK`, FK `target_ref`. **Row 5 (detailed)** — adds precision / check / storage. A populated field outside the row's set → **reject** (`detail: profile_violation_row{n}:{field}`). (Run-26 root cause: every Row-2 model carried `type`/`PK`/`FK`/`domain` — full Row-4 detail at the conceptual tier; the IM is to populate ONLY the row's fields, §5.4/§4b.);
...
*Stage 4 — referential (a **second sub-pass** after §4.4.3a sets every `entity_ref` and the whole row's `class_model` working set exists — Issue C):* `entity_ref` resolves to a `canonical` `DataDictionaryEntry` (`detail: entity_ref_unresolved` — trivially satisfied, set from the DD); every FK `target_ref` and every `relationships[].target` **resolves** to a known entity — a canonical with a defining `class_model`, looked up against the row's working set ∪ committed prior rows, so a same-row cross-entity target resolves (`detail: target_unresolved` only if unresolved after the full row exists). **[B] `entity_ref` uniqueness / one `class_model` per `(entity, row)`:** no two surviving Structural Requirements at the same `row_target` carry a `class_model` with the same `entity_ref` **or** the same `entity` name. Two models for one entity at one row → the lower-`confidence` one is **rejected** and the obligation merged/re-covered (`detail: duplicate_entity_model {entity_ref, requirement_ids}`); a **divergent** duplicate (same entity, conflicting attributes — Run-26 `ChildEarnings` as `earnings_total` vs `total_earnings`) is a HARD finding requiring IM repair, never silent coexistence. This is the integrity the `(statement, cci_refs)` dedup key (CHK-3d-07) structurally cannot provide — entity identity is not in that key.

A `class_model` failing a structural clause is **rejected in Stage 3** (log `{check_id:"CHK-3d-11", requirement_id_placeholder, detail}` in `validation_failures`, `execution_warnings += class_model_invalid`); a referential failure is rejected at §4.4.3c sub-pass 2. A rejected Structural's obligation is re-covered via the orphan (CHK-3d-05) / seed (CHK-3d-10) paths. **A valid `class_model`-bearing Structural SKIPS the CHK-3d-09 prose-atomicity checks** — the entity is the unit of atomicity (D4). (A prose-only Structural with no `class_model` — migration residue — still runs CHK-3d-09.) Decidable; no IM.
...
**CHK-3d-13 — a data-asserting Structural MUST carry a `class_model` (decidable-with-IM-fallback, HARD — [E]; F105).** F105 routes entity/attribute *data content* into a `class_model`, not prose. A Structural Requirement whose content **asserts data-model structure** MUST carry a `class_model`; a Structural that is a non-data structural **constraint/policy** (retention, auditability, a cross-entity rule that introduces no attribute) MAY remain prose and runs CHK-3d-09 as now.

**Data-asserting detector (`core/structural_data_assertion.py`).** A Structural is *data-asserting* iff its statement (a) references a DD-`canonical` **entity** AND (b) asserts **composition** of that entity — a *have / comprise / define / contain / consist-of / associated-with / with-attribute* predicate over an **attribute, relationship, key, or domain**. The predicate+object test is DM-decidable; a **genuinely ambiguous** statement is settled by a single-boolean IM **judge** (fingerprint `stage3_chk3d13_judge`, the analogue of the CHK-3d-08 subject judge). A **data-asserting Structural with no `class_model`** → conditional IM **repair** — re-derive it as a `class_model` scoped to that one entity using `requirement_derivation_prompt.py` with a `class_model`-only instruction (the §4.4.3c derivation path), **one call per failing Structural** (not batched), fingerprint `stage3_chk3d13_repair`; `chk3d13_repair_performed`. (Fingerprint stage names carry no `_R{nnn}` suffix — consistent with `chk3d12_repair` / `stage3_repair`; the per-call `requirement_id` lives in the fingerprint payload, which is where call→requirement traceability is recorded — A1.) Persistent → `validation_failures {check_id:"CHK-3d-13", requirement_id, detail:"data_assertion_without_class_model"}`, `execution_warnings += chk3d13_data_assertion_unmodelled` (HARD). A non-data structural constraint is **exempt** (`chk3d13_constraint_exempt` recorded for audit). (Run-26: R023 "a task shall have an associated monetary value", R069, R072 are attribute assertions emitted as prose Structurals *alongside* the `Task` `class_model` — the duplication CHK-3d-13 prevents; they are re-derived into, or merged with, the `Task` model.)
...
**ADVC-3d-02 — Interrogative slot completeness (advisory; realises Row 3 §4.1.1(f) / F87/F88).** For each surviving Requirement, check whether the slots its `requirement_type` requires are *filled* (Functional: Subject/Action/Object present, Condition where the source implies a trigger; Constraint: Subject/Constraint-Rule, Criteria where Measurement-verified; Structural: entity + composition/attribute/relationship assertion). This is distinct from CHK-3d-09 (which HARD-rejects compound/missing-required-slot *atomicity* violations): ADVC-3d-02 is a **soft generative-completeness advisory** — it flags a requirement whose type-required slots are thin or whose Object was not interrogated to structure where the source implied it, logging `interrogative_completeness_advisory {requirement_id_placeholder, type, thin_slots}` for Practitioner review (PLB-3d-08). Rationale for soft (not hard): interrogative completeness is a *generative guidance* property (it makes the AI surface more, validated in the F87 sandbox) — its absence is an under-elaboration to review, not a structural defect to reject. A missing *required* slot is already hard-caught by CHK-3d-09; ADVC-3d-02 catches the softer "could have been interrogated further" case. Informational; production proceeds. (Why advisory and not hard: an over-eager hard completeness gate would reject legitimately-terminal requirements — a Row 1 enterprise constraint need not decompose to an Object the way a Functional behaviour does. The row-and-type-awareness is in the guidance, §5.4; the check only flags thinness for review.)
...
3. **Bind / flag (names).** Resolved → the Requirement's entity name is bound (the binding lives in the DD via the entry's `provenance_ref` and synonym register; the *name* binding is not written onto the Requirement element). **Unresolved / flagged** → record `dd_unresolved {requirement_id, term, reason}` in `mechanism_data.dd_binding` (§4.4.3b). The Requirement is still produced (Non-Loss); the unresolved name is the signal that the Matching service must treat this Requirement as not-yet-matchable (Matching §4.1 / VER-rm-07).
   **(Structural — set `entity_ref`, strict — Issue 2 / [A].)** The `class_model`'s own entity name is resolved by **exact (normalised) canonical match, a registered synonym's `resolves_to`, or mint-new** — it **bypasses the AI-judge similarity path** that `resolve_and_record` uses for behavioural term synonymy. An entity name with no exact canonical/synonym **mints a new canonical**; it is **never** bound to a semantically-near existing canonical. (Run-26 root cause: the AI judge bound `ChildEarnings`→`monetary value` (DD003) at high confidence — entity *identity* is not semantic *relatedness*.) The strict path is a **`resolve_and_record(..., strict=True)`** contract (or a dedicated `resolve_entity_name`) that performs exact-canonical / synonym `resolves_to` / mint-new and **skips the AI-judge step entirely** — the DD service must expose this strict mode (DD-spec reciprocal: Row 4 DD v0.2 to document it). Write the exact/minted canonical's `dd_id` to `class_model.entity_ref`, before the §4.4.3c commit (so the committed `class_model` satisfies the ledger's `required` `entity_ref`). The **referential half of CHK-3d-11 does NOT run here** — it is deferred to a **second sub-pass** after the whole row's `class_model`s exist (Issue C, §4.4.3c referential sub-pass), so a same-row cross-entity target (e.g. `Task` referencing `Child`, both derived in this pass) resolves instead of false-failing on an in-flight target.
4. **`object_refs` materialisation — behavioural Requirements only (DM; F107).** `object_refs_resolver.py` is a **thin RD-side orchestrator** (Issue 3): it does NOT re-implement DD lookup. For each candidate path it (i) calls **`dd_service.resolve_object_ref(path)`** — the DD resolves the leading `<Entity>` segment to a `canonical` (directly or via a `synonym`'s `resolves_to`) and **hands back the canonical + the unresolved `.attr`/`.value` tail; the DD never touches `class_model`** (it holds no model content) — then (ii) resolves the returned tail itself against the `class_model`(s) whose `entity_ref` is that canonical, querying the Structural Requirements (RD's own table) that define the entity (typically this row and the rows above). The name half is the DD's job; the `class_model` value half is RD's — matching the existing "RD names, DD resolves the name" boundary. **A path is `formed` only if EVERY segment positively resolves — resolution is a *verification*, not a pass-through.** The leading `<Entity>` resolves to a canonical with a defining `class_model`; the `.attr` segment **matches an attribute of that `class_model` by `attr_name`**; and the `.value` segment (where present) matches that attribute's domain. A miss at ANY segment is a **dangling binding** carrying the segment-specific `reason` — `entity_not_in_working_set` (the leading entity resolves to no defining `class_model`), **`attribute_not_in_class_model`** (the `.attr` names no attribute of the resolved model — e.g. its attributes are anonymous or differently named), or `value_absent_from_class_model_domain` (the `.value` is absent from the attribute's domain) — recorded in `object_refs_binding.dangling [{requirement_id, ref, reason}]` (`execution_warnings += object_refs_dangling`) for the Quality pass (F107-T1), and **NOT** a derivation-time reject: the defining Structural may be derived later, so a reject here would false-fail a legitimate forward reference. **A two-segment `<Entity>.<attr>` path with no `.value` is NOT vacuously `formed` — its `.attr` must still match an attribute by `attr_name`, else it dangles `attribute_not_in_class_model`; the absence of a value-miss is not a pass.** `formed` counts only paths every segment of which resolved. `object_refs` is materialised AFTER §4.4.3c so the row's `class_model`s exist to resolve against.
...
"class_model_binding": {                                  // F105 — §4.4.3c
  ...
  "by_tier": { "conceptual": 0, "logical": 4, "physical": 2, "detailed": 0 }, // keys are tier-NAME strings (2→conceptual, 3→logical, 4→physical, 5→detailed), not integers — A4
...
"object_refs_binding": {                                  // F107 — §4.4.3a step 4
  ...
  // `reason` ∈ { entity_not_in_working_set, attribute_not_in_class_model, value_absent_from_class_model_domain } — one per unresolved segment; `formed` counts only all-segments-resolved paths (v0.38)
}
```
...
**Stage 4 runs Structurals in two sub-passes (Issue C).** *Sub-pass 1* (steps 1–4 below, per surviving Structural): resolve the entity name → DD canonical → set `entity_ref` (§4.4.3a), validate, and build the row's **working `class_model` set** in memory. *Sub-pass 2* (the referential check), once the whole row's working set exists: for each `class_model`, `entity_ref` resolves (trivially) and every FK `target_ref` / `relationships[].target` resolves to a known entity — looked up against **the working set (this row) ∪ committed prior rows**, so a same-row cross-entity target resolves. A target unresolved after sub-pass 2 is a genuine CHK-3d-11 referential failure → reject the Structural (`{check_id:"CHK-3d-11", detail:"target_unresolved"}`), re-cover via orphan/seed, and re-run dependent checks on the reduced set. The single ledger transaction (§4.4.6) commits the surviving working set, so a sub-pass-2 reject removes the Requirement before any DB write — nothing is un-committed.

1. **Validate (CHK-3d-11 structural).** Structural validity is checked in Stage 3; a `class_model` that failed has already removed its Requirement from the surviving set. (Belt-and-braces: a survivor with a malformed `class_model` reaching Stage 4 is a defect → reject, `class_model_invalid`. The *referential* clauses run in sub-pass 2 above, not here.)
2. **Profile conformance — enforced HARD by CHK-3d-11 [C] (§4.3), not a soft advisory ([C], Run-26).** The per-row attribute field-set (§5.4) is enforced at IM time by `requirement_row_guidance` and as a **hard CHK-3d-11 structural clause**: a Row-2/3 `class_model` carrying any physical field (`type` / `null` / PK-FK / `target_ref` / storage — those are R4–R5) is **rejected** (`detail: profile_violation_row{n}:{field}`), not committed. (The v0.33 soft `class_model_profile_advisory` is superseded — Run 26 showed the violation is 100% systematic, so a soft advisory only emits noise; the corrective levers are the row-restricted prompt AND the hard reject.)
3. **Commit.** Write the `class_model` JSONB onto the Requirement (§5.1). `entity_ref` was set in §4.4.3a (the resolved DD canonical id — Issue 2; the committed form now satisfies the ledger `required` `entity_ref`); `refinement_kind` and attribute `origin` are persisted as the IM recorded them; FK `target_ref`/`relationships[].target` names are resolved by the §4.4.3c **sub-pass 2** referential check (Issue C/F — the check moved here from §4.4.3a).
...
5. **Semantic-type registry (soft accretion — [G]).** Each attribute's (shape-valid) `semantic_type` is reconciled against an **accreting registry** (`core/semantic_type_registry.py`) by the exact-match-or-mint pattern the DD uses for entity canonicals: an exact match **reuses** the registered term; a novel term **mints** (registered on first use — no pre-population, novelty always admitted); a **near-duplicate** of an existing term (normalised form within a small edit distance, or one a stem/substring of the other — e.g. `money` / `monetary_value` / `moneyvalue`) **mints AND flags** `semantic_type_near_duplicate {new, existing}` (soft, for Practitioner consolidation — PLB-3d-07), **never rejecting**. The registry is the open, self-extending side of [G] (it grows with the domain, needs no curation); the hard shape/POS gate (§4.3 CHK-3d-11) is the bounded, universal side. The near-duplicate edit-distance threshold is a calibration parameter, deferred to test-and-evaluate. Record `mechanism_data.semantic_type_registry = {minted, reused, near_duplicates:[]}`.
...
- **Row 2 (conceptual):** each attribute carries a **`attr_name`** (a real name — `monetary_value`, `availability_status` — NOT null) and a **`semantic_type`** (a *kind-of-value* descriptor as a lowercase snake_case token: `money`, `calendar_date`, `identifier`, `lifecycle_state` — NOT a part-of-speech tag like `Noun`/`Qualifier`, NOT a physical DB type, NOT a phrase); `origin` is recorded. Relationships as plain associations. NO physical types, keys, or domains. **At least one named, semantic-typed attribute is REQUIRED** ([G]/Issue 6 / CHK-3d-11): an entity you can give no attribute is a name, not a model — return it as a DD entity name with NO `class_model`, not a degenerate empty model. (An anonymous attribute cannot be referenced by an `object_ref`, so a null `attr_name` is a hard reject, [G].)
- **Row 3 (logical):** allowed attribute fields = Row-2 set **+** logical `type` + `domain` + business `keys`. Technology-agnostic — FORBIDDEN: physical types, surrogate keys, FK columns, `null`.
- **Row 4 (physical):** + physical `type` + `null` + PK/FK + FK `target_ref` (relationships as FK columns). Surrogate keys appear here.
- **Row 5 (detailed):** + precision + check constraints + storage / index / default.

**Row 1 (scope / contextual) authors NO `class_model` ([D]).** Entity existence begins at Row 2; a Row 1 Structural is scope-level prose (and is rejected if it carries a `class_model`, CHK-3d-11 `tier1_class_model`). Populate **only** the fields listed for the row you are deriving — do not author physical `type`/`key`/`domain` at Row 2 (CHK-3d-11 [C] rejects it; Run-26 every Row-2 model over-populated to Row-4 detail).
...
**`object_refs` identification — behavioural requirements, all rows (§5.5, F107).** When a Functional / Constraint statement references a data-model element **by name** — an entity, an attribute, or a named value ("a task with status *available*", "the *weekly earnings* total") — return the dotted path(s) it names in `object_refs`: `<Entity>.<attr>` / `<Entity>.<attr>.<value>` (e.g. `Task.status.available`) — **≥2 segments ([F]/C2)**; a bare `<Entity>` is NOT an object_ref (naming an entity is the DD's job; the resolver rejects it `malformed_path_min_2_segments`). Use the entity's canonical name and the attribute/value as the model defines them. Do NOT invent values or attributes not present in the data model; the resolver (§4.4.3a) **verifies each segment** against the owning `class_model` — `.attr` by `attr_name`, `.value` by domain — and a path that names an attribute or value the model does not have is a **dangling binding** (flagged for review), never silently accepted as formed. A behavioural statement that references no data-model element by name returns an empty `object_refs`.
...
**Row 1 Structurals carry NO `class_model` ([D]).** At Row 1 a Structural requirement is a scope-level prose assertion (e.g. "The enterprise shall recognise each child as a participant with a task record."). Do NOT author a `class_model` for a Row 1 Structural — `class_model` authoring begins at Row 2 (a `tier=1` model is hard-rejected, CHK-3d-11).
...
### requirement_type reasoning (principle-based — choose, do not pattern-match)
**Row 2 `class_model` profile ([C]).** A Row 2 (conceptual) Structural's `class_model` populates ONLY `name`, `semantic_type`, and `origin` per attribute; a relationship carries only a plain `association` + `target`. Do NOT include `type`, `key` (PK/FK), `domain`, `target_ref`, `null`, `unique`, or `default` — those belong at Row 3 or later and are HARD-rejected here (CHK-3d-11 `profile_violation_row2:{field}`).
...
| Stage 3 — CHK-3d-13 data-assertion judge (ambiguous only) | IM | AI call; single boolean (is this statement a data assertion?) | `stage3_chk3d13_judge` fingerprint |
| Stage 3 — CHK-3d-13 data-assertion repair (conditional) | IM | AI call; re-derive the flagged prose Structural as a `class_model` | `stage3_chk3d13_repair` fingerprint |
...
```jsonc
{
  "mechanism_data": {
    ...
    "semantic_type_registry":    { "minted": 5, "reused": 7, "near_duplicates": [] }, // [G] — accreting semantic_type registry (§4.4.3c); near_duplicates: [{new, existing}] soft, PLB-3d-07
...
| **VER-3d-26** | `class_model` validity (F105, CHK-3d-11), **two phases** (Issue 2/6): **(a) structural (Stage 3, against the proposal)** — `entity` present, `tier`∈{2..5} & ==row (**[D]** tier-1 rejected), **≥1 attribute** (≥1 *semantic-typed* at Row 2), ≤1 `key=="PK"`, FK/relationship target *format* well-formed, domains well-formed, enum-valid `tier`/`refinement_kind`/`key`/`origin`, **[C] per-tier attribute profile** (Row 2 forbids physical `type`/`key`/`domain`/`target_ref`); `entity_ref` is NOT required of the proposal; **(b) referential (Stage 4, second sub-pass after the whole row's `class_model`s exist — Issue C)** — `entity_ref` resolves to a canonical DataDictionaryEntry; every FK `target_ref`/`relationships[].target` resolves against the row's working set ∪ prior rows; **[B] uniqueness** — no two surviving `(entity, row)` Structurals share an `entity_ref`/`entity`. Behavioural Requirements carry no `class_model`; Structural at rows 2–5 SHOULD carry one | `test_class_model_validity` — (a) structural clauses on each Structural survivor; negative fixtures: zero attributes → `no_attributes`, Row-2 typeless attribute → `row2_no_semantic_type`, two PKs → `multiple_pk`, **Row-2 attribute with `key`/physical `type`/`domain` → `profile_violation_row2:{field}` ([C])**, **`tier=1` class_model → `tier1_class_model` ([D])**, **[G]: `attr_name=null` → `attr_name_empty`; `semantic_type='Noun'` → `semantic_type_pos_tag`; `semantic_type='VARCHAR(255)'` → `semantic_type_malformed`; a novel `semantic_type='systolic_pressure'` PASSES (shape gate, not inclusion list)**; assert the AI proposal parses WITHOUT `entity_ref`; (b) post-§4.4.3a assert `entity_ref` is `^DD\d{3}$` resolving to a canonical; **same-row resolution fixture: `Task` and `Child` both derived in one Row-2 pass with `Task→Child` relationship → assert it resolves (no `target_unresolved`)**; a target with no defining `class_model` in this row or above → `target_unresolved`; **two `(ChildEarnings, row2)` models → `duplicate_entity_model` ([B]); an entity name with no exact canonical mints a NEW canonical, never fuzzy-binds to a near one ([A])**; assert DB CHECK rejects `class_model` on Functional / `object_refs` on Structural |
...
| **VER-3d-29** | data-asserting Structural carries a `class_model` (CHK-3d-13, [E]): a Structural whose statement asserts an entity's attribute / relationship / key / domain (a *have/comprise/define/contain/associated-with* predicate over an entity) MUST carry a `class_model`; a non-data structural constraint (retention, auditability) MAY remain prose | `test_chk3d13_data_assertion` — positive: "a task shall have an associated monetary value" with no `class_model` → `chk3d13_repair_performed` then, if unrepaired, `data_assertion_without_class_model` (HARD); a `class_model`-bearing Structural for the same content passes; exempt: "task data shall be retained for 7 years" → `chk3d13_constraint_exempt`, prose retained, runs CHK-3d-09 |
| **VER-3d-28** | `object_refs` materialisation (F107): every committed `object_refs` entry's leading `<Entity>` resolves to a canonical DataDictionaryEntry (directly or via synonym) and its trailing `.attr`/`.value` resolves to a `class_model` `attr_name`/domain value; an unresolvable value is recorded in `object_refs_binding.dangling` (NOT a derivation reject); no `object_refs` on a Structural Requirement | `test_object_refs_resolution` — `Task.status.available` fixture: assert it is committed and resolves end-to-end; `Task.status.archived` fixture (value absent): assert it is recorded dangling (`value_absent_from_class_model_domain`) + `object_refs_dangling`, the Requirement still produced; **a `<Entity>.<attr>` path whose `.attr` matches no attribute of the resolved model → dangling `attribute_not_in_class_model`, NOT `formed` (v0.38 — a two-segment path is not vacuously formed)**; **a bare-`<Entity>` candidate → `malformed_path_min_2_segments`, NOT committed ([F])**; **(migration two-step, Issue E)** assert the migration **strips** the DD `lifecycle_state` value-set (writes NO `class_model` domain), and the **first Pass 3d run** that derives the `Task` Structural builds its `Task.status` `class_model` domain from the Structural requirement content (de-duplicated against the Structural CHECK) |

(CHK-3d-08 subject mismatch is recorded in `subject_vocabulary_flags` and reviewed via PLB-3d-02; it is soft severity and not a VER gate. CHK-3d-09 atomicity is HARD — VER-3d-15 gates it. ADVC-3d-02 interrogative-completeness is a soft advisory — it logs `interrogative_completeness_advisory` for PLB-3d-08 review and is not a VER gate, consistent with its generative-guidance nature.)
...
7. **PLB-3d-07 — `class_model` fidelity and `refinement_kind` soundness (F105).** Review `class_model_binding` — the structured model captures the entity the source intends (no coined/fragmented entities; key attributes present); the recorded `refinement_kind` matches the transformation actually performed (especially `decompose` — Task → Task + TaskCompletion — and `introduce` for parentless surrogates). Where `profile_violation_row{n}:{field}` appears in `validation_failures` ([C]) or `class_model_statement_divergence` fired, confirm the row-profile fit and the prose projection. Structural validity is decidable (CHK-3d-11); this is the modelling judgement. Also review `semantic_type_registry.near_duplicates` ([G]) — consolidate concepts spelled multiple ways (`money`/`monetary_value`) — and the semantic *aptness* of each `semantic_type` (well-formed ≠ apt; aptness is not a hard check).
8. **PLB-3d-08 — Interrogative slot completeness (F87/F88).** Review `interrogative_completeness_advisory` (ADVC-3d-02) — a requirement whose type-required slots are thin, or whose Object was not interrogated to structure where the source implied it. A soft, generative-completeness review: an under-elaboration to surface, NOT a defect to reject (a missing *required* slot is already hard-caught by CHK-3d-09). Distinct from PLB-3d-02 (subject/vocabulary abstraction) and PLB-3d-07 (`class_model` fidelity); this is the slot-elaboration judgement.
...
**Assertions (two-step flow — Issue A):** **(migration)** `promote_dd_to_class_model` **strips** the legacy DD `lifecycle_state` value-set and all DD relationship entries, leaving DD names + synonyms only; a value-set with no backing surviving requirement emits `orphan_value_set_on_migration` (the fixture seeds one backed value-set → no orphan warning, and one unbacked → assert the warning). The migration **does NOT** write any `class_model` domain (no Structural `class_model` exists yet). **(first run)** the first Pass 3d run that derives the `Task` Structural builds its `class_model` `status` domain from the **Structural requirement content** (the prose value-list it asserts, de-duplicated against the Structural CHECK) — assert the domain ⊇ {available, claimed, completed}. **(object_refs)** in that same run the behavioural Requirement's `object_refs == ["Task.status.available"]` resolves end-to-end (VER-3d-28); a companion referencing `Task.status.archived` (absent value) is recorded in `object_refs_binding.dangling` with `object_refs_dangling`, NOT rejected.
...
**Primary inputs:**
- This spec (Row 4 Requirement Derivation v0.38) — implementation authority (DDL §5.1, schemas §5.2, guidance §5.4)
...
- **Issue 4 — migration scope (§12.3).** Common case is a clean strip (DD relationships + value-sets) leaving names + synonyms; promotion deferred to the first run (authoritative source = Structural requirement content); staging removed; `orphan_value_set_on_migration` Non-Loss guard; cross-spec ordering note (strip before/with the DD column-drop).
...
### 12.12 v0.30 → v0.31 change detail

v0.31 carries the four second-pass SW-review residuals; no new mechanism, no other spec touched (no ledger / DD / Row-3 change), realises Row 3 RD v0.19.

- **Issue A — Fixture 9 (§9.9).** Rewritten from the v0.29 staging/promotion assertion (which the v0.30 §12.3 removed) to the two-step flow: the migration **strips** DD value-sets/relationships (+ `orphan_value_set_on_migration` for any unbacked), and the **first Pass 3d run** builds the `class_model` domain from the Structural requirement content and materialises `object_refs`.
- **Issue B — §4.4.3a step 1.** The Structural sub-clause's pre-F107 relationship-triple wording (the retired `record_relationship`) and the "value → DD term" wording are corrected to names-only: present `class_model.entity` + each `class_model.relationships[].target` name; the relationship structure is `class_model.relationships`, a named value is `object_refs`.
- **Issue C — CHK-3d-11 Stage-4 referential (§4.3, §4.4.3a/c).** The inline per-Structural referential check would false-fail a same-row cross-entity target whose `class_model` is in-flight. Resolved with **two Stage-4 sub-passes** (option (c)): sub-pass 1 sets every `entity_ref` and builds the row's working `class_model` set; sub-pass 2 runs the referential resolution against the working set ∪ prior rows, before the §4.4.6 commit (so a reject removes the Requirement before any DB write).
- **Issue D — §7.** `orphan_value_set_on_migration` removed from the AnalysisPass `execution_warnings` enumeration (it is a migration-script log, not a mechanism warning); it remains the §12.3 migration output, with a note in §7.

**Re-implementation impact:** test-authoring (Fixture 9) and the Stage-4 two-sub-pass referential ordering in `stage4` / `core/class_model_validity.py`; §4.4.3a step 1 entity-name presentation; no schema/DDL change.

### 12.13 v0.31 → v0.32 change detail

v0.32 sweeps three same-pattern residuals the v0.31 edits left behind, plus every other instance of those patterns; no normative logic change, no other spec touched (no ledger / DD / Row-3 change), realises Row 3 RD v0.19.

- **Issue E — VER-3d-28 (§8.1).** The test description still asserted the v0.29 "migration promotes the value-set into the `class_model` domain" — the exact survival pattern of v0.30 Issue A (Fixture 9 updated, the paired verification row missed). Corrected to: migration **strips** (no `class_model` write); the **first Pass 3d run** builds the `Task.status` domain from the Structural requirement content.
- **Issue F — §4.4.3c Step 3.** The FK/target-resolution attribution "§4.4.3a Stage-4 referential check" was stale (Issue C moved the check to §4.4.3c sub-pass 2); re-pointed.
- **Issue G — §12.3 ¶3.** "first **v0.30** Pass 3d run" → "first run under this spec" (version-agnostic, so it does not re-stale next version).
- **Proactive sweep (same patterns, not separately flagged):** the §3.1 module comment for `promote_dd_to_class_model` ("DD value-sets → class_model domains") → strip-and-defer; the CHK-3d-11 intro and its rejection-location both pointed the *referential* check at §4.4.3a → re-pointed to §4.4.3c sub-pass 2 (`entity_ref` is *set* in §4.4.3a; the referential *check* runs in sub-pass 2). The Version / §12.x change-detail blocks are left as faithful per-version history.

**Re-implementation impact:** none beyond test-authoring (VER-3d-28 wording) and documentation accuracy. No schema/DDL/logic change.

### 12.14 v0.32 → v0.33 change detail

v0.33 fixes Issue H — the one remaining instance of the version-locked-forward-sentence pattern. Fixture 9 (§9.9) read "the first **v0.31** Pass 3d run … builds the `class_model` domain"; its paired VER-3d-28 was already version-agnostic (v0.32 Issue E/G), so the fixture was the un-swept half of the pair — the same split-pair failure mode as v0.30 Issue A → v0.31 Issue E. Corrected to "the first Pass 3d run that derives …". An exhaustive audit confirms no other forward-looking sentence is locked to a version number; remaining `v0.NN` references are stable history. No normative logic change.

### 12.15 v0.33 → v0.34 change detail

v0.34 is the first substantive F105/F107 *logic* change since v0.29, driven by Run 26's `class_model` integrity findings (the SW-agent diagnostics confirmed each root cause; the §4.4.3b audit blocks that should have surfaced them were written-but-never-read, now fixed agent-side). Six checks/rules:

- **[A] Strict `entity_ref` (§4.4.3a step 3).** The `class_model`'s own entity name binds by exact-match-or-mint, bypassing the AI-judge similarity path (root cause: `ChildEarnings`→`monetary value`). Entity identity ≠ semantic relatedness.
- **[B] CHK-3d-11 uniqueness (Stage-4 referential).** One `class_model` per `(entity, row)`; no shared `entity_ref`/`entity`; divergent duplicates are HARD. The `(statement, cci_refs)` dedup key (CHK-3d-07) cannot see entity identity.
- **[C] CHK-3d-11 attribute-profile by tier (Stage-3, HARD).** Each attribute's populated fields ⊆ the row's §5.4 field-set; Row 2 forbids physical `type`/`key`/`domain`/`target_ref`. Replaces the v0.33 soft `class_model_profile_advisory` (Run-26: 100% of Row-2 models over-populated to Row-4 detail).
- **[D] Row 1 authors no `class_model`** (`tier`∈{2..5}; `tier=1` rejected; Row 1 Structurals are scope prose).
- **[E] New CHK-3d-13** — a data-asserting Structural MUST carry a `class_model` (composition-predicate detector; constraint/policy Structurals stay prose). Kills the R023/R069/R072 prose duplicates.
- **[F] Drop the bare-`<Entity>` `object_refs` form** — `<Entity>.<attr>` minimum.

VER-3d-26 extended ([A]/[B]/[C]/[D]); VER-3d-28 ([F]); new **VER-3d-29** (CHK-3d-13); `core/structural_data_assertion.py`; new `validation_failures` `detail`s (`profile_violation_row{n}:{field}`, `tier1_class_model`, `duplicate_entity_model`, `data_assertion_without_class_model`) and warnings (`chk3d13_*`). **Re-implementation impact:** the entity_ref resolver's match path (strict for the model entity); CHK-3d-11 gains the profile + uniqueness + tier-1 clauses; new CHK-3d-13 + detector; the §5.4 Row-2 prompt restriction; object_refs ≥2-segment (already enforced). No ledger/DDL/DD-schema change. Realises Row 3 RD v0.20.

### 12.16 v0.34 → v0.35 change detail

v0.35 resolves the SW-agent v0.34 review — doc residuals, two clarifications, two prompt additions; **no change to the A–F logic.**

- **R1** the v0.33→v0.34 change-detail was mislabelled `§12.13` (duplicate) → **§12.15** — the split-pair label slip recurring; the numbering is now 12.10…12.15 contiguous.
- **R2** PLB-3d-07 → `profile_violation_row{n}:{field}` (the retired `class_model_profile_advisory`).
- **R3** §7 `execution_warnings` — `class_model_profile_advisory` removed (now a HARD `validation_failures` detail, [C]); `class_model_statement_divergence` retained.
- **R4** §6 mode table — CHK-3d-13 judge + repair IM sub-acts added.
- **R5/A2/A3** §4.3 budget table — CHK-3d-13 judge + repair calls; fingerprints `stage3_chk3d13_judge`/`_repair`; repair prompt = `requirement_derivation_prompt.py` (`class_model`-only, one entity, one call per failing Structural).
- **[C] clarification** — logical `type` at Rows 2–3 is a closed enum; a physical DB type there is `profile_violation_row{n}:type_not_logical`; the enum widens at Row 4+ (logical/physical decidable from the value).
- **[A] clarification** — strict binding is `resolve_and_record(..., strict=True)` skipping the AI judge; DD-spec reciprocal flagged for Row 4 DD v0.2.
- **P1/P2** — §5.4 Row 1 guidance gains the "no `class_model`" instruction; Row 2 gains the explicit profile-restriction (suppress over-generation at the prompt, not only the hard reject).
- **A1 / A4** — impl-aligns, spec shape stands: `model_coverage_gaps` is `[{parent_ref, element, kind, reason}]` (a bare `{entity}` is insufficient — an uncovered attribute within a covered entity must be identifiable); `class_model_binding.by_tier` is keyed by tier-NAME strings ({2:conceptual,3:logical,4:physical,5:detailed}), noted in the §4.4.3b example.

**Re-implementation impact:** the four unimplemented A/B/C/E changes are unchanged; this version only sharpens their spec text (the type-enum rule for [C], the `strict=True` contract for [A], the CHK-3d-13 fingerprints/prompt/budget for [E]) and adds the Row-1/Row-2 prompt restrictions. No DDL/ledger change.

### 12.17 v0.35 → v0.36 change detail

v0.36 clears two cross-reference residuals (PLB-3d-07's v0.29 reassignment orphaned ADVC-3d-02's PLB home) and a fingerprint-format inconsistency; no logic change.

- **R2** — ADVC-3d-02 (interrogative slot completeness) referenced PLB-3d-07, which became `class_model` fidelity at v0.29, leaving ADVC-3d-02 with no live PLB. Added **PLB-3d-08 — interrogative slot completeness**; the §4.3 ADVC-3d-02 body and the §8.1 parenthetical now point to it.
- **A1** — CHK-3d-13 fingerprints were `_R{nnn}`-suffixed in the body, bare in the §6/§4.3 tables. Harmonised to the bare stage form (`stage3_chk3d13_judge` / `stage3_chk3d13_repair`) to match the sibling convention (`chk3d12_repair`, `stage3_repair`); the per-call `requirement_id` is carried in the fingerprint payload, which is where call→requirement traceability belongs. Also removed a duplicate detector sentence the v0.35 A2/A3 edit left in the CHK-3d-13 body.
- **R1 (agent-reported) did not reproduce** — the §8.1 parenthetical already reviews CHK-3d-08 via PLB-3d-02 in v0.35.

No A–F logic change; no ledger/DD-schema change. Realises Row 3 RD v0.20.

### 12.18 v0.36 → v0.37 change detail

v0.37 adds **[G] attribute well-formedness**, closing the Run-28/29 gap where the CHK-3d-13 repair emitted `attr_name=null` + a POS-tag `semantic_type` that passed the present-only Row-2 check, leaving anonymous attributes that no `object_ref` can bind.

- **Hard gate (CHK-3d-11 Stage-3):** `attr_name` non-empty (`attr_name_empty`); `semantic_type` shape-valid lowercase snake_case (`semantic_type_malformed`) and not a part-of-speech tag (`semantic_type_pos_tag`). A **shape** gate, not an inclusion vocabulary — novel domain terms pass; the only closed list is the universal, project-independent POS set.
- **Soft accretion (§4.4.3c):** `core/semantic_type_registry.py` — exact-match-or-mint (like DD canonicals) with `semantic_type_near_duplicate` flagging for consolidation; never rejects a novel term, never pre-populated. `mechanism_data.semantic_type_registry`.
- **Aptness stays human:** whether a well-formed `semantic_type` is the *right* descriptor is a PLB-3d-07 judgement, not decidable.
- VER-3d-26 extended; §5.4 Row 2 reinforced (named + semantic-typed, no POS/physical/phrase); PLB-3d-07 note.

**Re-implementation impact:** CHK-3d-11 gains the attr_name + semantic_type shape/POS clauses; new `core/semantic_type_registry.py` + `mechanism_data` block; the producer must emit `attr_name` (the live Run-29 blocker). No DDL/ledger/DD-schema change. Realises Row 3 RD v0.21.

### 12.19 v0.37 → v0.38 change detail

v0.38 closes a spec gap that licensed the hollow `formed` count: §4.4.3a step 4 defined a dangling outcome only for an unresolved leading entity and an absent `.value`, never requiring the `.attr` segment to be verified — so a two-segment `<Entity>.<attr>` path passed vacuously and was reported `formed` whether or not the attribute existed.

- **`formed` is now a per-segment verification:** leading entity → canonical with a defining `class_model`; `.attr` → attribute of that model **by `attr_name`**; `.value` (where present) → that attribute's domain. A miss at any segment is a dangling binding with the segment-specific reason.
- **New dangling reason `attribute_not_in_class_model`** for an `.attr` that names no attribute of the resolved model. A two-segment path is explicitly **not vacuously formed** — the absence of a value-miss is not a pass.
- MD-8 + §5.5 aligned to per-segment verification; VER-3d-28 gains the `.attr`-miss fixture; §7 enumerates the three `reason`s.

This is the spec-side root of the Run-31/32 finding (`formed` climbing while every attribute was anonymous): the resolver was not violating the spec on `.attr`, because the spec never required it to check `.attr`. No ledger/DD-schema change. Realises Row 3 RD v0.22.

## Document End

End of SysEngage Row 4 Mechanism: Requirement Derivation v0.38. (v0.38: object_refs `formed` redefined as a per-segment verification — leading entity, `.attr` by `attr_name`, `.value` by domain must ALL resolve, else a dangling binding with a segment-specific reason; new `attribute_not_in_class_model` reason; a two-segment `<Entity>.<attr>` path is not vacuously formed. Closes the spec gap behind the hollow-`formed` finding (the spec never required `.attr` verification). MD-8/§5.5/VER-3d-28/§7 aligned; §12.19. No ledger/DD-schema change. Realises Row 3 RD v0.22.) (v0.37: [G] attribute well-formedness — CHK-3d-11 requires non-empty `attr_name` (`attr_name_empty`) and a shape-valid, non-POS `semantic_type` (`semantic_type_malformed`/`semantic_type_pos_tag`); a *shape* gate not an inclusion vocabulary, so novel domain terms pass while grammar tags/physical types/phrases are rejected. Soft accreting `semantic_type_registry` (exact-match-or-mint + `semantic_type_near_duplicate`, never rejecting, never pre-populated) on the open side; aptness stays a PLB-3d-07 judgement. VER-3d-26 extended; §12.18. No ledger/DD-schema change. Realises Row 3 RD v0.21.) (v0.36: R2 — added PLB-3d-08 (interrogative slot completeness); the two stale ADVC-3d-02 → PLB-3d-07 references (§4.3 body, §8.1 parenthetical) re-pointed to it (PLB-3d-07 became `class_model` fidelity at v0.29). A1 — CHK-3d-13 fingerprints harmonised to the bare `stage3_chk3d13_judge`/`_repair` form (sibling convention; `requirement_id` in the payload), and a duplicate detector sentence removed. R1 did not reproduce (CHK-3d-08 already → PLB-3d-02). §12.17. No A–F logic change. Realises Row 3 RD v0.20.) (v0.35: SW-review of v0.34 — R1 §12.13→§12.15 duplicate-label fix; R2 PLB-3d-07 → `profile_violation_row{n}:{field}`; R3 §7 drops the retired `class_model_profile_advisory` warning; R4 §6 mode table + R5 §4.3 budget table gain the CHK-3d-13 judge + repair; [C] logical `type` closed-enum clarification; [A] `strict=True` DD contract; P1/P2 §5.4 Row-1/Row-2 prompt restrictions; A1/A4 impl-align (spec shape stands). §12.16. No A–F logic change. Realises Row 3 RD v0.20.) (v0.34: Run-26 integrity findings → [A] strict exact-match-or-mint `entity_ref` (bypass the AI judge — `ChildEarnings`→`monetary value` root cause); [B] CHK-3d-11 `entity_ref`/entity uniqueness, one `class_model` per `(entity,row)`, divergent duplicates HARD; [C] CHK-3d-11 hard per-tier attribute profile (Row 2 forbids physical `type`/`key`/`domain`), replacing the soft advisory; [D] Row 1 authors no `class_model` (tier-1 rejected); [E] new CHK-3d-13 — data-asserting Structural MUST carry a `class_model`, constraint Structurals stay prose; [F] drop bare-`<Entity>` `object_refs` (≥2 segments). VER-3d-26 extended, VER-3d-28 [F], new VER-3d-29; §12.13. No ledger/DD-schema change. Realises Row 3 RD v0.20.) (v0.33: Issue H — Fixture 9 (§9.9) version-locked "first v0.31 run" made version-agnostic to match its paired VER-3d-28 and §12.3; exhaustive audit confirms no other forward-looking version-locked sentence remains. §12.14. No ledger/DD/Row-3 change. Realises Row 3 RD v0.19.) (v0.32: third SW-review pass — Issue E VER-3d-28 corrected to the migrate-strip-then-first-run-builds flow (the v0.30-Issue-A pattern surviving in a verification row); Issue F §4.4.3c Step 3 referential attribution re-pointed §4.4.3a→§4.4.3c sub-pass 2; Issue G §12.3 "first v0.30 run"→version-agnostic; plus a proactive sweep — §3.1 module comment strip-and-defer, CHK-3d-11 intro + rejection-location re-pointed to §4.4.3c sub-pass 2. §12.13. No ledger/DD/Row-3 change; no normative logic change. Realises Row 3 RD v0.19.) (v0.31: second SW-review pass — Issue A Fixture 9 rewritten to the migrate-strip-then-first-run-promotes flow; Issue B §4.4.3a step 1 Structural presents names only (`entity` + `relationships[].target`), the retired relationship-triple wording removed; Issue C CHK-3d-11 Stage-4 referential runs as a second sub-pass after the whole row's `class_model`s exist, so a same-row cross-entity target resolves; Issue D `orphan_value_set_on_migration` moved out of the §7 AnalysisPass `execution_warnings` (migration-only). VER-3d-26(b) + §12.12. No ledger/DD/Row-3 change. Realises Row 3 RD v0.19.) (v0.30: SW-agent spec-review resolutions (six issues) — Issue 1 `statement` Optional+validator (all three proposal classes); Issue 2 AI-facing `ClassModel.entity_ref` Optional + CHK-3d-11 split structural(Stage 3)/referential(Stage 4), `entity_ref` set from the DD in §4.4.3a (the AI authors the entity name, never a DD id); Issue 3 `object_refs_resolver` calls `dd_service.resolve_object_ref()` for the name half and queries `class_model` itself; Issue 4 migration simplified to a clean strip with deferred promotion + `orphan_value_set_on_migration` guard (staging removed); Issue 5 CHK-3d-12 parent set = non-retired parents (+Fixture 7 assertion); Issue 6 zero-attribute `class_model` hard-rejected, Row 2 needs ≥1 semantic-typed attribute. VER-3d-26 split; §12.11. Version-stamp quadruple swept. No ledger/DD-spec change. Realises Row 3 RD v0.19.) (v0.29: **F105** — structured data-model representation: the `requirement` table gains `class_model` (JSONB NULL, Structural) and `object_refs` (JSONB, behavioural) columns; `RequirementProposal` gains a `ClassModel` payload (Structural) and candidate `object_refs` (behavioural); Stage 2 formulates a Structural *as* a `class_model` at the row's population profile with `refinement_kind`/attribute-`origin` recorded at derivation; **CHK-3d-11** (structural validity, `core/class_model_validity.py` — a valid `class_model`-bearing Structural skips the CHK-3d-09 prose checks, the entity being the unit, D4) and **CHK-3d-12** (concept-coverage refinement, `core/class_model_coverage.py`, run per adjacent row pair — chain-together) added to Stage 3; §4.4.3c commits the `class_model` and renders the prose projection; **F107** — §4.4.3a rewritten to the names-only DD (relationship-record/value-record ops removed) plus a new step 4 materialising `object_refs` (name→DD canonical→`class_model` value, danglers recorded for the Quality pass); the one-time `promote_dd_to_class_model` migration. VER-3d-26/27/28; PLB-3d-07; Fixtures 8/9; `class_model_invalid`/`chk3d12_model_element_extinct`/`chk3d12_repair_performed`/`chk3d12_repair_failed`/`object_refs_dangling`. Version-stamp quadruple swept. Realises Row 3 RD v0.18 / ledger v2.17.) (v0.28: F106 — decompose executor discipline: `_parse_repair_response` made item-level resilient (a malformed child is logged `decompose_child_discarded` and discarded individually, never an all-or-nothing collapse — the F100/F101 swallow class again); decompose children inherit the parent's `cci_refs` by construction (repair schema drops `cci_refs` `minItems=1`, `confidence` Optional); the derivation-time two-step subsume-or-split test removed from the decompose prompt + a conjoined-predicate exemplar added, so the executor splits without re-deciding. VER-3d-25; `decompose_child_discarded`. Reframes F104(a) — R4_Run10's "retained" compounds ("retrieve and display tasks") were a decompose-mechanism failure, not un-splittability. Realises Row 3 RD v0.17.) (v0.27: F104(A) — `core/slots.py` detector precision: the hard branches split on "and" only, never disjunction ("or" in the predicate/object is a single-obligation choice — splitting inverts either→both, P1); the verb-on-both-sides test is scoped to the main predicate, exempting conjunctions inside a relative/subordinate clause (P2); the member-list carve-out covers operation/privilege lists (P3); temporal subordinators (prior to/before/after/then) are not conjunctions (P4); plus a `statement_preview` every-path sweep (F100 residual). VER-3d-24 asserts the four carve-outs do not hard-flag while a genuine conjoined predicate still does. Realises Row 3 RD v0.16; bounds *when* CHK-3d-09 fires — F98/F102/F103 unchanged. The structured data-model representation B item is split to F105.) (v0.26: F102 — retain-on-failure: a hard-rejected compound that cannot be decomposed is RETAINED in `surviving_09` (`chk3d09_decompose_failed_retained`), never dropped — the v0.25 build silently dropped it because orphan recovery doesn't fire when coarse CCIs are coincidentally covered (R5_Run12: 17+5 drops); VER-3d-22 extended from outcome-traced to obligation-present; the F100 `statement_preview` residual fixed on both emission sites; the "CHK-3d-05 carries the recovery" claim withdrawn. F103 — `core/slots.py` carves out inseparable member lists (enumeration value lists, column/attribute-definition lists — conjuncts are bare values/attribute-names under one enumeration/definition verb) before the compound-object hard branch, so they are logged-soft not hard-rejected. Realises Row 3 RD v0.15. Surfaced by R5_Run12 once F100 logging made decompose failures visible; F98/F99/F101 already validated at volume there.) (v0.25: F100 — CHK-3d-09 decompose-failure logging made per-invocation (not per-domain), audit invariant `|hard_reject| = |decompose_performed| + |decompose_failed|` per domain (VER-3d-22); F101 — Stage 4 DD-extraction batched and output-ceiling-bounded like Stage 2 Path-R (v0.19), truncation is a loud `dd_extraction_batch_truncated` hard failure with re-split (never a silent all-empty), `_parse_extraction_response` may not swallow a parse error, and the `extract_slot_terms` loop is guarded (`slot_parse_failed`); completeness invariant (requirement-level) `|reqs with ≥1 presented term| + |dd_zero_term| == row count` with `terms_presented == resolved + |dd_unresolved|` and no truncation (VER-3d-23, corrected on re-issue). Both surfaced validating F98/F99 on R3_Run7/Run8 + R5_Run11 — F98 validated (R5_Run11: 36 rejects / 30 decomposes), the DD-wipe was a 2048-ceiling truncation exposed by decompose enlarging the row, not a structural interaction. Physical-tier only; no Row 3 logical change.) (v0.24: F98 — CHK-3d-09 gains a conjoined-predicate hard branch in `core/slots.py` (verb on both sides of the conjunction), compound-object promoted from soft to hard, repair changed to automatic in-place decomposition with dependency carried as a Condition slot; F99 — §4.4.3a Constraint entity extraction from the Constraint-Rule not the Subject (`_extract_constraint_slots` + `_raw_slot_description` + `build_dd_extraction_prompt` redirect), `stmt[:60]` fallback removed → `dd_zero_term`, VER-3d-19 promoted from post-commit warn to pre-presentation reject; shared root — the slot machinery was Functional-Object-centric. Realises Row 3 RD v0.14. SW-agent-confirmed against R5_Run9.) (v0.23: SC-6 — §7 example places ai_model_fingerprints (and execution_warnings, concern_entities) at top-level outputs, siblings of mechanism_data, matching the implementation; example-structure fix.) (v0.22: §7 dd_binding example keys corrected to the canonical §4.4.3b / VER-3d-17 shape; example-key fix, no logic change.) (v0.21: five spec-silence points aligned to implementation — SC-1 any-domain IncrementalRerun fallback; SC-2/5 full IdempotentRerun mechanism_data sourced from prior pass; SC-3 downstream_rerun_required=false on IdempotentRerun; SC-4 repair AI-call output-budget table §4.3.) (v0.20: execution_status vocabulary standardised — Completed→Success, CompletedWithWarnings→PartialSuccess, Skipped retired (IdempotentRerun = Success + idempotent flag); Failed unchanged. Label rename, no logic change.) (v0.19: Path-R batching reconciled — token-ceiling-driven ≤15-seed batches, not one call; §7 audit example + warning types completed; version stamps swept. v0.18: Stage 4 connection-lifecycle conformance to Common Implementation Reference §1 — invalidate → refresh_engine_pool → fresh session across the IM boundary, validated PMT rows 1–5.)
