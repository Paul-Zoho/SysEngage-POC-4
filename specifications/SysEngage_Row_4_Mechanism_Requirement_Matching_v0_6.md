...
**v0.6 — provenance-blind cross-row matching (refinement-driven derivation, F87).** Under RD Row 4 v0.13, a Path-R requirement arrives with `refines_refs` set at derivation; a row-native (Path-N) requirement arrives empty. Matching is **provenance-blind** and keys only on whether `refines_refs` is populated. **D-rm-8 (new):** the cross-row refine judgement (§4.2/§4.3 refine/no-match/no_candidates branch) runs ONLY when `child.refines_refs == []`; a child arriving with `refines_refs` populated is **pre-linked** — verify each parent resolves at row_target − 1 (integrity), record `pre_linked`, skip the judgement, do NOT overwrite. Within-row duplicate-merge (D-rm-5/6/7) unchanged, applies to all. **Cardinality explicit:** «refine» is many-to-many, lower-bounded at the parent (every row n−1 parent ≥ 1 child — enforced at *derivation* CHK-3d-10), unbounded at the child (0..* parents); never assume 1→1 / tree. **Downward-gap (parent-orphan) detection moves to derivation** (CHK-3d-10); the matching-side parent-orphan check is demoted to a verification cross-check. §5.5/interface, §4.3 judge entry, and outcome enum (`pre_linked`) updated. No change to D-rm-5/6/7, confidence band, survivor logic.

**v0.5 (06 June 2026) — requirement_type pre-separates duplicates (D-rm-7); D-rm-5 subject extended to the Structural entity-subject.**
- **D-rm-7 (new) — requirement_type is a duplicate pre-separator**, parallel to row (D-rm-2) and subject class (D-rm-5). Only requirements of the **same `requirement_type`** (Functional / Constraint / Structural) are duplicate-eligible. A Functional, a Structural, and a Constraint requirement assert categorically different *kinds* of obligation — a behaviour, a data-structure fact, a bound — and are never duplicates of one another, regardless of shared entity / subject vocabulary. Decidable (`requirement_type` is a field), automatic, no IM weighing. §4.1 sibling set and §4.2 / §4.5 updated; new **VER-rm-13**. Fixes the observed cross-type merges (PMT R2 Run13: R030 Functional "a parent can define tasks" ↔ R014 Structural "each task shall have a monetary value"; R028 Functional ↔ R029 Constraint — the latter a recurring Non-Loss loss, a retention Constraint retired into a Functional record-keeping requirement, the same loss vector as F97/R025-R037).
- **D-rm-5 extended** — the subject-class set now explicitly includes the **Structural entity-subject** ("Each Task shall …", subject = the entity). A Structural requirement's subject is the entity it describes; it is a distinct subject class, not "unclassified". (In practice D-rm-7 already excludes Structural-vs-Functional/Constraint pairs by type; the subject-class refinement matters for the same-type case and removes the prior "unclassified → not excluded" hole.)
- Net rule: a duplicate requires **same row (D-rm-2) AND same requirement_type (D-rm-7) AND same subject class (D-rm-5)** — three decidable pre-separators — before the IM duplicate judgement runs. No change to D-rm-6 (`no_candidates`, validated live in Run13), the survivor logic (v0.3), or the provenance carrier (v0.2).
...
| **Row 4 Requirement Derivation v0.13 §5.5** | The caller-side interface (v0.13): derivation emits `refines_refs` **pre-set** for Path-R (seed-elaborated) requirements and **empty** for Path-N (row-native); this service populates only the empty ones (D-rm-8) and integrity-checks the pre-set ones |
...
- **D-rm-7 (v0.5) — requirement_type pre-separates duplicates.** A third decidable pre-separator on the duplicate branch, alongside row (D-rm-2) and subject class (D-rm-5). **Only requirements of the same `requirement_type` are duplicate-eligible.** Functional, Structural, and Constraint requirements assert categorically different kinds of obligation — a behaviour, a data-structure fact, a bound — so two requirements of *different* type are never duplicates, even when they share the same entity and read alike. This is decidable (`requirement_type` is a Requirement field), automatic, and applied *before* the IM duplicate judgement: cross-type same-entity pairs are excluded from the duplicate candidate set, never merged. Rationale from T&E (PMT R2 Run13): every damaging merge crossed a type boundary — R030 (Functional, "a parent can define tasks with values") was merged with R014 (Structural, "each Task shall have a monetary value"); R028 (Functional, "maintain a historical record") absorbed R029 (Constraint, "retain completion records for the required duration"), retiring a distinct retention bound into a record-keeping behaviour — the same Constraint-into-Functional Non-Loss loss F97 identified at R025/R037. Type-separation is the structural cause; it removes the loss vector at its root and is why the F97 deferred CCI-coverage merge-guard may prove unnecessary (every loss seen so far crossed a type boundary that this rule now blocks). The subject of a **Structural** requirement is the entity it describes ("Each Task shall…"); this is a legitimate subject class for D-rm-5, not "unclassified" — though D-rm-7 already excludes Structural-vs-other-type pairs by type.

## 4. Operation-by-Operation Implementation

### 4.1 `candidates(child)` — DD-based pre-filter

Resolve the child requirement's Object/entity to its canonical DD entry (via `resolve_object`). Candidate parents = row n−1 requirements whose Object resolves to the same canonical entry, or to an entry related to it (via a DD `relationship`). If the child's Object is DD-unresolved (DD flagged it, §5.2 of Row 3), the child is marked not-yet-matchable and skipped until its DD resolution completes. This pre-filter is what makes matching tractable — it replaces a free-text all-pairs comparison with an entity-anchored candidate set. **The candidate set returned here is retained for the provenance record** (§5.3) so the rejected candidates of a no-match are auditable. **Sibling set (D-rm-5, D-rm-7):** the same-row sibling candidates are filtered to the child's own **`requirement_type`** (D-rm-7) **and** subject class (D-rm-5); same-entity siblings of a different type or a different subject class are dropped from the duplicate candidates (they are different kinds of obligation, or complementary boundary/column aspects — not duplicates). **Empty-set signal (D-rm-6):** if the child is DD-resolved but the candidate-parent set is empty, that emptiness is carried through as `no_candidates` (§4.3), distinct from a judged no-match.
...
- Against **same-row candidate_siblings**: judge duplicate vs none. Output: duplicate-of id + confidence. **Pre-separation (D-rm-7, D-rm-5):** the candidate_siblings set contains only siblings of the **same `requirement_type`** AND the **same subject class** as the child; same-entity siblings of a different type (Functional / Structural / Constraint) or different subject class are excluded upstream (§4.1) and never judged duplicate — a different type is a different *kind* of obligation; a different subject class is a complementary boundary/column aspect. Neither is a repeat.
...
0. **Pre-linked short-circuit (D-rm-8, provenance-blind).** If `child.refines_refs != []`, the child was elaborated from its parent(s) at derivation: verify each referenced parent exists at row_target − 1 (dangling → flag); record outcome `pre_linked` with the pre-set parents; emit a match record; **skip steps 1+ (refine judgement)** and proceed only to within-row duplicate handling. Steps below run only for `child.refines_refs == []`.
...
| VER-rm-13 | two requirements of different `requirement_type` (Functional / Structural / Constraint) are never merged as duplicates, regardless of shared entity/subject (requirement_type pre-separation, D-rm-7) | `test_cross_type_not_merged` |
...
End of SysEngage Row 4 Mechanism: Requirement Matching Service v0.6.
