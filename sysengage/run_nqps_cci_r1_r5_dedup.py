"""
run_nqps_cci_r1_r5_dedup.py — Phase 3b CCI Construction for NQPS, Rows 1–5, dedup enabled.

Runs each row sequentially with skip_deduplication=False (full Stage 4a+4b+4c
deduplication sweep). Idempotent: skips any row that already has a
Completed / CompletedWithWarnings pass. Exports one JSON ledger on completion.

Intended to be run against a test branch cloned from snap_NQPS_ph03_3a_R5.

Invocation (from workspace root):
    python -u sysengage/run_nqps_cci_r1_r5_dedup.py
"""
