...
Filename: SysEngage_Row_4_Mechanism_Requirement_Derivation_v0_34.md
...
**Data-asserting detector (`core/structural_data_assertion.py`).** A Structural is *data-asserting* iff its statement (a) references a DD-`canonical` **entity** AND (b) asserts **composition** of that entity — a *have / comprise / define / contain / consist-of / associated-with / with-attribute* predicate over an **attribute, relationship, key, or domain**. The predicate+object test is DM-decidable; a genuinely ambiguous statement → IM judge (a single boolean, like the CHK-3d-08 subject judge). A **data-asserting Structural with no `class_model`** → conditional IM repair (re-derive it as a `class_model` scoped to that entity, the §4.4.3c path; `chk3d13_repair_performed`); persistent → `validation_failures {check_id:"CHK-3d-13", requirement_id, detail:"data_assertion_without_class_model"}`, `execution_warnings += chk3d13_data_assertion_unmodelled` (HARD). A non-data structural constraint is **exempt** (`chk3d13_constraint_exempt` recorded for audit). (Run-26: R023 "a task shall have an associated monetary value", R069, R072 are attribute assertions emitted as prose Structurals *alongside* the `Task` `class_model` — the duplication CHK-3d-13 prevents; they are re-derived into, or merged with, the `Task` model.)
...
**Primary inputs:**
- This spec (Row 4 Requirement Derivation v0.34) — implementation authority (DDL §5.1, schemas §5.2, guidance §5.4)
...
### 12.13 v0.33 → v0.34 change detail
