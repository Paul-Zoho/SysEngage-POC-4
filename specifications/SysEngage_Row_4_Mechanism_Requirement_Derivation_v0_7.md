...
1. **Term extraction (DM).** Functional → the **Object** slot (the entity the action operates on); Structural → the **entity** asserted, plus any **relationship** `(entity_a, relation, entity_b)` the statement asserts; any **value referenced by name** in any type (e.g. `Task.status.available`) → the attribute value. Extraction reuses the slot parse already computed for CHK-3d-09 / ADVC-3d-02 (the slots are identified there); no second AI call is required for extraction.
...
**Mode.** DM from this mechanism's perspective (it consumes a service result); the DD service's internal resolution judgement is IM within the DD service and is fingerprinted there, not here. No additional `ai_model_fingerprints` entry is added to the Pass 3d AnalysisPass for the DD resolution.
...
End of SysEngage Row 4 Mechanism: Requirement Derivation v0.7.

Physical realisation of the Row 3 (logical) Requirement Derivation spec v0.4, against ledger v2.15. Type/atomicity/schema reconciled (v0.5) PLUS interrogative-elaboration guidance (v0.6) PLUS Data Dictionary Object-slot binding activated (v0.7): §4.4.3a presents each Functional Object / Structural entity / asserted relationship / named value to the DD service's resolve-and-record during Stage 4 construction — the DD's incremental population path, with Pass 3d as primary caller; DM/service step, AI does not propose bindings; `dd_binding` audit block in mechanism_data; VER-3d-17/18 (binding complete; DD-non-empty regression guard). §5.4 slot-filling-by-interrogation across all rows; ADVC-3d-02 soft completeness advisory; hard typed-slot atomicity (CHK-3d-09). `refines_refs` schema present, populated by the Requirement Matching service (Row 3/4 v0.2). OQ-3d-01..05 resolved. F80 Open; F81 Open (Rows 1–2 validated, Rows 3–5 candidate pending test, Row 6 stub); F82/F87/F88/F89/F90 derivation portions realised here.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Requirement_Derivation_v0_4.md — logical authority (DD binding active, §5.5)
- SysEngage_Row_3_Mechanism_Requirement_Matching_v0_2.md / SysEngage_Row_4_Mechanism_Requirement_Matching_v0_2.md — populates `refines_refs`; downstream consumer of the populated DD
- SysEngage_Row_3_Mechanism_Data_Dictionary_v0_1.md / SysEngage_Row_4_Mechanism_Data_Dictionary_v0_1.md — the service §4.4.3a calls (resolve_and_record, relationship/value record)
- SysEngage_Row_4_Domain_Derivation_v0_24.md — structural sibling
- SysEngage_Issues_Tracker_v0_65.md — F80–F93 disposition
- sysengage_minimal_ledger_spec_v2_15.md — canonical Requirement / AnalysisPass / DD-type schema authority
