...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_15.md

Version: 0.15 (Row 1 prompt guidance — §5.4 ROW_ABSTRACTION_PHRASES replaced with structured ROW_GUIDANCE; Row 1 principle-based guidance block added; §8.2 PLB criteria updated — PLB-3c-01/05 extended for Row 1; PLB-3c-06 single-artefact domain prohibition added. Gaps 1/4/6 deferred in tracker. Supersedes v0.14.)
...
| **Row 4 Understanding v0.19** | §13 | Pass 3c implementation framework: module structure (row_abstraction_vocabulary.py absent), re-run scenarios (prose), ProjectProfile parameters (three), mode discipline, VER criteria → pytest mapping, 7-fixture table, Replit Agent handoff. |
...
| **Tracker v0.41** | F43, F53, F58 | F43: Action-Required (pass catalogue mode label). F53: Action-Required (Row 3 Understanding §E.12.1 amendment). F58: v0.12 execution_warnings/no_cci_input fixes (Resolved). |
...
| **Phase 2 — Mechanism Activation** | `project_profile` row — provides three domain-specific parameters (§13.9 of Row 4 Understanding v0.19). | Soft prerequisite — ProjectProfile must exist. If any parameter is NULL, default value is substituted. |
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.15) — implementation authority (all detail including DDL in §5.1)
...
- Row 4 Understanding v0.19 §13 — framework (module structure, prose descriptions, ProjectProfile parameters, VER criteria mapping, fixture table)
...
- **Understanding v0.19 §13.7 updated to match.**
...
- SysEngage_Row_4_Understanding_v0_19.md — implementation framework
...
End of SysEngage Row 4 Mechanism: Domain Derivation v0.15.

Changes from v0.14 (Row 1 prompt guidance — POC gap analysis):
- **§5.4:** `ROW_ABSTRACTION_PHRASES` replaced with `ROW_GUIDANCE`. Row 1 entry expanded from a single phrase to a structured principle-based guidance block. Block covers: what a Row 1 domain represents, appropriate/prohibited vocabulary, cross-column integration expectation, what to look for in the CCI set, and prohibition rules. Rows 2–5 retain short phrases pending their own POC review and validation cycles.
- **§8.2 PLB-3c-01:** Extended to call out Row 1 function-level naming as a PLB failure.
- **§8.2 PLB-3c-02:** Extended to note that single-column domains at Row 1 are a strong abstraction-level signal.
- **§8.2 PLB-3c-03:** Extended to note that high domain count at Row 1 signals feature-level grouping.
- **§8.2 PLB-3c-05:** Extended with explicit Row 1 vocabulary guidance.
- **§8.2 PLB-3c-06 (new):** Single-artefact domain prohibition — any Domain with exactly one CCI is flagged for mandatory review. Sources the Domain Derivation Prohibition Rule from the SysEngage POC §1.1.5.
