...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_22.md

Version: 0.22 (CHK-3c-08 domain concentration split added; ROW_GUIDANCE["2"] strengthened with single-objective test and "&" prohibition; SplitRepairResponseSchema added; VER-3c-14; PLB-3c-03 reframed. Supersedes v0.21.)

Date: 26 May 2026

**Purpose.** Implementation specification for the Domain Derivation mechanism (Pass 3c). v0.22 adds CHK-3c-08 — the symmetric counterpart to CHK-3c-07: over-concentrated domains (any domain containing more than half the row's CCIs) are automatically split via an IM repair prompt. Also strengthens ROW_GUIDANCE["2"] with the single-objective test and "&" prohibition from the LeChat POC. Supersedes v0.21.
...
| **Tracker v0.48** | F43, F53, F58 | F43: Action-Required (pass catalogue mode label). F53: Action-Required (Row 3 Understanding §E.12.1 amendment). F58: v0.12 execution_warnings/no_cci_input fixes (Resolved). |
...
Row 2 domain names use business responsibility vocabulary:
  ...
  Avoid: 'and' or '&' in domain names — these signal that two distinct business
         responsibilities have been merged into one domain
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.22) — implementation authority (all detail including DDL in §5.1)
...
End of SysEngage Row 4 Mechanism: Domain Derivation v0.22.

Changes from v0.21 (CHK-3c-08 + ROW_GUIDANCE["2"] strengthening):
- **§5.4 ROW_GUIDANCE["2"]:** Single-objective test added to qualification questions ("does this domain align with a single business objective, or multiple?"). Overlap detection question added ("are there overlapping domains that need to be consolidated or separated?"). "&" / "and" prohibition added to vocabulary signals and prohibition rules — drawn from Row 2 LeChat POC §3.3.1 and the Q15 explanation test. Addresses PMT D006 "Task Entitlement & Accountability" pattern.
- **§3.1 module structure:** `domain_split_repair_prompt.py` and `domain_split_repair_response_schema.py` added.
- **§4.3 CHK-3c-08 (new):** Over-concentrated domain split check — symmetric counterpart to CHK-3c-07. Fires when any domain has `len(cci_refs) > floor(eligible_count/2)`. Exceptions: `eligible_count <= 4`; `len(proposals)==1`; all domains concentrated (circular). IM repair prompt produces `List[SplitDomainProposal]`; merge replaces concentrated proposal with split sub-proposals; Non-Loss re-checked; CHK-3c-07 re-checked for newly thin proposals. Failure fallback: `chk3c08_repair_failed` advisory, domain left in place.
- **§5.2:** `domain_split_repair_response_schema.py` added — `SplitDomainProposal` with `name`, `description`, `cci_refs`, `rationale`. DISTINCT class. Four-schema IMPORTANT warning updated.
- **§7 mechanism_data:** `domain_split_issued: bool` and `splits: List[...]` added.
- **§7 execution_warnings:** Three CHK-3c-08 types added: `chk3c08_split_performed`, `chk3c08_repair_failed`, `chk3c08_all_domains_concentrated`.
- **§8.1 VER-3c-14 (new):** No active Domain has more CCI refs than `floor(cci_count_input/2)`, with sparse-row and circular-split exceptions.
- **§8.2 PLB-3c-03:** Reframed as residual check after CHK-3c-08 (same pattern as PLB-3c-06 after CHK-3c-07).

Changes from v0.20: ADVC-3c-01 inverted-range guard.
Changes from v0.19: Rows 3–5 prompt guidance.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md — architectural authority
- SysEngage_Row_4_Understanding_v0_24.md — structural framework
