...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_19.md

Version: 0.19 (Row 2 prompt guidance — §5.4 ROW_GUIDANCE["2"] expanded from short phrase to structured principle-based guidance block. Validated conceptually against 22 PMT Row 2 CCIs. Supersedes v0.18.)

Date: 26 May 2026

**Purpose.** Implementation specification for the Domain Derivation mechanism (Pass 3c). v0.19 adds the Row 2 prompt guidance block — a structured principle-based replacement for the previous single phrase, following the same approach validated for Row 1. Supersedes v0.18.
...
| **Tracker v0.45** | F43, F53, F58 | F43: Action-Required (pass catalogue mode label). F53: Action-Required (Row 3 Understanding §E.12.1 amendment). F58: v0.12 execution_warnings/no_cci_input fixes (Resolved). |
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.19) — implementation authority (all detail including DDL in §5.1)
...
End of SysEngage Row 4 Mechanism: Domain Derivation v0.19.

Changes from v0.18 (Row 2 prompt guidance):
- **§5.4 ROW_GUIDANCE["2"]:** Expanded from single phrase to structured principle-based guidance block. Covers: three domain qualification tests (bounded, persistent, distinct failure mode); Row 2 Zachman column semantics for all six columns; domain qualification questions (condensed from POC 15-question checklist); vocabulary signals; stakeholder actor guidance; prohibition rules.
- Validated conceptually against 22 PMT Row 2 CCIs — expected 4–5 business-responsibility-level domains vs. current 6 function-verb domains.
- Source: Domain_Elements_SysEngage_from_Row_2_Parts_1–4 — principle-based elements only; SysEngage-specific domain patterns excluded.

Changes from v0.17: CHK-3c-07 single-CCI absorption; VER-3c-13; PLB-3c-06 reframed.
...
- SysEngage_Issues_Tracker_v0_45.md — finding disposition
