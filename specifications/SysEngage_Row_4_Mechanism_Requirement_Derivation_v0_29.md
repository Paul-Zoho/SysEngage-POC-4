...
Filename: SysEngage_Row_4_Mechanism_Requirement_Derivation_v0_29.md
...
**CHK-3d-11 — `class_model` structural validity (decidable, HARD; realises Row 3 §4.1.1(g) / CHK-3d-11; F105).** For each surviving **Structural** Requirement carrying a `class_model` (`core/class_model_validity.py`):
- `entity_ref` resolves to a `canonical` `DataDictionaryEntry` (by query);
- `tier` matches `row_target` (2→`conceptual`, 3→`logical`, 4→`physical`, 5→`detailed`);
- **at most one** attribute has `key == "PK"`;
- every attribute `key == "FK"` carries a `target_ref` (or `keys[].target_ref`) that resolves to a known entity, and every `relationships[].target` resolves to a known entity;
- attribute `domain`s are well-formed (a parseable enumeration / range / type expression, non-empty where present);
- `refinement_kind` ∈ {`identity`,`decompose`,`realise_relationship`,`introduce`,`merge`} and each attribute `origin` ∈ {`refines`,`realises`,`introduced`} (enum-checked).

A `class_model` failing any clause is **rejected** (log `{check_id:"CHK-3d-11", requirement_id_placeholder, detail}` in `validation_failures`, `execution_warnings += class_model_invalid`); the Structural Requirement is removed from the surviving set and its obligation re-covered via the orphan (CHK-3d-05) / seed (CHK-3d-10) paths. **A valid `class_model`-bearing Structural SKIPS the CHK-3d-09 prose-atomicity checks** — the entity is the unit of atomicity (D4), so the conjoined-predicate / compound-object machinery does not apply; structural validity replaces it. (A prose-only Structural with no `class_model` — a migration residue — still runs CHK-3d-09.) Decidable; no IM.

**CHK-3d-12 — `class_model` concept-coverage refinement (decidable, downward Non-Loss over the model; rows ≥ 3; realises Row 3 §4.1.1(g) / CHK-3d-12 / OQ-105-01; F105).** The model-level analogue of CHK-3d-10 (`core/class_model_coverage.py`), run **per adjacent parent→child row pair** as each child row completes (chain-together, MD-7). For each row-n `class_model` and its `refines_refs` parent row n−1 `class_model`(s):
...
4. **`object_refs` materialisation — behavioural Requirements only (DM; F107).** `object_refs_resolver.py` processes each behavioural (Functional / Constraint) Requirement carrying candidate `object_refs` (the dotted paths the IM identified at Stage 2 — `<Entity>` / `<Entity>.<attr>` / `<Entity>.<attr>.<value>`). For each candidate: (i) resolve the leading `<Entity>` segment to a `canonical` `DataDictionaryEntry` (directly or via a `synonym`'s `resolves_to`); (ii) resolve the trailing `.<attr>` / `.<value>` against the `class_model`(s) whose `entity_ref` is that canonical (query the Structural Requirements defining the entity — typically this row and the rows above). A **fully-resolving** path is committed to `requirement.object_refs`. A path whose leading entity does not resolve, or whose `.<value>` is absent from every defining `class_model` domain, is a **dangling binding** — recorded in `object_refs_binding.dangling [{requirement_id, ref, reason}]` (`execution_warnings += object_refs_dangling`) for the Quality pass (F107-T1), and **NOT** a derivation-time reject: the defining Structural may be derived later (the value resolves when its row completes), so a reject here would false-fail a legitimate forward reference. `object_refs` is materialised AFTER §4.4.3c so the row's `class_model`s exist to resolve against.
...
3. **Commit.** Write the `class_model` JSONB onto the Requirement (§5.1). `entity_ref` denormalises `entity`; `refinement_kind` and attribute `origin` are persisted as the IM recorded them.
...
RequirementProposal:
  statement:            str                 (minLength=1)        # OPTIONAL when class_model present (projected, §4.4.3c)
...
ClassModel:             # mirrors ledger v2.17 ClassModel $def (schemas/class_model_schema.py)
  entity_ref:           str                 (pattern ^DD\d{3}$)
  entity:               str                 (minLength=1)
...
```
**Cross-field rules (Pydantic validators):** `class_model` present ⟺ `requirement_type=="Structural"`; `object_refs` present ⟹ `requirement_type` ∈ {Functional, Constraint}; a Structural proposal SHOULD carry a `class_model` at rows 2–5 (a prose-only Structural is logged un-promoted, not rejected — migration residue). The candidate `object_refs` the AI returns are **resolved and materialised** in §4.4.3a step 4 (the AI names the references; the DM resolves them through DD→`class_model`), exactly as the AI names entity terms but the DD resolves them.
...
- **Row 2 (conceptual):** attribute *existence* + *semantic type* ("money", "date", "identifier"); relationships as plain associations. NO physical types, keys, or domains.
...
| **VER-3d-26** | `class_model` structural validity (F105, CHK-3d-11): every Structural Requirement at row_target 2–5 SHOULD carry a `class_model`; every present `class_model` has `entity_ref` resolving to a canonical DataDictionaryEntry, `tier`==row, ≤1 `key=="PK"`, every FK `target_ref`/`relationships[].target` resolving, well-formed domains, enum-valid `refinement_kind`/`origin`; `class_model` is absent on behavioural Requirements | `test_class_model_validity` — for each Structural survivor assert the structural-validity clauses; assert DB CHECK rejects a `class_model` on a Functional row and an `object_refs` on a Structural row; negative fixture (two PKs / dangling FK target) asserts CHK-3d-11 reject + `class_model_invalid` |
...
`promote_dd_to_class_model.py` runs once, after `add_class_model_object_refs_columns` and before the first F105/F107 Pass 3d run. It is a reconciliation, not a discard:

1. **Promote value-sets.** For each legacy `canonical` DataDictionaryEntry carrying `attributes` with value-sets, promote each value-set into the `domain` of the corresponding attribute on the `class_model` of the Structural Requirement(s) defining that entity — **de-duplicated against the authoritative Structural CHECK domain** (the row Structural CHECK is the authority; the DD copies were noisy/duplicative — case-variants, invented noise). Where no `class_model` yet exists for the entity, the value-set is held in a staging table for the entity's first Structural derivation to absorb (it is never silently dropped — Non-Loss).
2. **Promote relationships.** Each retired `relationship` `entry_kind` becomes a `class_model.relationships` entry on the appropriate row's Structural Requirement.
3. **Strip the DD.** Delete the `canonical` `attributes` arrays and the `relationship` entries; the DD retains `name`, `description`, `provenance_ref`, `confidence`, and `synonym` entries only (ledger v2.17 / Row 4 DD v0.2).
4. **Establish `object_refs`.** For each behavioural Requirement whose prose references a now-`class_model`-defined value, form the `object_refs` dotted path (resolved through DD→`class_model`); unresolvable references are recorded as dangling for the Quality pass (not back-filled).

Idempotent and re-runnable (a second run finds nothing to promote). Validated by Fixture 9's migration assertions. Pre-v2.17 residual DD model content reaching a post-migration run is treated as un-migrated, not authoritative.
...
**Primary inputs:**
- This spec (Row 4 Requirement Derivation v0.29) — implementation authority (DDL §5.1, schemas §5.2, guidance §5.4)
