...
**Changes from v0.1 (F107 — reference vs specification; physical realisation of the DD-as-naming-index):** Realises ledger **v2.17**. The DD stops persisting model content: the `attributes` (JSONB), `from_ref`, `to_ref`, and `cardinality` columns are dropped; the `entry_kind` CHECK reduces to `('canonical','synonym')`; the `relationship_has_ends` constraint is removed (§5.1). The `record_relationship` and `record_value` operations are removed (§4.4–4.5) — relationships and value domains are `class_model` content authored by Requirement Derivation, not persisted here. `resolve_object_ref` replaces `relationships_of` (§4.7): it resolves the entity **name** here and hands the remaining attribute/value segments to the `class_model` whose `entity_ref` is that canonical. The Pydantic model reduces to canonical/synonym variants (§5.2). Logical authority: Row 3 DD v0.3.
...
**Purpose.** Implementation specification for the Data Dictionary service (findings F90, F85, F93, F107): builds and maintains the project-wide **naming/synonym index** that requirement names resolve to, judging each incoming surface term three-ways (canonical / synonym / flagged) under confidence-banded auto-resolve-with-register-review gating. The DD references the data model (`class_model`); it does not persist it. Records the physical realisation: module structure, DDL (names + synonyms only), the resolution-judgement implementation, the confidence constant, rejection propagation, audit, fixtures, and VER→pytest mapping. Logical authority: the Row 3 Data Dictionary Service spec v0.3.
...
| **Logical authority** | SysEngage Data Dictionary Service (Row 3 logical) v0.3 |
...
| **Realises ledger** | v2.17 (`DataDictionaryEntry`, `DataDictionaryRegister`; canonical / synonym; no model content) |
...
| Source | Relevance |
|---|---|
| **Row 3 Data Dictionary Service v0.3** | All sections — logical authority: operations, three-way resolution, gating, inclusion rule (names only), read operations, edge cases |
| **Canonical Ledger v2.17** | `DataDictionaryEntry` / `DataDictionaryRegister` schema (canonical / synonym; no model content, F107); DDL §5.1 realises it |
...
```
data_dictionary/
  service.py                  # public operations: resolve_term, resolve_object,
                              #   resolve_object_ref, aliases_of
  ...
  ddl/                        # Alembic migration realising v2.17
```
...
### 4.4–4.5 `record_relationship` / `record_value` — REMOVED (v0.2, F107)

Both operations are removed. Relationships are `class_model.relationships` and an attribute's legal values are a `class_model` attribute `domain` — authored by Requirement Derivation as `class_model` content, persisted on Structural Requirements, not in the DD. A caller that previously recorded `Task.status.available` now resolves the **name** `Task` here (`resolve_term`) and forms an `object_refs` path that resolves through this canonical to the `class_model` domain (Requirement Derivation §5.5; resolution read at §4.7 `resolve_object_ref`).
...
`resolve_object(term)` → canonical entry or unresolved-signal; `aliases_of(canonical_id)` → query `synonym WHERE resolves_to = canonical_id`; `resolve_object_ref(path)` → resolve the leading `<Entity>` segment to a canonical here, then resolve the remaining `.<attr>`/`.<value>` segments against the `class_model` whose `entity_ref` is that canonical (the DD resolves the name; `class_model` resolves attribute/value). Relationships and value domains are read from `class_model`, not the DD. All DD reads are direct queries (synonym-as-single-source makes aliases a query, not a field read).

## 5. Schema and Validation

### 5.1 Database DDL (realises ledger v2.17)

```sql
CREATE TABLE data_dictionary_entry (
  dd_id           VARCHAR(8)  PRIMARY KEY,           -- ^DD\d{3}$
  entry_kind      VARCHAR(16) NOT NULL CHECK (entry_kind IN ('canonical','synonym')),
  ...
  -- NO row_target column: the DD is project-wide (v2.17)
  -- NO attributes / from_ref / to_ref / cardinality: the model is class_model, not the DD (F107)
...
A discriminated model on `entry_kind` (canonical / synonym variants), mirroring the v2.17 conditional requireds: canonical requires name+description; synonym requires surface_term+resolves_to. No `attributes` / relationship fields (model content is `class_model`, F107). Distinct variant classes (same discipline as the Requirement Derivation distinct-schema warning).
...
| VER-dd-03 | no `canonical` row stores model content (`attributes`/relationship columns absent); model is `class_model` (F107) | `test_no_model_content` |
...
- **F-dd-5** `object_refs` path `Task.status.available` resolves: name `Task` resolved here, attribute/value resolved in the referenced `class_model` domain (`test_object_ref_resolves`).
- **F-dd-6** a dangling `object_refs` value (absent from every defining `class_model` domain) is flagged by the Quality check, not silently accepted (`test_dangling_object_ref_flagged`).
...
Per Row 3 §10: empty dictionary (all-canonical); term == canonical name (resolve, no synonym); a value/relationship referenced by a requirement (not DD content — resolved against `class_model` via `object_refs`); a structural/row-requirement term, e.g. `stored as DECIMAL(10,2)` (inclusion rule rejects — it is `class_model`, not vocabulary; logged). Model-judge failure → fail-safe to flagged (never auto-merge on error, D-dd-1 retry then flag).

## 9. Cross-Mechanism Interactions

- **Requirement Derivation (caller, §5.5):** presents Object-slot terms / Structural entities to `resolve_term` for **name** resolution; forms the behavioural requirement's `object_refs` by resolving each returned canonical's `class_model` (dotted path). A flagged term blocks that binding pending resolution.
...
- **State-model capability (deferred, F91):** transitions over an attribute's `class_model` domain values (the When-column state machine) are modelled against `class_model`, not the DD; not built.
...
End of SysEngage Mechanism: Data Dictionary Service (Row 4 — Physical) v0.2.

Physical realisation of the Row 3 logical Data Dictionary Service spec, against ledger v2.17. Two-tier resolution (deterministic pre-filter + model-judged semantic comparison); confidence-banded auto-resolve-with-register-review (fixed constant BAND=0.85, provisional); naming/synonym index only — relationships and value domains are `class_model` (F107); false-merge rejection with dependent-binding re-resolution; service-log audit. Completes the Data Dictionary mechanism pair (Row 3 logical v0.3 + Row 4 physical v0.2).

Companion artefacts:
- SysEngage_Row_3_Mechanism_Data_Dictionary_v0_3.md — logical authority
- sysengage_minimal_ledger_spec_v2_17.md — DataDictionaryEntry / DataDictionaryRegister schema authority (canonical / synonym; model in class_model)
