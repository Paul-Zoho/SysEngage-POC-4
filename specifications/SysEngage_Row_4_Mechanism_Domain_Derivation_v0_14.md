...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_14.md

Version: 0.14 (build-evidence corrections — domain_cci_membership join table removed; cell_content_item_refs stored as JSONB array directly on domain table; mechanism name corrected to CellContentItemConstruction; mode_active/declared_transformation_modes corrected on DomainDerivation AnalysisPass; ledger schema_id/version inconsistency noted. Supersedes v0.13.)

Date: 26 May 2026

**Purpose.** Implementation specification for the Domain Derivation mechanism (Pass 3c). v0.14 applies corrections identified from the first PMT production ledger export. The most critical correction: domain_cci_membership join table removed — cell_content_item_refs is stored as a JSONB array on the domain table, matching the canonical ledger v2.12 schema exactly. Supersedes v0.13.
...
| **Row 4 Understanding v0.18** | §13 | Pass 3c implementation framework: module structure (row_abstraction_vocabulary.py absent), re-run scenarios (prose), ProjectProfile parameters (three), mode discipline, VER criteria → pytest mapping, 7-fixture table, Replit Agent handoff. |
...
| **Tracker v0.40** | F43, F53, F58 | F43: Action-Required (pass catalogue mode label). F53: Action-Required (Row 3 Understanding §E.12.1 amendment). F58: v0.12 execution_warnings/no_cci_input fixes (Resolved). |
...
2. **PLB-3c-02 — Column span:** For each Domain, expand `cell_content_item_refs` and group by `cell_content_item.zachman_cell.column`. A Domain containing CCIs from only one column is flagged for review. Exception: a Why-only Domain (pure constraint domain) may be valid — verify intentionality.
...
| **Phase 2 — Mechanism Activation** | `project_profile` row — provides three domain-specific parameters (§13.9 of Row 4 Understanding v0.18). | Soft prerequisite — ProjectProfile must exist. If any parameter is NULL, default value is substituted. |
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.14) — implementation authority (all detail including DDL in §5.1)
...
- Row 4 Understanding v0.18 §13 — framework (module structure, prose descriptions, ProjectProfile parameters, VER criteria mapping, fixture table)
...
End of SysEngage Row 4 Mechanism: Domain Derivation v0.13.

Changes from v0.11 (two significant findings from internal consistency fix):
- **§4.4.6 (Finding 1 — Significant):** `no_cci_input` removed from the Stage 4 "Completed" informational list. It belongs to the §4.1 Stage 1 early-exit path, which bypasses Stage 4 and sets `CompletedWithWarnings` directly. §4.4.6 now carries an explicit scope note: it governs runs that reach Stage 4 only.
- **§7 (Finding 2 — Significant):** The blanket "entries in execution_warnings do NOT change execution_status" was false for `incremental_fallback_to_fullrerun`, which is §4.4.6 condition (b). The table now has a Status impact column. `incremental_fallback_to_fullrerun` is marked CompletedWithWarnings; `no_cci_input` is marked "Governed by §4.1". All other entries are Informational only.
- **§4.4.6 (Finding 2 — Significant):** Condition (b) now explicitly notes `incremental_fallback_to_fullrerun` is "the one `execution_warnings` entry that directly triggers CompletedWithWarnings".
- **Understanding v0.18 §13.7 updated to match.**

Changes from v0.10: internal consistency pass (six issues).
Changes from v0.9: four noted findings.
Changes from v0.8: two significant findings.
Changes from v0.7: ledger compliance.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md — architectural authority
- SysEngage_Row_4_Understanding_v0_18.md — implementation framework
- SysEngage_Issues_Tracker_v0_40.md — finding disposition

**This remains the designated handoff version.** Further spec iteration driven by implementation evidence only.

---
*v0.14 replaces the above document end note with the following:*

End of SysEngage Row 4 Mechanism: Domain Derivation v0.14.

Changes from v0.13 (build-evidence corrections — PMT first production ledger):
- **MD-4 / §5.1 / §4.4.3 (Critical):** domain_cci_membership removed; cell_content_item_refs JSONB array on domain table.
- **§6 (Significant):** mode_active build correction note added.
- All domain_cci_membership query references updated throughout.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md
- SysEngage_Row_4_Understanding_v0_18.md
- SysEngage_Issues_Tracker_v0_40.md
