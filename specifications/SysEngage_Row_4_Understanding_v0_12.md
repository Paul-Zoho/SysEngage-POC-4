...
**Filename:** SysEngage_Row_4_Understanding_v0_12.md (§13 addendum)

**Version:** 0.12 — §13.9 ProjectProfile parameters reduced from four to three (domain_upstream_match_threshold removed — upstream_domain_ref withdrawn); §13.11 Fixture 2 title updated (Logical qualifier label removed). Supersedes v0.11.

**Date:** 26 May 2026

**Purpose.** §13 carries structural guidance only. The full CREATE TABLE DDL (now simplified — no domain_qualifier or upstream_domain_ref columns) lives in Mechanism Spec v0.8 §5.1. v0.12 replaces v0.11 in full; all §1–§12 content from v0.5 remains authoritative.

**Scope note.** This document records §13 only.

**Precedence rule.** Where this artefact appears to differ from canonical ledger spec v2.12 or Row 4 Mechanism Spec (Domain Derivation) v0.8, the Mechanism Spec takes precedence.

---

## §13 Pass 3c — Domain Derivation Implementation Framework

This section establishes the Row 4 implementation framework for Pass 3c. The architectural authority is **SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md**. The implementation specification is **SysEngage_Row_4_Mechanism_Domain_Derivation_v0_8.md**. This section provides structural framework guidance — module structure, parameter tables, VER criteria mapping, fixture table, and Replit Agent handoff notes. Implementation detail (stage-by-stage behaviour, DDL, Pydantic schemas) lives in the Mechanism Spec §5.1 and is not duplicated here.
...
The `mechanism_data` sub-structure in `AnalysisPass.outputs` is defined in Mechanism Spec v0.8 §7. The implementation must populate ALL fields; zero-value arrays MUST be `[]` not null. The AnalysisPass is written outside the main ledger transaction (same discipline as Pass 3b Step 6).
...
Pass 3c requires the `domain` and `domain_cci_membership` tables and a DomainRegister seed row. The authoritative CREATE TABLE DDL is in **Mechanism Spec v0.8 §5.1**. The DDL is not duplicated here.
...
- Row 4 Mechanism Spec (Domain Derivation) v0.8 — primary implementation reference (all detail including DDL)
- This section (Row 4 Understanding §13 v0.12) — structural framework
...
- `migrations/XXXX_add_domain_tables.py` — creates `domain` and `domain_cci_membership` tables and seeds DomainRegister row (use DDL from Mechanism Spec v0.8 §5.1)
...
End of SysEngage Row 4 Understanding v0.12 — §13 addendum.

**Changes from v0.11:**
- §13.6: domain_qualifier and upstream_domain_ref bullets removed; Domain entity described as six canonical attributes only; transaction ref updated to §4.4.3
- §13.7: domain_data → mechanism_data throughout; unmatched_upstream_refs row removed; idempotent field added; field table reordered by Stage
- §13.8: DDL pointer updated to Mechanism Spec v0.8 §5.1; FK/DEFERRABLE note removed; "no domain_qualifier or upstream_domain_ref columns" noted explicitly
- §13.9: four → three parameters; domain_upstream_match_threshold row removed; withdrawal note added
- §13.10: domain_data → mechanism_data in mode violations reference
- §13.11: VER-3c-08/09/10 descriptions updated; fixture table: qualifier assertions removed, Skipped → Completed, Fixture 2 title updated
- §13.12: Skipped reference removed; IdempotentRerun gate clarified
- §13.13: upstream_domain_ref adaptation bullet removed; parameter count corrected to three; Mechanism Spec v0.8 as primary reference; row_abstraction_vocabulary.py module withdrawal noted
- §13.14: OQ-3c-02 updated to reflect withdrawal

§13 content from v0.11 is superseded by v0.12. All §1–§12 content from v0.5 unchanged.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md — architectural authority for Pass 3c
- SysEngage_Row_4_Mechanism_Domain_Derivation_v0_8.md — implementation spec
