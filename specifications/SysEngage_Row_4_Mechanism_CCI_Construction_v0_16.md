...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_16.md

Version: 0.16 (column_interrogatives.py — named-instance instruction for Where column at Rows 4 and 5)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). v0.16 adds enumeration split group protection to Stage 4a — sub-signals from the same Stage 3a-pre split are identified before pairwise evaluation, bypass auto-merge entirely, and route to Stage 4b as a protected group. Also removes Stage 4b diagnostic logging requirement (OQ-3b-11 resolved).
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_15.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.15** | All sections | Architectural spec including Stage 4a enumeration split group protection |
...
Before evaluating any pairs, scan the new candidate set for sub-signals from Stage 3a-pre splits. A candidate is an enumeration sub-signal if its `signal_refs` contains exactly one Signal id AND that id appears in the current batch's `enumeration_splits` record. Group all sub-signals sharing the same `original_signal_id` into a protected group.

Protected group handling:
- Set `stage4a_routed=True` on every member of the group
- Do NOT evaluate protected group members in the pairwise loop — neither against other new candidates nor against existing committed CCIs
- Pass all protected group members directly to Stage 4b as a named-instance group
- Record `stage4a_named_instance_routed` in `execution_warnings` with `{cell_id, classification_type, member_count}` reflecting the full group size

This ensures the entire named-instance group arrives at Stage 4b intact, regardless of whether any member has a similar existing CCI in the ledger.
...
Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_15.md
