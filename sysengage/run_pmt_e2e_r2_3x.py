"""
run_pmt_e2e_r2_3x.py — PMT Row 2 triple-run pipeline on a single test branch.

Workflow
--------
1.  Clone one disposable test branch from snap_PMT_ph03_3d_R1_3x_YYYYMMDD
...
Design notes
------------
- Clones from the Row 1 combined snapshot so all three projects already have
  SC committed.  SC idempotency check fires immediately for each project.
- Row-scoped idempotency strings (e.g. "All Sources (Row 2)") ensure RLSRA/
  CCI/DD/RD for Row 2 run fresh even though Row 1 passes exist on the branch.
- Source snapshot branch is resolved by name at runtime so no branch ID is
  hardcoded here.

Invocation (from workspace root):
    python -u sysengage/run_pmt_e2e_r2_3x.py
"""
...
# ---------------------------------------------------------------------------
# Step 1 — Create test branch from Row 1 combined snapshot
# ---------------------------------------------------------------------------
...
# Resolve source snapshot branch ID by name
...
def _run_one_pipeline(project_id: str, run_label: int) -> None:
    ...
    SC will be skipped (already committed from Row 1 snapshot).
    RLSRA/CCI/DD/RD use Row-scoped idempotency strings so Row 2 runs fresh.
    ...
    # ── Step 4: Source Capture (expect SKIP — already done in Row 1) ────────
    ...
    # ── Step 7: Domain Derivation Row 2 ─────────────────────────────────────
    ...
    # ── Step 8: Requirement Derivation Row 2 ────────────────────────────────
...
print(SEP, flush=True)
print("[3x] PMT Row 2 triple-run COMPLETE — production branch untouched", flush=True)
print(SEP, flush=True)
...
print(flush=True)
print("[3x] To clone a test branch for Row 3 from this snapshot:", flush=True)
print(
