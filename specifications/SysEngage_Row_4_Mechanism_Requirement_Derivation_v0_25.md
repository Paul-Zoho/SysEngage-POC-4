...
mechanisms/requirement_derivation/
  ...
  stage4_entity_production.py                  # DM: requirement_id allocation; domain_refs DM-derivation;
                                               #     Requirement construction; §4.4.3a DD entity-extraction + ops
...
| **VER-3d-22** | CHK-3d-09 decompose audit invariant (F100): within each `source_domain_id`'s proposal set, `|conjoined_predicate_hard_reject| == |chk3d09_decompose_performed| + |conjoined_predicate_decompose_failed|` (every hard-rejected compound left a decompose outcome — performed or failed — with no untraced residual) | `test_decompose_outcome_traced` — group warnings by `source_domain_id`; assert the per-domain identity; negative fixture forces ≥2 decompose failures in one domain and asserts 2 distinct `conjoined_predicate_decompose_failed` entries (not deduped) |
| **VER-3d-23** | Stage 4 extraction completeness + no swallowed truncation (F101): every surviving Requirement is accounted for at extraction — `terms_presented + |dd_zero_term| + |dd_unresolved| == row requirement count` — AND no batch recorded `dd_extraction_batch_truncated`; a row-wide `dd_entity_extraction_all_empty` is permitted ONLY when no batch truncated (otherwise it is a masked truncation) | `test_extraction_complete_and_untruncated` — assert the count identity holds; assert no `dd_extraction_batch_truncated` in a clean producing run; negative fixture forces an over-ceiling batch and asserts `dd_extraction_batch_truncated` fires and the batch is re-split (NOT all-empty); assert extraction runs in `stage4_dd_entity_extraction_batch<N>` stages, never one unbatched call |
...
**Primary inputs:**
- This spec (Row 4 Requirement Derivation v0.25) — implementation authority (DDL §5.1, schemas §5.2, guidance §5.4)
