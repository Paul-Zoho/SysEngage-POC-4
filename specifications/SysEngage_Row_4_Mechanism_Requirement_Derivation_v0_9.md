...
End of SysEngage Row 4 Mechanism: Requirement Derivation v0.9.

Physical realisation of the Row 3 (logical) Requirement Derivation spec v0.6, against ledger v2.15. Type/atomicity/schema reconciled (v0.5) + interrogative elaboration (v0.6) + DD Object-slot binding (v0.7) + DD entity-reduction term extraction (v0.8) + **Row 2 subject taxonomy / boundary test (v0.9)**: §5.4 REQUIREMENT_ROW_GUIDANCE["2"] subject block rewritten to a four-class taxonomy (actor / system-affordance / business / business-role) chosen by the boundary test; vocabulary block made subject-class-aware with the no-realisation-vocabulary rule as the WHAT/HOW guard for system subjects; atomicity block gains the over-generation brake (complementary actor/system pair, not duplicates); CHK-3d-08 Row 2 taxonomy widened (actor- and system-subject no longer mismatches). Fixes the false-merge cascade at root — the subject slot now discriminates. §5.4 slot-filling-by-interrogation; ADVC-3d-02 advisory; CHK-3d-09 hard atomicity. `refines_refs` populated by the Requirement Matching service (Row 3/4 v0.3). F80 Open; F81 Open (Rows 1–2 validated, Rows 3–5 candidate pending test, Row 6 stub); F82/F87/F88/F89/F90 derivation portions realised here; Row 2 subject taxonomy realises R2-AMEND-9 / OD-R2-30.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Requirement_Derivation_v0_6.md — logical authority (Row 2 subject taxonomy, §4.1.1(a))
- SysEngage_Row_2_Understanding_v1_5.md §2.3.3 — subject taxonomy & boundary semantics (R2-AMEND-9 / OD-R2-30)
- SysEngage_Row_3_Mechanism_Requirement_Matching_v0_3.md / Row_4 v0.3 — populates `refines_refs`; subject-class distinctness rule (duplicate logic) is the remaining cascade item
