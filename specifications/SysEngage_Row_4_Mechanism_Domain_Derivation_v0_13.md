...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_13.md

Version: 0.13 (self-review pass — §4.3 ADVC-3c-01 trailing paragraph simplified to §7 pointer; §2 tracker finding list updated to current; Understanding §13.2 and §13.7 collapsed to eliminate duplication driving churn. Supersedes v0.12.)

Date: 26 May 2026

**Purpose.** Implementation specification for the Domain Derivation mechanism (Pass 3c). v0.13 completes the self-review pass: two minor Mechanism Spec fixes plus Understanding restructure to eliminate parallel content that requires updating whenever the Mechanism Spec changes. This is the final handoff version. Supersedes v0.12.
...
| **Row 4 Understanding v0.17** | §13 | Pass 3c implementation framework: module structure (row_abstraction_vocabulary.py absent), re-run scenarios (prose), ProjectProfile parameters (three), mode discipline, VER criteria → pytest mapping, 7-fixture table, Replit Agent handoff. |
...
| **Tracker v0.39** | F43, F53, F58 | F43: Action-Required (pass catalogue mode label). F53: Action-Required (Row 3 Understanding §E.12.1 amendment). F58: v0.12 execution_warnings/no_cci_input fixes (Resolved). |
...
| **Phase 2 — Mechanism Activation** | `project_profile` row — provides three domain-specific parameters (§13.9 of Row 4 Understanding v0.17). | Soft prerequisite — ProjectProfile must exist. If any parameter is NULL, default value is substituted. |
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.13) — implementation authority (all detail including DDL in §5.1)
...
- Row 4 Understanding v0.17 §13 — framework (module structure, prose descriptions, ProjectProfile parameters, VER criteria mapping, fixture table)
...
- **Understanding v0.17 §13.7 updated to match.**
...
- SysEngage_Row_4_Understanding_v0_17.md — implementation framework
