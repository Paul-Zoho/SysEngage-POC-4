...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_9.md

Version: 0.9 (is_named_instance field; Stage 4b named-instance bypass)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). v0.9 adds `is_named_instance` to the Stage 3a response schema and candidate CCI struct, and adds the named-instance bypass to Stage 4b — groups where all new candidates carry `is_named_instance: true` are treated as Distinct without an AI cluster review call.
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_8.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.8** | All sections | Architectural spec: six-step pass structure, batch processing model, composite PK, clustering deduplication with group-size cap, Stage 4a three-condition rule, is_named_instance bypass, column vocabulary, merge rule, re-run semantics, verification criteria, edge cases |
...
Parse response against `cci_construction_response_schema.py`. The schema includes `column` and `is_named_instance` as fields:
```json
{
  "items": [
    {
...
- Construct candidate CCI struct: `{cell_id=f"ZC-R{row_ref}-C-{column}", classification_type, description, signal_refs, confidence, is_named_instance, trigger_condition, justification}`. `is_named_instance` defaults to `false` if absent from the AI response.
...
**Named-instance bypass (DM):** Before calling the AI, check whether all new candidates in the group carry `is_named_instance=True`. If yes: treat the entire group as Distinct without an AI cluster review call. All members survive unchanged. Add `{warning_type: "stage4b_named_instance_bypass", detail: {cell_id, classification_type, member_count}}` to the `execution_warnings` buffer.

If any new candidate in the group has `is_named_instance=False` (or the field is absent), the bypass does not apply — proceed to the AI cluster review call normally.
...
**`cci_construction_response_schema.py`** — for Stage 3a. Includes `column` and `is_named_instance` as fields:
```json
{
  "type": "object",
  "required": ["items"],
    ...
    "items": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "is_named_instance": {"type": "boolean", "default": false},
...
End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.9.

Changes from v0.8: `is_named_instance` optional boolean added to Stage 3a response schema (§4.3 and §5.3) and Stage 3b candidate struct; Stage 4b named-instance bypass added — groups where all new candidates carry `is_named_instance=True` treated as Distinct without AI cluster review; `stage4b_named_instance_bypass` warning type implicit in execution_warnings (defined in Row 3 §7); Row 3 spec reference updated to v0.8.

Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_8.md
