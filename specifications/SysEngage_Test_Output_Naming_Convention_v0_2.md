...
**Version:** 0.2
**Date:** 16 May 2026
**Changes from v0.1:** (1) passLabel table: `RowLensReanalysis` corrected to `RowLensSourceReanalysis` to match the `mechanism` field value in the AnalysisPass entity. (2) passLabel table: `SegmentCapture` entry added for Ph01 pass 0b. (3) ProjectID field definition: letters-only constraint made explicit. (4) Run number section: file-count assumption documented as a known constraint.
...
| Field | Format | Rules |
| --- | --- | --- |
| `{ProjectID}` | UPPERCASE letters only, 2 or more characters | Client project identifier. Matches `project_id` in the ledger. Letters only — no digits, no spaces, no hyphens. Example: `PMT`, `ACME`. This constraint is intentional at v1; if numeric suffixes are needed in future (e.g. `PMT2`), a convention revision is required. |
...
| `{passLabel}` | CamelCase mechanism name | Human-readable label for the pass. Must match the `mechanism` string on the AnalysisPass entity in the ledger. Example: `SourceCapture`, `RowLensSourceReanalysis`, `CCIConstruction`, `DomainDerivation`, `RequirementDerivation`. |
...
| PMT Phase 3 Row-Lens Re-Analysis, Row 1, first run | `PMT_Ph03_3a_RowLensSourceReanalysis_R1_Run1.json` |
...
**Known constraint:** The file-count approach is stateless and simple, which is a deliberate trade-off. It carries one known limitation: if a prior output file for the same `(project_id, phase, pass, row)` combination is deleted, renamed, or stored in a different directory, the counter will not account for it — producing a run number that either duplicates an existing file or resets unexpectedly. This is acceptable in a controlled test environment where output files are not moved or deleted between runs. Implementors must not rely on the run number being globally unique across storage locations or across file system operations outside the designated output directory.
...
| Ph03 | 3a | `RowLensSourceReanalysis` |
...
End of SysEngage Test Output Document Naming Convention v0.2.
