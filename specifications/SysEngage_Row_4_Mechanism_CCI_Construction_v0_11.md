...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_11.md

Version: 0.11 (Stage 4b mixed-group prompt variant clarified)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). v0.11 clarifies the Stage 4b prompt variant selection when a group contains both `stage4a_routed=True` candidates and existing CCIs: named-instance framing applies to the entire group if any member carries `stage4a_routed=True`.
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_10.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.10** | All sections | Architectural spec including Stage 4b mixed-group clarification |
...
- If **any member** of the group carries `stage4a_routed=True` (whether a new candidate or an existing CCI): use the named-instance framing variant for the entire group. This applies even when existing CCIs without a routing marker are present in the same group. The named-instance framing is a superset of the standard framing — it adds context without suppressing information the AI needs for standard duplicate review.
- If no member carries `stage4a_routed=True`: standard cluster review prompt.
...
End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.11.

Changes from v0.10: Stage 4b mixed-group clarification — if any member of a group carries `stage4a_routed=True`, named-instance framing applies to the entire group including existing CCIs; Row 3 spec reference updated to v0.10.

Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_10.md
