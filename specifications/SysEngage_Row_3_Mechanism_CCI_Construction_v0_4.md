...
**Architectural mechanism specification — v0.4**

Filename: SysEngage_Row_3_Mechanism_CCI_Construction_v0_4.md
...
**Status.** v0.4 — three targeted changes to Step 4, all driven by the ROW1 production-scale run (223 Signals, 23 batches, 310 CCIs). (1) Stage 4b pairwise comparison replaced with clustering — production data showed groups of 30–63 same-type CCIs, making pairwise comparison architecturally impossible (1,953 pairs for a 63-CCI group). (2) SSL connection resilience added to Stage 4b ledger read — production run showed database connection drop during Step 4 existing CCI query. (3) NoneType guard added to Stage 4b — downstream consequence of SSL failure caused `.confidence` access on None. No other sections changed.

**Changes from v0.3.** Stage 4b entirely rewritten: pairwise → clustering; SSL resilience on existing CCI read; NoneType guard before cluster review. Stage 4c updated: cluster verdict handling replaces pair verdict handling. §6.2 mode discipline table updated. §7 AnalysisPass outputs updated. §11.2 OQ-3b-03 and OQ-3b-04 recorded and resolved.
...
- All members of the group: for each item `{ref, source, description, signal_refs, confidence}` where `source ∈ {"new_candidate", "existing_cci"}`
...
**OQ-3b-03 — RESOLVED v0.4.** Stage 4b deduplication approach: pairwise comparison vs clustering. Production-scale run (ROW1, 223 Signals) produced groups of 30–63 same-type CCIs — pairwise is architecturally impossible at this scale (1,953 pairs for a 63-CCI group). Resolution: clustering — one AI call per group presenting all members; AI identifies duplicate clusters directly. No pairwise comparison.
...
End of SysEngage Row 3 Mechanism: CellContentItem Construction v0.4.

Changes from v0.3: Stage 4b pairwise replaced with clustering; SSL resilience and NoneType guard added to Stage 4b; Stage 4c updated for cluster verdicts; OQ-3b-03 and OQ-3b-04 recorded and resolved.

Companion artefact: SysEngage_Row_4_Mechanism_CCI_Construction_v0_4.md
