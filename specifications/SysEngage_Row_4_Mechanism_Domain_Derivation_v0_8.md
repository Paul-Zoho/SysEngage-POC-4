...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_8.md

Version: 0.8 (ledger compliance corrections — domain_qualifier and upstream_domain_ref withdrawn as redundant; execution_status "Skipped" → "Completed"; outputs.domain_data → outputs.mechanism_data; domain_upstream_match_threshold ProjectProfile parameter removed; DDL simplified; Stage 4 upstream heuristic removed; Understanding references updated to v0.12; tracker reference updated to v0.34)

Date: 26 May 2026

**Purpose.** Implementation specification for the Domain Derivation mechanism (Pass 3c). v0.8 applies ledger v2.12 compliance corrections identified after systematic audit: (1) domain_qualifier removed — redundant with row_target; (2) upstream_domain_ref removed — cross-row Domain tracing is navigable via CCI → Requirement → Domain path through existing canonical relationships; (3) execution_status "Skipped" corrected to "Completed" with idempotent note; (4) outputs sub-key renamed domain_data → mechanism_data for consistency with all other mechanisms. Supersedes v0.7.
...
| **Row 4 Understanding v0.12** | §13 | Pass 3c implementation framework: module structure, re-run scenarios (prose), ProjectProfile parameters (three), mode discipline, VER criteria → pytest mapping, 7-fixture table, Replit Agent handoff. |
...
| **Tracker v0.34** | F43, F44, F46–F53 | F43/F44: Action-Required. F46–F52: Resolved. F53: Row 3 Understanding §E.12.1 amendment (Action-Required). |
...
| **Phase 2 — Mechanism Activation** | `project_profile` row — provides three domain-specific parameters (§13.9 of Row 4 Understanding v0.12). | Soft prerequisite — ProjectProfile must exist. If any parameter is NULL, default value is substituted. |
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.8) — implementation authority (all detail including DDL in §5.1)
...
- Row 4 Understanding v0.12 §13 — framework (module structure, prose descriptions, ProjectProfile parameters, VER criteria mapping, fixture table)
...
End of SysEngage Row 4 Mechanism: Domain Derivation v0.8.

Changes from v0.7 (ledger v2.12 compliance corrections):
- **domain_qualifier withdrawn** (§1, §3.2, §4.4, §5.1, §5.3, §5.4, §6, §8, §9, §12): redundant with row_target. QUALIFIER_MAP, row_abstraction_vocabulary.py module, and domain_qualifier column removed throughout.
- **upstream_domain_ref withdrawn** (§1, §3.2, §4.4, §5.1, §7, §8, §9, §12): cross-row Domain tracing navigable via CCI → Requirement → Domain canonical path. Jaccard heuristic, find_upstream_match function, domain_upstream_match_threshold ProjectProfile parameter, unmatched_upstream_refs AnalysisPass field, and upstream_domain_ref DB column all removed.
- **execution_status "Skipped" → "Completed"** (§4.1, §4.4.6, §7, §8, §9, §11): "Skipped" is not a canonical enum value. IdempotentRerun now produces execution_status="Completed" with mechanism_data.idempotent=true.
- **outputs.domain_data → outputs.mechanism_data** (§4.1, §7, §8, §9): renamed for consistency with all other mechanisms (Pass 3b uses mechanism_data).
- **DDL simplified** (§5.1): domain_qualifier column, upstream_domain_ref column, CONSTRAINT fk_upstream_domain, DEFERRABLE INITIALLY DEFERRED all removed. Domain table now has five columns plus retired_at and created_at.
- **ProjectProfile parameters reduced from four to three** (§12): domain_upstream_match_threshold removed.
- **F-3c-02/F44 resolved** (§12.1): domain_qualifier withdrawal makes this moot — no migration needed, no default to confirm.
- **F53 added (Action-Required)**: Row 3 Understanding v1.1 §E.12.1 amendment required.
- **§12.3 build sequence updated**: row_abstraction_vocabulary.py module withdrawn; inline ROW_ABSTRACTION_PHRASES dict used instead.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md — architectural authority (note: domain_qualifier and upstream_domain_ref mentioned there are withdrawn)
- SysEngage_Row_4_Understanding_v0_12.md — implementation framework
- SysEngage_Issues_Tracker_v0_34.md — finding disposition

Next artefact in sequence:
- SysEngage_Row_4_Mechanism_Domain_Derivation_v0_9.md — after Replit Agent build and first PMT/NQPS production run
