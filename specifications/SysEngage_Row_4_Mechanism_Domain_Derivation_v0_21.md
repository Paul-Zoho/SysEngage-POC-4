...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_21.md

Version: 0.21 (ADVC-3c-01 spec defect fixed — inverted-range guard added for sparse rows; §10 edge case and §12 OQ-3c-01 updated. Two Run 5 production findings tracked. Supersedes v0.20.)

Date: 26 May 2026

**Purpose.** Implementation specification for the Domain Derivation mechanism (Pass 3c). v0.21 fixes a spec defect in ADVC-3c-01 — the domain count bounds formula produced an inverted range for sparse rows (cci_count <= 3), causing a misleading advisory to fire. Fixed with an inverted-range guard. Supersedes v0.20.
...
| **Tracker v0.47** | F43, F53, F58 | F43: Action-Required (pass catalogue mode label). F53: Action-Required (Row 3 Understanding §E.12.1 amendment). F58: v0.12 execution_warnings/no_cci_input fixes (Resolved). |
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.21) — implementation authority (all detail including DDL in §5.1)
...
End of SysEngage Row 4 Mechanism: Domain Derivation v0.21.

Changes from v0.20 (ADVC-3c-01 spec defect — surfaced by PMT Run 5 Row 4 production evidence):
- **§4.3 ADVC-3c-01 (Bug fix):** Inverted-range guard added. When `floor(cci_count/2) < 1 + ceil(cci_count/15)` (i.e. `cci_count <= 3`), the formula produces an incoherent range where upper_bound < lower_bound. The advisory is now skipped entirely in this case. Previously: Row 4 PMT (1 CCI) fired `domain_count_advisory` with `lower_bound=2, upper_bound=0` — an impossible range that misled Practitioners. The guard is self-documenting: the advisory only fires when the bounds define a valid, non-degenerate range.
- **§10 edge case table:** "Single CCI for the row" entry updated — now correctly states no advisory fires (inverted-range guard), CHK-3c-07 skipped (cci_count_input<=1 exception), execution_status=Completed.
- **§12 OQ-3c-01:** Resolution note updated to include the inverted-range guard and the condition under which it applies (cci_count <= 3).
- **Tracker F73:** ADVC-3c-01 sparse row defect (Resolved).
- **Tracker F74:** Run 5 Row 5 When-002 absorption target (Noted — When-002 absorbed into D014 child UI; analytically better fit would be D013 deployment; guidance noted but not blocking).
...
- SysEngage_Issues_Tracker_v0_47.md — finding disposition
