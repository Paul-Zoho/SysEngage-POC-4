...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_1.md

Version: 0.1 (Initial — Pass 3b implementation spec)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). Translates the Row 3 Mechanism Specification (SysEngage_Row_3_Mechanism_CCI_Construction_v0_1.md) into Row 4 implementation detail consistent with the SysEngage v1 architectural commitments (SysEngage_Row_4_Applied_to_SysEngage_v0_2.docx) and canonical ledger spec v2.12. At depth tier (i)+ per Row 4 Understanding §8: architectural specification + verification criteria + test fixtures. Suitable for handoff to Replit Agent for implementation.

**Excludes.** Per Row 4 Understanding §8.1: NO pseudo-code, NO function signatures, NO code-level interface definitions. Replit Agent derives these from the architectural specification + canonical schemas. The spec describes WHAT each step does and WHAT correctness looks like; Agent decides HOW to write the code.
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_1.md — all sections |
| **Operational location** | Phase 3 Pass 3b. Executes after Pass 3a (Row-Lens Source Re-Analysis) completes; before Pass 3c (Domain Derivation). Six steps: Step 1 (Signal query, DM), Step 2 (ZachmanCell upsert + Signal grouping, DM), Step 3 (per-cell CCI derivation, IM+DM), Step 4 (deduplication, DM+IM+DM), Step 5 (identifier allocation + commit, DM), Step 6 (AnalysisPass production, DM). |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.1** | All sections | Architectural spec: six-step pass structure, column vocabulary, merge rule, re-run semantics, verification criteria, edge cases |
| **Row 4 Understanding v0.3** | §12 | Pass 3b implementation framework: per-cell processing pattern, Signal-to-CCI derivation pattern, deduplication pattern, re-run-aware commit pattern, module structure |
...
mechanisms/cci_construction/
  ...
  prompts/
    ...
    column_interrogatives.py             # Row × Column framing table (Row 4 Understanding §12.2.1)
...
- **Single transactional ledger write** — all ledger writes for Pass 3b (ZachmanCell upserts, CCI inserts, CCI updates, CellContentItemRegister updates, AnalysisPass) committed atomically in one Postgres transaction at the end of the mechanism execution. AI API calls happen before the transaction opens. Per Row 4 Applied §5 transactional discipline.
...
### 4.2 Step 2 — Upsert ZachmanCells and Group Signals by Cell

**Mode:** DM. No AI involvement. Ledger write (ZachmanCell upsert + register update).

**Step 2a — ZachmanCell upsert:**

For each column in `[What, How, Where, Who, When, Why]`:
- Construct `cell_id = f"ZC-R{row_ref}-C-{column}"`
- UPSERT into ZachmanCell table: if `cell_id` exists, no-op; if not exists, INSERT with `row_target = str(row_ref)`, `column = column`.
- After all six upserts: UPDATE ZachmanCellRegister `member_ids` to include all six cell_ids (idempotent set union).

**Step 2b — Signal grouping:**

Partition the eligible Signal working set by column. Each Signal's `column` attribute (set during Pass 3a classification) determines its group. Result: a dict `{column: [Signal]}` with six keys. Columns with no Signals have empty lists.

**Output:** Six groups. ZachmanCells committed to ledger.

---

### 4.3 Step 3 — Per-Cell CCI Construction

**Mode:** IM (Stage 3a) + DM (Stage 3b, 3c). Per-cell iteration. Ledger write deferred to Step 5.
...
- `row_ref`: integer
- `column`: column name
- `column_interrogative`: the analytical framing for this column at this row (from `column_interrogatives.py[row_ref][column]`)
- `permitted_classification_types`: list from `column_vocabulary.py[column]`
...
Parse response against `cci_construction_response_schema.py`:
```json
{
  "items": [
    {
...
Validation failure: record malformed items in `candidates_rejected` buffer; proceed with valid items (PartialSuccess for this cell).
...
- Validate `signal_refs` is non-empty — reject if empty; record in `candidates_rejected`
- Validate `confidence` ∈ [0.0, 1.0] — reject if outside range; record in `candidates_rejected`
- Validate `description` is non-empty — reject if empty; record in `candidates_rejected`
- Construct candidate CCI struct (no ci_id yet): `{cell_id, classification_type, description, signal_refs, confidence, trigger_condition, justification}`

**Stage 3c — Pydantic validation (DM):**

Apply `CCICandidateModel` Pydantic validation to each candidate struct. Additional cross-field validations:
- Each `signal_refs` entry must resolve to a Signal in the eligible working set for this run
- `cell_id` must match the current cell being processed

Malformed candidates recorded in `candidates_rejected`. Valid candidates added to cell's candidate buffer.

**AI failure handling:**
- Retry up to 3 times with exponential backoff (1s, 2s, 4s).
- If all retries fail: cell produces no candidates for this run. Recorded in AnalysisPass failure detail. Continue to next cell. execution_status = CompletedWithWarnings if other cells succeeded; Failed if all cells failed.

---

### 4.4 Step 4 — Per-Cell Deduplication Sweep

**Mode:** DM (Stage 4a) + IM (Stage 4b) + DM (Stage 4c). Per-cell. No ledger write (merge execution updates in-memory structs).

For each cell's candidate buffer from Step 3:

**Stage 4a — Structural pre-filter (DM):**

For each pair of candidates in the buffer:
- Compare `classification_type` (string equality) AND `signal_refs` (set equality: `set(a.signal_refs) == set(b.signal_refs)`)
- If both match: structural duplicate. Apply merge rule (§4.4.1). Remove the lower-confidence candidate from the buffer; update the higher-confidence candidate's struct with merged attributes.
- Process all pairs before proceeding (N² comparison, acceptable for expected CCI counts per cell).

**Stage 4b — AI semantic review (IM):**

Read existing committed CCIs for the current cell from ledger: SELECT CCI WHERE `cell_id = current_cell_id`.

Group candidates and existing CCIs by `classification_type`. For each group with more than one member (new candidates + existing CCIs combined):

Construct prompt from `dedup_semantic_review_prompt.py` with parameters:
- `cell_id`: current cell
- `column`: column name
- `column_interrogative`: analytical framing
- `pairs`: for each pair within the group: `{item_a: {source, ref, description, signal_refs}, item_b: {source, ref, description, signal_refs}}` where `source ∈ {"new_candidate", "existing_cci"}` and `ref` is a candidate index or ci_id

Call Claude Sonnet API. Record AI model version fingerprint.

Parse response against `dedup_review_response_schema.py`:
```json
{
  "verdicts": [
    {
...
- If both items are new candidates: apply merge rule to candidate structs in-memory; record merge in merges buffer.
- If one is an existing CCI (source="existing_cci") and the other is a new candidate: mark the new candidate as "merge-into-existing"; record the existing CCI's `ci_id`, the merge attributes (`signal_refs` union, `confidence` max, `description` from `merged_description` if non-null), and both original descriptions in the merges buffer. The new candidate is NOT added as a new entity.

For each `Ambiguous` verdict:
- Both items survive.
- Reduce confidence of each by 0.1 (floor at 0.0) — update in candidate struct or existing CCI update buffer.
- Record in consolidation_flags buffer with `{cell_id, pair_ref}`.

**Consolidation threshold check (DM):**

After all verdicts applied: compute `ratio = (candidates_in - surviving_new_candidates) / candidates_in` where `candidates_in` is the Stage 4a input count (before any deduplication). If ratio > `ProjectProfile.cci_consolidation_threshold` (default 0.80): record in consolidation_flags buffer.

#### 4.4.1 Merge Rule (DM)

Applied in Stage 4a (structural pre-filter) and Stage 4c (merge execution):

| **Attribute** | **Merge outcome** |
| --- | --- |
| `ci_id` | If merging with existing CCI: existing ci_id preserved. If merging two new candidates: neither has a ci_id yet; result is one candidate struct. |
| `signal_refs` | `list(set(a.signal_refs) union set(b.signal_refs))` — ordered by signal_id for determinism |
| `confidence` | `max(a.confidence, b.confidence)` |
| `description` | If `merged_description` from AI is non-null: use merged_description. Else: description from the higher-confidence item. |
| `classification_type` | Unchanged (must match for merge to have been triggered) |
| `trigger_condition` | Higher-confidence item's value if both have it; whichever has it if only one does; null if neither. |
| `justification` | Concatenate both with `" | "` separator if both non-null; use whichever is non-null if only one is. |
...
```python
{
    ...
    "execution_status": computed_status,  # Completed / CompletedWithWarnings / Failed
    "mode_active": "DM",
    ...
    "outputs": {
        "cci_data": {
            ...
            "candidates_rejected": count_of_rejections_across_all_cells,
...
**`cci_construction_response_schema.py`** — for Stage 3a:
```json
{
  "type": "object",
  "required": ["items"],
  "properties": {
    "items": {
      ...
      "items": {
        "type": "object",
        "required": ["classification_type", "description", "signal_refs", "confidence"],
        "properties": {
...
**`dedup_review_response_schema.py`** — for Stage 4b:
```json
{
  "type": "object",
  "required": ["verdicts"],
  "properties": {
    "verdicts": {
      ...
      "items": {
        "type": "object",
        "required": ["item_a_ref", "item_b_ref", "verdict", "rationale"],
        "properties": {
          "item_a_ref": {"type": "string"},
          "item_b_ref": {"type": "string"},
          "verdict": {"type": "string", "enum": ["Duplicate", "Distinct", "Ambiguous"]},
...
### 9.1 Fixture 1 — Happy path: single populated cell (What column)

**Purpose:** Verify Step 1–6 end-to-end for a minimal case. One cell, multiple Signals, straightforward CCI derivation, no deduplication needed.

**Pre-conditions in ledger:**
- `row_ref = 2`
- Signals present (row_target="2", column="What"):
  ...
  - SG003: description="Transactions are categorised as income or expenditure", signal_type="Normative", confidence=0.80
- No other Signals for row 2.

**Expected outcomes:**
- VER-3b-01: 6 ZachmanCells exist for row 2 (ZC-R2-C-What through ZC-R2-C-Why)
- VER-3b-09: AnalysisPass with mechanism="CellContentItemConstruction" exists for row 2
- ZC-R2-C-What has at least 2 CCIs (expect: Transaction entity, Account entity, possibly Category entity)
- All CCIs have ci_id matching `^CCI-ROW2-C-What-\d{3}$`
- All CCI.signal_refs entries resolve to SG001, SG002, or SG003
- All CCIs have classification_type ∈ {"Entity", "Attribute", "Relationship"}
- VER-3b-10: SG001, SG002, SG003 each appear in at least one CCI.signal_refs
- Cells ZC-R2-C-How, Where, Who, When, Why have zero CCIs (no Signals)
- AnalysisPass.outputs.cci_data.cells_populated = 1, cells_empty = 5

### 9.2 Fixture 2 — Deduplication: two Signals producing overlapping candidates

**Purpose:** Verify Stage 4a structural pre-filter fires correctly when two Signals produce identical candidates.

**Pre-conditions in ledger:**
- `row_ref = 2`
- Signals (row_target="2", column="Who"):
  - SG010: description="Parents manage the system on behalf of children", signal_type="Actor", confidence=0.88
  - SG011: description="Children are the primary users who view balances", signal_type="Actor", confidence=0.82

**Expected outcomes:**
- At least one Actor CCI for "Parent" role and one for "Child" role
- If AI produces duplicate Actor CCIs for the same role from different Signals: Stage 4a or 4b should merge them; merged CCI.signal_refs contains both signal ids
- VER-3b-07: sequence numbers unique within Who cell
- AnalysisPass.outputs.cci_data.merges is populated if any merge occurred
...
Default value 0.80 applies. This is the expected case at v1 — threshold is not yet empirically validated. F41 (deferred) covers future calibration work for this parameter.
...
| --- | --- | --- |
| **RA-1** | Batch size for Stage 4b semantic review — how many pairs per AI call? | Present all same-classification_type pairs for a cell in one call. Expected cell sizes are small (5–20 CCIs per cell). If a cell produces >30 same-type pairs, split into batches of 20. |
...
- Row 3 Mechanism Spec v0.1 (architectural reference)
- Row 4 Understanding v0.3 §12 (implementation patterns)
...
- Per-cell iteration instead of per-chunk iteration
- Two-phase AI invocation (Step 3a derivation + Step 4b deduplication)
- Re-run-aware deduplication (comparing to existing committed CCIs)
- In-place update of existing CCIs (signal_refs, confidence, description)

---

## Document End

End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.1.

Companion artefacts produced with this spec:
- SysEngage_Row_3_Mechanism_CCI_Construction_v0_1.md — Row 3 architectural spec
- SysEngage_Row_4_Understanding_v0_3.md — Row 4 Understanding additions
