...
**Filename:** SysEngage_Row_4_Understanding_v0_27.md (§14 addendum)

**Version:** 0.27 — §14 reindexed to the Tier 2-reconciled Requirement Derivation specs (Row 3 logical v0.2, Row 4 physical v0.5; ledger v2.13) per findings F82–F93, and the **de-version-and-pin** convention applied: inline body references name the Mechanism Spec without a version suffix; the exact pinned version lives only in the §14.5 handoff table, so future mechanism increments no longer force an Understanding re-version unless the indexed *structure* changes. Substantive reconciliations: VER-3d-09 updated to the three-value enum; VER-3d-15 (hard typed-slot atomicity, CHK-3d-09) and VER-3d-16 (refines_refs validity) added; the `requirement_type` triad and `refines_refs` noted; ledger reference updated to v2.13. Supersedes v0.26 (which went to the build agent indexing mechanism v0.2). §1–§12 and §13 (Pass 3c) remain authoritative and unchanged; v0.27 supersedes v0.26 by §14 replacement only.

**Date:** 02 June 2026

**Purpose.** §14 carries structural guidance only for Pass 3d — module structure, ProjectProfile parameters, VER→pytest mapping, fixture→pytest mapping, and Agent handoff notes. All implementation detail (stages, DDL, Pydantic schemas, prompt text, REQUIREMENT_ROW_GUIDANCE content, edge cases, OQ resolutions) lives in the Row 4 Requirement Derivation Mechanism Spec (version pinned in §14.5) and is not duplicated here. This is the same structural/implementation firewall established for §13 (Pass 3c): the Understanding indexes the Mechanism Spec; it does not restate it.

**Scope note.** This document records §14 only.

**Authority chain.** Pass 3d is specified at two abstraction levels. The **logical authority** is the Row 3 Requirement Derivation Mechanism Spec (what the mechanism must do and why). The **physical/implementation spec** is the Row 4 Requirement Derivation Mechanism Spec (the buildable realisation), which §14 indexes. The Row 4 Domain Derivation Mechanism Spec is the **structural sibling** (shared four-stage pattern and conventions). Exact pinned versions are in the §14.5 handoff table. Where this artefact appears to differ from the canonical ledger spec (v2.13) or the Mechanism Spec, the Mechanism Spec takes precedence.

---

## §14 Pass 3d — Requirement Derivation Implementation Framework

Structural framework for Pass 3d. Logical authority: the Row 3 Requirement Derivation Mechanism Spec. Implementation spec: the Row 4 Requirement Derivation Mechanism Spec (versions pinned in §14.5) — all implementation detail lives there and is not duplicated here.
...
| `requirement_large_cci_set_advisory_threshold` | integer | 80 | Row CCI count above which the large-set advisory fires (signal only — per-Domain processing proceeds; no chunking) |
...
| VER-3d-09 | All `requirement_type` values in `{Functional, Constraint, Structural}` | `tests/test_requirement_derivation.py` |
...
| VER-3d-15 | No surviving Requirement violates typed-slot atomicity (CHK-3d-09, HARD) — no surviving `atomicity_violation`; survivors parse to their type's required slots | `tests/test_requirement_derivation.py` |
| VER-3d-16 | `refines_refs` valid — empty as produced by Pass 3d; if present (post-Matching), each references an existing Requirement at `row_target − 1` | `tests/test_requirement_derivation.py` |

**CHK-3d-08 (row-subject vocabulary)** is a Stage 3 decidable check, soft severity — it records `mechanism_data.subject_vocabulary_flags` and is reviewed via PLB-3d-02. **CHK-3d-09 (typed-slot atomicity)** is a Stage 3 decidable check, HARD severity — it rejects compound/slotless statements and is gated by VER-3d-15. It is NOT a VER gate (Mechanism Spec §4.3, §8.1, OQ-3d-03). Listed here so the Agent does not mistake its absence from the VER table for an omission.
...
- Row 4 Mechanism Spec (Requirement Derivation) **v0.5** — primary implementation reference (all detail including DDL §5.1, schemas §5.2, REQUIREMENT_ROW_GUIDANCE §5.4). Logical authority: Row 3 Requirement Derivation **v0.2**
...
- Canonical Ledger v2.13 — Requirement and RequirementRegister entity schemas
- Segmentation spec v9.2 — requirement statement formulation discipline

**This is a re-implementation against the reconciled Mechanism Spec, not a fresh build.** v0.1 is already implemented (it produced the PMT Row 1 / NQPS Row 1 runs). Per the Mechanism Spec §12.8, the code changes from the v0.1 implementation are:
...
- **CHK-3d-08** (row-subject vocabulary) has no Domain Derivation analogue — soft severity; **CHK-3d-09** (typed-slot atomicity) is hard severity
...
- CHK-3d-08 severity is soft (OQ-3d-03); CHK-3d-09 (atomicity) is hard
...
- **F81** (Open → Row 1 portion addressed): §5.4 REQUIREMENT_ROW_GUIDANCE["1"] + CHK-3d-08 are the Row 1 closure path. F81 stays Open until Rows 2–6 guidance is authored in their own validation cycles. Re-running PMT Row 1 / NQPS Row 1 under the reconciled guidance is the F81 Row 1 validation step.

---

## Document End

End of SysEngage Row 4 Understanding v0.27 — §14 addendum.

**Changes in v0.27 (Tier 2 reconciliation, findings F82–F93):**
- §14 reindexed to the reconciled Requirement Derivation specs — Row 3 logical **v0.2**, Row 4 physical **v0.5** — and to canonical ledger **v2.13**.
- **De-version-and-pin convention adopted:** inline body references now name the Mechanism Spec without a version suffix; the exact pinned version appears only in the §14.5 handoff/companion list. Future mechanism increments no longer force an Understanding re-version unless the indexed *structure* (modules, parameters, VER/fixture inventory, handoff) changes.
- VER-3d-09 reconciled to the three-value enum `{Functional, Constraint, Structural}` (F89); **VER-3d-15** (hard typed-slot atomicity, gating CHK-3d-09, F88) and **VER-3d-16** (`refines_refs` validity, F82) added to the index.
- CHK-3d-09 (hard atomicity) and the `refines_refs` field (Matching-service-populated) noted alongside the existing CHK-3d-08 (soft) index lines.
- Supersedes v0.26 (provided to the build agent, indexing mechanism v0.2). §1–§12 and §13 (Pass 3c) unchanged; v0.27 supersedes v0.26 by §14 replacement only.

**Changes in v0.26 (prior, retained for traceability):**
...
Companion artefacts (pinned versions):
- SysEngage_Row_3_Mechanism_Requirement_Derivation_v0_2.md — logical authority (Tier 2-reconciled)
- SysEngage_Row_4_Mechanism_Requirement_Derivation_v0_5.md — physical implementation spec for Pass 3d (Tier 2-reconciled)
- SysEngage_Row_4_Mechanism_Domain_Derivation_v0_24.md — structural sibling (four-stage pattern)
- SysEngage_Issues_Tracker_v0_61.md — F80–F93 disposition
- sysengage_minimal_ledger_spec_v2_13.md — canonical Requirement / RequirementRegister schema authority
