...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_5.md

Version: 0.5 (Group size cap; execution_warnings schema field; RA-1 retired)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). v0.5 adds a group-size cap of 50 to Stage 4b clustering, adds `execution_warnings` to the `cci_data` schema, and retires the stale RA-1 open question replacing it with the resolved group-size cap guidance.
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_5.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.5** | All sections | Architectural spec: six-step pass structure, batch processing model, composite PK requirement, clustering deduplication with group-size cap, column vocabulary, merge rule, re-run semantics, verification criteria, edge cases |
...
mechanisms/cci_construction/
  ...
  prompts/
    ...
    dedup_cluster_review_prompt.py      # Template: semantic equivalence review
  ...
  schemas/
    ...
    dedup_cluster_review_response_schema.py      # Pydantic: AI deduplication response validation
```
...
**Connection handling:** Acquire a fresh database connection for the existing CCI read — do not reuse any connection held open since Step 3. Execute: SELECT all CCIs WHERE `cell_id = current_cell_id AND project_id = current_project_id`. If the query raises a connection or operational error: retry up to 3 times with 1s/2s/4s backoff. If all retries fail: set `existing_ccis = []`; record `step4_read_failure` in AnalysisPass failure detail; proceed with new candidates only.

**NoneType guard:** Before entering the cluster review, validate every item in the combined set (new candidates + existing CCIs). Any item that is None or lacks a `confidence` attribute is excluded from the cluster review and recorded in AnalysisPass failure detail. Do not allow a NoneType to reach Stage 4b or 4c logic.

Combine surviving new candidates and existing CCIs. Group by `classification_type`. For each group with more than one member:

**Group-size cap:** If a group contains more than 50 members, split it into sub-groups of 50 (last sub-group may be smaller) before calling the AI. Process each sub-group as a separate AI call. After all sub-groups have been reviewed, apply Stage 4c merge execution across all sub-group cluster results together. Record `step4_sub_group_split` in `execution_warnings` with `{cell_id, classification_type, group_size, sub_group_count}`.
...
```python
{
    ...
    "outputs": {
        "cci_data": {
            ...
            "execution_warnings": execution_warnings_buffer  # step4_read_failure, step4_nonetype_excluded, step4_sub_group_split
        },
        "mode_violations": []
    },
...
- This Implementation Spec v0.3 (primary input)
...
End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.5.

Changes from v0.4: group-size cap of 50 added to Stage 4b with sub-group splitting; `execution_warnings` added to AnalysisPass outputs in §4.6; RA-1 retired and replaced with resolved group-size cap guidance; Row 3 spec reference updated to v0.5.

Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_5.md
