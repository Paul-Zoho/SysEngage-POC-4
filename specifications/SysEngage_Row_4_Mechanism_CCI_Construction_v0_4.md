...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_4.md

Version: 0.4 (Stage 4b clustering; SSL resilience; NoneType guard)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). v0.4 replaces Stage 4b pairwise AI comparison with clustering, adds SSL connection resilience to the Step 4 existing CCI read, and adds a NoneType guard before the cluster review. Driven by production-scale ROW1 run (223 Signals, 23 batches, groups of up to 63 same-type CCIs making pairwise impossible).
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_4.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.4** | All sections | Architectural spec: six-step pass structure, batch processing model, composite PK requirement, clustering deduplication, column vocabulary, merge rule, re-run semantics, verification criteria, edge cases |
...
Make **one AI call per group** using `dedup_cluster_review_prompt.py`:
...
End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.4.

Changes from v0.3: Stage 4b pairwise replaced with clustering; SSL connection resilience and NoneType guard added; Stage 4c updated for cluster verdicts; dedup prompt and schema renamed to `dedup_cluster_review_prompt.py` and `dedup_cluster_review_response_schema.py`; cluster response schema replaces pairwise schema in §5.3; module structure updated.

Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_4.md
