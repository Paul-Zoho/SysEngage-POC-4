...
End of SysEngage Row 4 Mechanism: Requirement Derivation v0.8.

Physical realisation of the Row 3 (logical) Requirement Derivation spec v0.5, against ledger v2.15. Type/atomicity/schema reconciled (v0.5) PLUS interrogative-elaboration guidance (v0.6) PLUS Data Dictionary Object-slot binding activated (v0.7) PLUS DD term extraction corrected to entity reduction (v0.8): §4.4.3a step 1 is an IM reduction of the Object to its domain entity head(s) — entity-grade noun phrases presented to the DD, never the verbatim Object-slot clause; model-assisted and fingerprinted (`stage4_dd_entity_extraction`); VER-3d-19 guards against clausal terms. §4.4.3a remains the DD's incremental population path with Pass 3d as primary caller; `dd_binding` audit block; VER-3d-17/18 (binding complete; DD-non-empty guard). §5.4 slot-filling-by-interrogation across all rows; ADVC-3d-02 soft completeness advisory; hard typed-slot atomicity (CHK-3d-09). `refines_refs` schema present, populated by the Requirement Matching service (Row 3/4 v0.2). OQ-3d-01..05 resolved. F80 Open; F81 Open (Rows 1–2 validated, Rows 3–5 candidate pending test, Row 6 stub); F82/F87/F88/F89/F90 derivation portions realised here.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Requirement_Derivation_v0_5.md — logical authority (DD binding active + entity-term reduction, §5.5)
- SysEngage_Row_3_Mechanism_Data_Dictionary_v0_2.md / SysEngage_Row_4_Mechanism_Data_Dictionary_v0_1.md — the service §4.4.3a calls; v0.2 fixes the candidate-term contract (entity-grade, not clausal)
