...
**Filename:** SysEngage_Row_4_Understanding_v0_19.md (§13 addendum)

**Version:** 0.19 — §13.4 prompt parameterisation note updated (ROW_ABSTRACTION_PHRASES → ROW_GUIDANCE; Row 1 structured guidance block noted). Supersedes v0.18.

**Date:** 26 May 2026

**Purpose.** §13 carries structural guidance only. DDL lives in Mechanism Spec v0.15 §5.1. v0.19 replaces v0.18 in full; all §1–§12 content from v0.5 remains authoritative.

**Scope note.** This document records §13 only.

**Precedence rule.** Where this artefact appears to differ from canonical ledger spec v2.12 or Row 4 Mechanism Spec (Domain Derivation) v0.15, the Mechanism Spec takes precedence.

---

## §13 Pass 3c — Domain Derivation Implementation Framework

This section establishes the Row 4 implementation framework for Pass 3c. The architectural authority is **SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md**. The implementation specification is **SysEngage_Row_4_Mechanism_Domain_Derivation_v0_15.md**. This section provides structural framework guidance — module structure, parameter tables, VER criteria mapping, fixture table, and Replit Agent handoff notes. Implementation detail (stage-by-stage behaviour, DDL, Pydantic schemas) lives in the Mechanism Spec §5.1 and is not duplicated here.
...
mechanisms/domain_derivation/
  ...
  stage4_entity_production.py              # DM: domain_id allocation; Domain entity construction (six canonical
                                           #     attributes, cell_content_item_refs as JSONB array on domain row);
...
- The CCI delta calculation for IncrementalRerun uses `query_committed_cci_ids_for_row(row_ref, project_id)` — a live DB query using `jsonb_array_elements_text(cell_content_item_refs)` on the `domain` table, not a stored field in `mechanism_data`.
...
| `row_guidance` | `ROW_GUIDANCE[str(row_ref)]` | For Row 1: a structured multi-paragraph guidance block (see Mechanism Spec §5.4). For Rows 2–6: a single short phrase injected inline. The prompt template handles both forms. |
...
`ROW_GUIDANCE` is defined in the prompt template file (Mechanism Spec §5.4). Row 1 carries a structured principle-based guidance block covering: what a Row 1 domain represents, appropriate/prohibited vocabulary, cross-column integration expectation, what to look for in the CCI set, and prohibition rules. Rows 2–5 retain short phrases pending their own POC review and validation cycles. The `row_abstraction_vocabulary.py` module has been withdrawn.
...
The `mechanism_data` sub-structure in `AnalysisPass.outputs` is defined in Mechanism Spec v0.15 §7. The implementation must populate ALL fields; zero-value arrays MUST be `[]` not null. The AnalysisPass is written outside the main ledger transaction (same discipline as Pass 3b Step 6).
...
**`execution_warnings` — placement note:** `execution_warnings` is a **standard top-level AnalysisPass field** outside `mechanism_data`. The complete enumeration of all seven advisory types, their source sections, and their status impact is in **Mechanism Spec v0.15 §7** — that is the single authoritative source and is not duplicated here. Key point: most entries are informational only; `incremental_fallback_to_fullrerun` is the one exception that triggers `CompletedWithWarnings` per §4.4.6 condition (b).

---

### §13.8 Database Schema

Pass 3c requires the `domain` table and a DomainRegister seed row. The authoritative CREATE TABLE DDL is in **Mechanism Spec v0.15 §5.1**. The DDL is not duplicated here.

Key schema decisions:
- `cell_content_item_refs` is a **JSONB array on the `domain` table** — matching canonical ledger v2.12 directly. No `domain_cci_membership` join table (retired after first PMT production run confirmed it broke canonical compliance).
- `retired_at` is the soft-delete column — `NULL` = active; non-null = retired.
- Composite PK `(domain_id, project_id)` on the `domain` table.
...
| VER-3c-03 | Assert `jsonb_array_length(cell_content_item_refs) >= 1` for all active domains | `tests/test_domain_derivation.py` |
| VER-3c-04 | Assert all ci_ids in `cell_content_item_refs` resolve to `cell_content_item` with matching row_target | `tests/test_domain_derivation.py` |
| VER-3c-05 | Assert every eligible CCI ci_id appears in at least one Domain's `cell_content_item_refs` array | `tests/test_domain_derivation.py` |
...
- Row 4 Mechanism Spec (Domain Derivation) v0.13 — primary implementation reference (all detail including DDL)
- This section (Row 4 Understanding §13 v0.17) — structural framework
...
- `cell_content_item_refs` stored as JSONB array directly on `domain` row (no join table)
...
- `migrations/XXXX_add_domain_tables.py` — creates `domain` table (with `cell_content_item_refs JSONB`) and seeds DomainRegister row (use DDL from Mechanism Spec v0.15 §5.1)
...
End of SysEngage Row 4 Understanding v0.17 — §13 addendum.

**Changes from v0.19:**
- §13.4: prompt parameterisation note updated — `ROW_ABSTRACTION_PHRASES` → `ROW_GUIDANCE`; Row 1 structured guidance block described; parameter renamed from `abstraction_level_phrase` to `row_guidance`
- All Mechanism Spec references updated v0.14 → v0.15; tracker → v0.41

§13 content from v0.18 is superseded by v0.19. All §1–§12 content from v0.5 unchanged.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md — architectural authority for Pass 3c
- SysEngage_Row_4_Mechanism_Domain_Derivation_v0_15.md — implementation spec
- SysEngage_Issues_Tracker_v0_41.md — finding disposition
