"""
run_pmt_cci_r1_r5.py — Phase 3b CCI Construction for PMT_E2E, Rows 1–5, dedup disabled.

Runs each row sequentially with skip_deduplication=True (raw duplicate level —
no cluster-based dedup sweep).  Idempotent: skips any row that already has a
Completed / CompletedWithWarnings pass to prevent double-runs.

Invocation (from workspace root):
    python -u sysengage/run_pmt_cci_r1_r5.py
"""
...
print(f"[runner] Phase 3b CCI Construction — {PROJECT_ID}  Rows {ROWS}  skip_dedup=True", flush=True)
print(flush=True)
...
print(f"[runner]   Total new CCIs: {total_ccis}", flush=True)
