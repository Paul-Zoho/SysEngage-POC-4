...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_8.md

Version: 0.8 (Stage 4a third condition — description similarity check)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). v0.8 adds the Stage 4a third condition: description similarity check. Auto-merge only when `classification_type` matches AND `signal_refs` are identical AND descriptions are materially similar (≥60% token overlap). Where type and refs match but descriptions differ materially, candidates are routed to Stage 4b. Adds `stage4a_similarity_threshold` to ProjectProfile configuration.
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_7.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.7** | All sections | Architectural spec: six-step pass structure, batch processing model, composite PK, clustering deduplication with group-size cap, Stage 4a three-condition rule, column vocabulary, merge rule, re-run semantics, verification criteria, edge cases |
...
End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.8.

Changes from v0.7: Stage 4a three-condition rule added — Jaccard similarity check (default threshold 0.60, ProjectProfile parameter `stage4a_similarity_threshold`); routing rules for all three match/mismatch combinations specified; `stage4a_similarity_threshold` added to §3.2 design decisions; Row 3 spec reference updated to v0.7.

Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_7.md
