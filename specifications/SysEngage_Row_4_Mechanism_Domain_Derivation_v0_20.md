...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_20.md

Version: 0.20 (Rows 3–5 prompt guidance — §5.4 ROW_GUIDANCE["3"], ["4"], ["5"] all expanded from short phrases to structured principle-based guidance blocks, following the same template as Rows 1–2. Validated against PMT Row 3 (6 CCIs), Row 4 (1 CCI), Row 5 (9 CCIs). Supersedes v0.19.)

Date: 26 May 2026

**Purpose.** Implementation specification for the Domain Derivation mechanism (Pass 3c). v0.20 completes the ROW_GUIDANCE set — all six rows now have structured principle-based guidance blocks. Rows 3–5 follow the same template proven for Rows 1–2. Supersedes v0.19.
...
| **Row 4 Understanding v0.23** | §13 | Pass 3c implementation framework: module structure (row_abstraction_vocabulary.py absent), re-run scenarios (prose), ProjectProfile parameters (three), mode discipline, VER criteria → pytest mapping, 7-fixture table, Replit Agent handoff. |
...
| **Tracker v0.46** | F43, F53, F58 | F43: Action-Required (pass catalogue mode label). F53: Action-Required (Row 3 Understanding §E.12.1 amendment). F58: v0.12 execution_warnings/no_cci_input fixes (Resolved). |
...
| **Phase 2 — Mechanism Activation** | `project_profile` row — provides three domain-specific parameters (§13.9 of Row 4 Understanding v0.23). | Soft prerequisite — ProjectProfile must exist. If any parameter is NULL, default value is substituted. |
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.20) — implementation authority (all detail including DDL in §5.1)
...
- Row 4 Understanding v0.23 §13 — framework (module structure, prose descriptions, ProjectProfile parameters, VER criteria mapping, fixture table)
...
End of SysEngage Row 4 Mechanism: Domain Derivation v0.20.

Changes from v0.19 (Rows 3–5 prompt guidance — ROW_GUIDANCE complete):
- **§5.4 ROW_GUIDANCE["3"] — Row 3 Designer/Logical:** Expanded from short phrase to structured block. Covers: what a Row 3 domain represents (logical design responsibility, technology-agnostic); six-column logical-level semantics; qualification questions; vocabulary (logical model, state management, constraint enforcement — avoid technology names and Row 2 business language); sparse CCI set guidance; prohibition rules.
- **§5.4 ROW_GUIDANCE["4"] — Row 4 Builder/Physical:** Expanded from short phrase to structured block. Covers: what a Row 4 domain represents (physical construction responsibility — names specific technologies); six-column physical-level semantics; qualification questions; explicit acknowledgement that sparse (single-CCI) rows are legitimate at Row 4; column-specific grouping guidance for WHERE-heavy CCI sets; prohibition rules.
- **§5.4 ROW_GUIDANCE["5"] — Row 5 Implementer/Detailed:** Expanded from short phrase to structured block. Covers: what a Row 5 domain represents (detailed specification responsibility); six-column detailed-level semantics; qualification questions; explicit column-sparse guidance (WHERE + WHO CCI sets map naturally to deployment infrastructure + UI specification domains); prohibition rules.
- All three blocks validated conceptually against PMT CCIs: Row 3 (6 CCIs → expect 2–3 domains), Row 4 (1 CCI → single degenerate domain, CHK-3c-07 exception applies), Row 5 (9 CCIs, WHERE/WHO/WHEN only → expect 2–3 domains by natural column grouping).
- ROW_GUIDANCE is now complete for all six rows. Row 6 retains a short phrase — Row 6 (operational) content is rare in source documents and no PMT CCIs exist at Row 6.

Changes from v0.18: Row 2 prompt guidance.
Changes from v0.17: CHK-3c-07 single-CCI absorption; VER-3c-13.
Changes from v0.16: Row 1 prompt guidance; PLB-3c-06.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md — architectural authority
- SysEngage_Row_4_Understanding_v0_23.md — structural framework
- SysEngage_Issues_Tracker_v0_46.md — finding disposition
