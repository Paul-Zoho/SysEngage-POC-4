...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_2.md

Version: 0.2 (Signal column attribute removal — batch processing model)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). Translates the Row 3 Mechanism Specification (SysEngage_Row_3_Mechanism_CCI_Construction_v0_2.md) into Row 4 implementation detail. v0.2 replaces the per-column Signal grouping approach with a batch processing model — the AI receives all Signals in fixed-size batches alongside all six column interrogatives and assigns column placement as part of CCI derivation. No Signal.column attribute required.
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_2.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.2** | All sections | Architectural spec: six-step pass structure, batch processing model, column vocabulary, merge rule, re-run semantics, verification criteria, edge cases |
| **Row 4 Understanding v0.3** | §12 | Pass 3b implementation framework: batch processing pattern, Signal-to-CCI derivation pattern (all-column prompt), deduplication pattern, re-run-aware commit pattern, module structure |
...
- **Batch processing, not per-column grouping** — Signals are partitioned into fixed-size batches (`ProjectProfile.cci_batch_size`, default 20). Each batch is presented to the AI with all six column interrogatives simultaneously. The AI determines column placement as part of CCI derivation. This approach requires no `column` attribute on Signal entities and is consistent with the canonical ledger spec v2.12 Signal schema.
...
- UPSERT into ZachmanCell table (ON CONFLICT DO NOTHING).
...
**Mode:** IM (Stage 3a) + DM (Stage 3b, 3c). Per-batch iteration. Ledger write deferred to Step 5.
...
**`cci_construction_response_schema.py`** — for Stage 3a. Includes `column` as a required field:
```json
{
  "type": "object",
  "required": ["items"],
...
End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.2.

Changes from v0.1: batch processing model replacing per-column grouping; `column` field added to AI response schema; Signal.column attribute dependency removed; Step 3 reframed as per-batch; Step 4 reframed as per-cell across global candidate buffer; AnalysisPass outputs gain batches_processed and batches_failed fields; test fixtures updated.
