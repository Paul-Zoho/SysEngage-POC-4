...
- Represent one entry in the project Data Dictionary — the single project-wide controlled vocabulary that behavioural requirements bind to (finding F90).

- Hold, across the whole project and all Zachman rows, the canonical entities, their author-facing synonyms, and the relationships between entities — the vocabulary authority for requirement Object-slot binding and for term resolution / matching.

## Non-Purpose (NORMATIVE)

- Hold attribute representation, bounds, types, storage, or keys. These are NOT vocabulary — they are row requirements that *elaborate* a DataDictionaryEntry. The DD holds only what behaviour binds to (entity identity, attribute existence, value-sets where behaviour references the values, relationships).

- Be row-stratified. A DataDictionaryEntry is project-wide and has NO `row_target`; an entity's identity is row-independent even though the per-row detail of its attributes lives in row requirements that reference it.
...
A DataDictionaryEntry carries an `entry_kind` taking exactly one of three values, which determines the meaningful attribute set:

- `canonical` — a project entity (e.g. Task, Inventory, EarningsRecord).
...
- `relationship` — a directed, cardinality-bearing association between two canonical entries (e.g. Inventory 1:* Task).
...
- `entry_kind: string` (REQUIRED; one of `canonical`, `synonym`, `relationship`)
...
- `name: string` (REQUIRED for `canonical`; OPTIONAL/absent otherwise)
  ...
  Notes: SHOULD be distinct within the DD. For `synonym` and `relationship` entries, `name` is not used (the surface term / endpoints carry the meaning).
...
- `attributes: object[]` (OPTIONAL; applies to `canonical`)

  Description: The entity's attributes that behaviour references. Each attribute object carries: `attr_name` (REQUIRED), `attr_description` (OPTIONAL), and `value_set` (OPTIONAL array of named values). A `value_set` is present ONLY where behaviour binds to the individual values (e.g. `status` with values `available`, `claimed`, `completed`); a requirement references a value by the dotted path `<name>.<attr_name>.<value>` (e.g. `Task.status.available`). Attribute *representation* (length, type, storage) is NOT held here — that is a row requirement.

  Notes: A value in a `value_set` is named-addressable but is NOT a separate ledger element. Transition behaviour over values (a state model) is NOT represented in v2.14 (deferred, finding F91).
...
- `from_ref: string` (REQUIRED for `relationship`; references a `canonical` `DataDictionaryEntry.dd_id`)

  Description: The source endpoint of a directed relationship (e.g. Inventory in "Inventory contains Task").

- `to_ref: string` (REQUIRED for `relationship`; references a `canonical` `DataDictionaryEntry.dd_id`)

  Description: The target endpoint of a directed relationship (e.g. Task).

- `cardinality: string` (REQUIRED for `relationship`; OPTIONAL otherwise)

  Description: The relationship cardinality read from `from_ref` to `to_ref` (e.g. "1:*", "1:1", "*:*").
...
- `entry_kind` MUST be one of `canonical`, `synonym`, `relationship`.

- If `entry_kind == canonical`: `name` and `description` MUST be present and non-empty. `surface_term`, `resolves_to`, `from_ref`, `to_ref` MUST be absent.
...
- If `entry_kind == relationship`: `from_ref`, `to_ref`, and `cardinality` MUST be present; `from_ref` and `to_ref` MUST each reference an existing `DataDictionaryEntry` whose `entry_kind == canonical`. The relationship is directed (`from_ref` → `to_ref`).

- A DataDictionaryEntry MUST NOT carry a `row_target` (the DD is project-wide, not row-stratified).

- Within `attributes`, each `attr_name` MUST be unique for the entry; a `value_set`, if present, MUST contain at least one named value with unique names.
...
"entry_kind":{"type":"string","enum":["canonical","synonym","relationship"]},
...
"value_set":{"type":"array","minItems":1,"uniqueItems":true,"items":{"type":"string","minLength":1}}
...
"from_ref":{"type":"string","pattern":"^DD\\d{3}$"},

"to_ref":{"type":"string","pattern":"^DD\\d{3}$"},
...
{"if":{"properties":{"entry_kind":{"const":"relationship"}},"required":["entry_kind"]},

"then":{"required":["from_ref","to_ref","cardinality"]}}
