...
**Architectural mechanism specification — v0.1**

Filename: SysEngage_Row_3_Mechanism_CCI_Construction_v0_1.md

Version: 0.1 (Initial — Pass 3b architectural specification)

Date: 16 May 2026

**Purpose.** Specifies the designed mechanism by which SysEngage constructs CellContentItem (CCI) entities from the Signal set produced by Pass 3a (Row-Lens Source Re-Analysis). For each active ZachmanCell, Signals are taken as evidence and transformed into atomic, classified content items that serve as the analytical bridge between raw classification evidence (Signals) and formal outputs (Domains, Requirements). Operates under DM (Derivation Mode) as the primary mode, with IM (Interpretive Mode) for the AI-assisted derivation and deduplication acts. Runs once per active row (Row-sequential), after Pass 3a completes.

**Status.** v0.1 — initial specification. All design decisions locked during Phase 3b step-by-step design (16 May 2026). One finding raised (F40 — §8.12 boundary analysis deferred to Phase 5 Robustness; F41 — Safety/CyberSec evaluation deferred to designated phases). No open questions remain.
...
| **Mechanism scope** | Upsert all six ZachmanCells for the current row. Group the eligible Signal set by Zachman column. For each populated cell, derive candidate CCIs from its Signals via AI-assisted interpretive act. Deduplicate candidates within the cell using structural pre-filter followed by AI semantic review. Commit surviving CCIs to the ledger with identifiers allocated per cell sequence. On re-run, extend existing CCIs rather than replacing them. Produce an AnalysisPass execution record. |
...
### Step 2 — Upsert ZachmanCells and Group Signals by Cell
...
**Step 2b — Signal grouping.** Partition the eligible Signal working set by column — each Signal's column classification from Pass 3a determines which ZachmanCell group it belongs to. The result is six groups, some populated, some empty.

Empty groups are valid — they represent cells with no Signal evidence. They produce no CCIs. Phase 5 Cell Quality Analysis will surface empty cells as coverage findings. Pass 3b does not flag or treat empty cells as errors.

**Output:** Six ZachmanCell groups (populated or empty). ZachmanCells committed to ledger.

---

### Step 3 — Per-Cell CCI Construction (IM + DM)

For each populated ZachmanCell group, execute the three-stage construction sequence. Empty groups are skipped.

**Stage 3a — AI derivation act (IM).**

Present the cell's Signal set to the AI with:
- The current row's abstraction lens
- The column interrogative (What / How / Where / Who / When / Why) and its analytical framing
- The column's permitted `classification_type` vocabulary (see §5.1)
- Each Signal's `signal_id`, `description`, `signal_type`, and `confidence`

The AI produces, for each distinct classified content item it identifies in the Signal set:
- `classification_type` — from the column's permitted vocabulary
- `description` — a derived statement of classified meaning (not a copy of Signal text)
- `signal_refs` — the Signal id(s) from which this item was derived (at least one; may be multiple)
- `confidence` — derivation confidence, 0.0–1.0
- `trigger_condition` (optional) — where content is conditional
- `justification` (optional) — reasoning for classification

A single Signal may produce more than one candidate CCI where it contains multiple distinct classified items. Multiple Signals may ground a single CCI where they converge on the same classified content.
...
- `signal_refs` is non-empty
- `classification_type` is within the column's permitted vocabulary (§5.1)
- `confidence` is within 0.0–1.0
- `description` is non-empty

Reject malformed AI outputs — record in AnalysisPass failure detail. Do not silently discard.

**Stage 3c — Pydantic validation at AI response boundary.**

Parse the full AI response against the `cci_construction_response_schema`. Items that fail schema validation are recorded in AnalysisPass failure detail; valid items proceed to Step 4.
...
### 9.1 All Signals for a cell are in the same column but produce no valid CCIs

If Stage 3c rejects all candidates for a cell (all fail Pydantic validation), the cell remains empty. The AnalysisPass records the full rejection count in `candidates_rejected`. The cell's ZachmanCell entity exists (committed in Step 2). Phase 5 will surface as a coverage finding. Not a Pass 3b failure unless all cells produce this outcome.

### 9.2 No eligible Signals (Signal set is empty)

If the Step 1 query returns zero Signals (e.g. all Pass 3a content was blocked by open Concerns), Step 3 is skipped for all cells. All six ZachmanCells are still upserted in Step 2. Zero CCIs are produced. AnalysisPass records `ccis_created = 0`. execution_status = Completed (not Failed — zero CCIs is a valid outcome; Phase 5 and Phase 9 surface the coverage implications).
...
- If all retries fail for a cell: that cell's construction is abandoned for this run. The cell's ZachmanCell exists (from Step 2); it will produce no CCIs from this run. Failure recorded in AnalysisPass failure detail.
- execution_status = CompletedWithWarnings if some cells succeeded; Failed if all cells failed.
...
None. All design decisions locked during Phase 3b step-by-step design session (16 May 2026).
...
End of SysEngage Row 3 Mechanism: CellContentItem Construction v0.1.

Companion artefacts produced with this spec:
- SysEngage_Row_4_Understanding_v0_3.md — Row 4 Understanding additions for Pass 3b
- SysEngage_Row_4_Mechanism_CCI_Construction_v0_1.md — Row 4 implementation specification
- SysEngage_Issues_Tracker_v0_27.md — tracker current state (F40 recorded)

Next artefacts in sequence:
- Replit Agent implementation of Pass 3b per Row 4 Mechanism spec
- Verification run and Practitioner review
- Pass 3c design (Domain Derivation) pending Pass 3b verification
