...
**Filename:** SysEngage_Row_4_Understanding_v0_18.md (§13 addendum)

**Version:** 0.18 — §13.8 DDL note updated (domain_cci_membership removed; cell_content_item_refs JSONB on domain table); §13.2 stage4 comment updated (DomainCCIMembershipModel removed). Supersedes v0.17.

**Date:** 26 May 2026

**Purpose.** §13 carries structural guidance only. DDL lives in Mechanism Spec v0.14 §5.1. v0.18 replaces v0.17 in full; all §1–§12 content from v0.5 remains authoritative.

**Scope note.** This document records §13 only.

**Precedence rule.** Where this artefact appears to differ from canonical ledger spec v2.12 or Row 4 Mechanism Spec (Domain Derivation) v0.14, the Mechanism Spec takes precedence.

---

## §13 Pass 3c — Domain Derivation Implementation Framework

This section establishes the Row 4 implementation framework for Pass 3c. The architectural authority is **SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md**. The implementation specification is **SysEngage_Row_4_Mechanism_Domain_Derivation_v0_14.md**. This section provides structural framework guidance — module structure, parameter tables, VER criteria mapping, fixture table, and Replit Agent handoff notes. Implementation detail (stage-by-stage behaviour, DDL, Pydantic schemas) lives in the Mechanism Spec §5.1 and is not duplicated here.
...
The `mechanism_data` sub-structure in `AnalysisPass.outputs` is defined in Mechanism Spec v0.14 §7. The implementation must populate ALL fields; zero-value arrays MUST be `[]` not null. The AnalysisPass is written outside the main ledger transaction (same discipline as Pass 3b Step 6).
...
- `migrations/XXXX_add_domain_tables.py` — creates `domain` table (with `cell_content_item_refs JSONB`) and seeds DomainRegister row (use DDL from Mechanism Spec v0.14 §5.1)
...
**Changes from v0.15:**
- §13.7: execution_warnings note qualified — `incremental_fallback_to_fullrerun` is the one exception that triggers CompletedWithWarnings; `no_cci_input` is governed by §4.1 directly; Status impact column added to advisory type table
**Changes from v0.16:**
- §13.2: ADVC-3c-01 added to stage3 module comment
- §13.7: execution_warnings table collapsed to single-sentence pointer to Mechanism Spec v0.14 §7 — eliminates the parallel table that required updating on every §7 change
- All Mechanism Spec references updated v0.12 → v0.13; tracker → v0.39

§13 content from v0.16 is superseded by v0.17. All §1–§12 content from v0.5 unchanged.

**This is the final handoff version.** §13 now carries no content that duplicates the Mechanism Spec — future Mechanism Spec changes do not require Understanding updates unless module structure, parameter table, VER mapping, or fixture table changes.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md — architectural authority for Pass 3c
- SysEngage_Row_4_Mechanism_Domain_Derivation_v0_14.md — implementation spec (final handoff version)
- SysEngage_Issues_Tracker_v0_40.md — finding disposition
