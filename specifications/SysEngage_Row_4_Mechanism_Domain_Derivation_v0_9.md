...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_9.md

Version: 0.9 (seventh Agent review — §4.4.4 retirement_mapping Jaccard threshold specified (0.50 hardcoded); §4.3 repair assign-to-new fallback field values specified; Understanding references updated to v0.13; tracker reference updated to v0.35)

Date: 26 May 2026

**Purpose.** Implementation specification for the Domain Derivation mechanism (Pass 3c). v0.9 closes two significant and two noted findings from the seventh Agent review. Supersedes v0.8.
...
| **Row 4 Understanding v0.13** | §13 | Pass 3c implementation framework: module structure (row_abstraction_vocabulary.py absent), re-run scenarios (prose), ProjectProfile parameters (three), mode discipline, VER criteria → pytest mapping, 7-fixture table, Replit Agent handoff. |
...
| **Tracker v0.35** | F43, F53, F55 | F43: Action-Required (pass catalogue mode label). F53: Action-Required (Row 3 Understanding §E.12.1 amendment). F55: seventh-review corrections (Resolved). |
...
- Merge repair response into proposals using action-based logic:
  - For `action="assign"` entries: match `domain_name` case-insensitively against existing proposals; add `new_cci_refs` to the matched proposal's `cci_refs`. If no match: treat as `action="new"` — use `domain_name` as the Domain name, `description = "Domain created from repair assignment — review recommended"`, `classification_type = null`. Advisory logged: `{type: "repair_assign_name_not_found", domain_name: name}`.
...
| **Phase 2 — Mechanism Activation** | `project_profile` row — provides three domain-specific parameters (§13.9 of Row 4 Understanding v0.13). | Soft prerequisite — ProjectProfile must exist. If any parameter is NULL, default value is substituted. |
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.9) — implementation authority (all detail including DDL in §5.1)
...
- Row 4 Understanding v0.13 §13 — framework (module structure, prose descriptions, ProjectProfile parameters, VER criteria mapping, fixture table)
...
End of SysEngage Row 4 Mechanism: Domain Derivation v0.9.

Changes from v0.8 (seventh Agent review — 2 significant, 2 noted):
- **Finding 1 (§4.4.4 — Significant):** Retirement_mapping Jaccard threshold now specified as 0.50 (hardcoded; not a ProjectProfile parameter). Algorithm stated explicitly: tokenise both names, compute `|intersection| / |union|`, assign if ≥ 0.50 else null.
- **Finding 2 (§4.3 — Significant):** Repair assign-to-new fallback field values specified: `description = "Domain created from repair assignment — review recommended"`, `classification_type = null`. Previously left as an undocumented implementer decision.
- **Finding 3 (§2, §11.1, §12.3 — cross-refs):** Understanding v0.12 → v0.13; tracker v0.34 → v0.35; self-reference v0.8 → v0.9.

Changes from v0.7: ledger v2.12 compliance (domain_qualifier, upstream_domain_ref withdrawn; Skipped → Completed; domain_data → mechanism_data).

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md — architectural authority
- SysEngage_Row_4_Understanding_v0_13.md — implementation framework
- SysEngage_Issues_Tracker_v0_35.md — finding disposition

Next artefact in sequence:
- SysEngage_Row_4_Mechanism_Domain_Derivation_v0_10.md — after Replit Agent build and first PMT/NQPS production run
