...
description: CompletedWithWarnings is only triggered by three specific conditions per spec v0.18 §4.4.6; all other advisory warnings are informational only.
---

# Domain Derivation `execution_status` decision rule

Per spec v0.18 §4.4.6, `CompletedWithWarnings` fires **only** when one or more of:
