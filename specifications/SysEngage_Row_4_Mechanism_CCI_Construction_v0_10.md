...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_10.md

Version: 0.10 (Named-instance detection moved to Stage 4a; is_named_instance removed)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). v0.10 removes `is_named_instance` from Stage 3a response schema and candidate struct, removes the Stage 4b named-instance bypass, and replaces both with Stage 4a routing context passed to Stage 4b for groups with dissimilar descriptions despite matching type and signal_refs.
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_9.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.9** | All sections | Architectural spec: six-step pass structure, batch processing model, composite PK, clustering deduplication with Stage 4a three-condition rule and routing context, column vocabulary, merge rule, re-run semantics, verification criteria, edge cases |
...
- If yes: use the named-instance framing variant in `dedup_cluster_review_prompt.py` — the AI is told these items share the same Signal and classification type but have dissimilar descriptions, and asked to determine whether each represents a genuinely distinct named entity (preserve all) or a reformulation of the same concept (merge). Record `{warning_type: "stage4a_named_instance_routed", detail: {cell_id, classification_type, member_count}}` in `execution_warnings`.
- If no: standard cluster review prompt — identify semantic duplicates across different Signals.
...
End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.10.

Changes from v0.9: `is_named_instance` removed from Stage 3a response schema (§4.3 and §5.3) and Stage 3b candidate struct; Stage 4b named-instance bypass removed; Stage 4b now receives Stage 4a routing context (`stage4a_routed` marker) with named-instance framing for groups with dissimilar descriptions; `stage4a_named_instance_routed` replaces `stage4b_named_instance_bypass` in execution_warnings; Row 3 spec reference updated to v0.9; `is_named_instance` database column and Pydantic field may be removed via migration now that the field is no longer used.

Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_9.md
