"""
run_pmt_dd_ph3c_r5_branch.py — Self-contained PMT Pass 3c Rows 1–5 test run.

Creates a fresh disposable test branch from snap_ph03_3b_AllProjects, applies
schema migrations, runs Domain Derivation for Rows 1–5 in sequence, and exports
the ledger.  The test branch is left alive after the run for analysis; delete
it when done:

  python sysengage/scripts/branch_manager.py delete_test_branch \
    --branch test_ALL_ph03_3b_R5_Ph3c_DD_R1R5

This runner validates the v0.20 ROW_GUIDANCE["3"], ["4"], ["5"] structured
guidance blocks against PMT_E2E's Row 3 (6 CCIs), Row 4 (1 CCI), Row 5
(9 CCIs).
...
# ── Step 5: Run Domain Derivation — Rows 1–5 ──────────────────────────────────
all_ok = True
...
print(flush=True)
print("[orchestrator] Rows 1–5 completed successfully.", flush=True)
