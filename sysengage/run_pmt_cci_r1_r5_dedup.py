"""
run_pmt_cci_r1_r5_dedup.py — Phase 3b CCI Construction for PMT_E2E, Rows 1–5, dedup enabled.

Runs each row sequentially with skip_deduplication=False (full Stage 4a+4b+4c
deduplication sweep including the v0.8 Jaccard similarity guard).
Idempotent: skips any row that already has a Completed / CompletedWithWarnings pass.

Intended to be run against a test branch cloned from snap_PMT_ph03_3a_R1 so
results can be compared against the dedup-disabled run (run_pmt_cci_r1_r5.py).

Invocation (from workspace root):
    python -u sysengage/run_pmt_cci_r1_r5_dedup.py
"""
...
print(f"[runner] Phase 3b CCI Construction — {PROJECT_ID}  Rows {ROWS}  skip_dedup=False", flush=True)
print(flush=True)
...
for row in ROWS:
    ...
    try:
        ...
        print(f"[runner]   candidates_rejected={cd['candidates_rejected']}", flush=True)
...
for row_ref, r in sorted(results.items()):
    if r.get("skipped"):
        ...
        n_created = r["cci_data"]["ccis_created"]
        n_merged = r["cci_data"]["ccis_merged"]
        total_created += n_created
        total_merged += n_merged
        print(
            f"[runner]   R{row_ref}: {n_created} created  {n_merged} merged"
            f"  ({r['pass_id']}  {r['execution_status']})",
            flush=True,
        )
print(f"[runner]   Total CCIs created: {total_created}  merged: {total_merged}", flush=True)

print(flush=True)
print("[runner] Exporting full ledger (post all-row run)...", flush=True)
