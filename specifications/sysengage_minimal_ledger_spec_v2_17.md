...
- Data Dictionary scope corrected — reference vs specification (finding F107). The v2.14 `DataDictionaryEntry` over-reached: a *reference* artefact (a naming / disambiguation index) was holding *specification* content — typed `attributes`, `value_set`s used as attribute domains, and directed `relationship` entries. Those are claims about what must be true (which entities exist, what values are legal, how entities relate), i.e. model, not vocabulary. A behavioural-binding trace confirmed the over-reach concretely: the DD's `lifecycle_state` value-set was *unreferenced* (no requirement carried a stored binding to it) and *inconsistent* with the authoritative Structural CHECK domain — an unreferenced, inconsistent shadow of the real specification. **Correction:** the `DataDictionaryEntry` `canonical` kind loses its `attributes` array (and the `value_set`s within); the `relationship` `entry_kind` is **retired**. The DD reverts to a pure reference — canonical entity **names**, `synonym` entries, and `provenance_ref`. The entity/attribute/value-set/relationship model relocates to the new `class_model` payload (below), which the DD now *references* rather than *contains*: a canonical entity's definition is exactly the set of Structural Requirements whose `class_model.entity_ref` points at it (found by query, mirroring the existing synonym pattern). This partially supersedes the v2.14 DD-scope decision (finding F90); F90's naming + synonym-layer portion stands.

- New OPTIONAL `class_model` payload on `Requirement` (applies to `requirement_type == Structural`) — finding F105. The authoritative, structured data-model representation of one entity at one Zachman row: `entity_ref` (binds the model to the DD canonical name), `entity`, `tier` (conceptual|logical|physical|detailed, mirroring `row_target` 2–5), `attributes[]` (each with `attr_name`, optional `attr_ref`, `type`, `domain`, `key`, `null`, `unique`, `default`, `aggregatable`, `derived`, `composite`, `origin`), `keys[]`, `relationships[]`, and `refinement_kind`. `class_model` is **per-row** (one per entity per row, chained by `refines_refs`); the payload **schema is uniform** across rows while a per-row population profile governs which fields a row populates (a mechanism concern — Requirement Derivation F105 spec). Where present, the prose `statement` is a generated **projection** of the payload (so existing `statement` consumers keep working and the two cannot drift). The prose-atomicity machinery (conjoined-predicate / compound-object decomposition) does NOT apply to a `class_model`-bearing Structural; structural-validity rules replace it. Scope: Rows 2–5 (R1 contributes entity names to the DD; R6 is out). `refinement_kind` (finding OQ-105-01) types the cross-row transformation — `identity | decompose | realise_relationship | introduce | merge` — so refinement validation enforces **concept-level parent→child coverage** (every parent model-element maps to ≥1 child element; physical introductions such as surrogate keys are legitimately parentless, the `class_model` analogue of Path-N row-native content) rather than 1:1 entity identity.

- New OPTIONAL `object_refs` on `Requirement` (applies to behavioural requirements — `Functional` / `Constraint`) — finding F107. The materialised binding from a behavioural requirement to the data-model element(s) it references, addressed by dotted path through the DD canonical name to the owning `class_model` (e.g. `Task.status.available` → DD canonical `Task` → the `class_model` defining `status` → the value in its domain). This replaces the former prose-only Object-slot binding; the value authority is the `class_model` attribute domain, the DD provides only name/synonym resolution.

- Path A discipline: this amendment lands WITH the Data Dictionary mechanism spec and the Object-slot binding revisions (Requirement Derivation Row 3/4 §5.5; finding F107-T1) and gates the F105 mechanism build. Migration: Appendix D v2.16 → v2.17.

Updates (v2.16):

- Requirement provenance relaxed for refinement-driven derivation (Requirement Derivation Row 3 v0.12 / Row 4 v0.13; finding F87 corrected). `cci_refs` is no longer unconditionally REQUIRED. Under refinement-driven derivation a Requirement at row n>1 may be produced by **elaborating a row n−1 parent** rather than from a row-n CellContentItem — it then traces to its parent via `refines_refs` and may have empty `cci_refs`. Conversely a row-native Requirement traces via `cci_refs` and may have empty `refines_refs`. **Which provenance is populated is a property of the input's abstraction profile** (a policy/strategy source is top-heavy — its lower-row Requirements are nearly all parent-elaborated, `refines_refs` only; a technical specification is bottom-heavy — `cci_refs` only), so neither attribute can be individually required. The invariant becomes: **a Requirement MUST trace to something — at least one of `cci_refs` / `refines_refs` non-empty; both empty is the only invalid (free-floating) state.** Ownership note: `refines_refs` is set by Pass 3d derivation for a parent-elaborated Requirement (link known by construction) and by the Requirement Matching service for a row-native Requirement (parent discovered, or left empty as an upward gap, F86). No other Requirement rule changes; `cci_refs` row-equality and `refines_refs` parent-row rules are unchanged.



- Defect closed (CellContentItemRegister non-canonical): `CellContentItemRegister` added to the Permitted Element Types list and the Identifier Prefix Register (prefix `CELLCONTENTITEM_REG`). Every other first-class element type with a completeness-register counterpart was already listed; the omission of `CellContentItemRegister` was a spec/implementation divergence — the exporter correctly emitted the register but the spec did not permit it. No attributes or normative rules change; the register follows the standard Register element schema (§ Element Type — Register). Register identifier convention: `CELLCONTENTITEM_REG001` (or higher sequence numbers for additional registers).

- Rationale for v2.15 amendment: the ledger exporter has always produced a `CellContentItemRegister` listing all CCI identifiers for completeness validation. CellContentItem is a first-class element whose membership completeness is enforced by a register for every other comparable element type (Source→SourceRegister, Domain→DomainRegister, Requirement→RequirementRegister, Signal→SignalRegister, etc.). Omitting `CellContentItemRegister` from the permitted types was a bookkeeping defect, not an intentional architectural exclusion. Path A discipline (validate-then-canonicalise) requires the spec to catch up to the correct implementation before further mechanism work proceeds.

Updates (v2.14):

- F90 / F85 / F93 resolved (Data Dictionary element types added): the project Data Dictionary — the single project-wide controlled-vocabulary artefact (finding F90) — is canonicalised. Two new element types: (1) `DataDictionaryEntry` (DD### identifier prefix) with an `entry_kind` discriminator taking one of three values — `canonical` (an entity: name, description, attributes, where an attribute MAY carry a named-addressable value_set), `synonym` (an author surface term resolving to a canonical entry — the cross-reference register), `relationship` (a directed, cardinality-bearing association between two canonical entries); (2) `DataDictionaryRegister` (completeness collection, ledger-sense). The canonical Requirement element is unchanged in v2.14 — Object-slot binding to DD entries is a mechanism-level concern (Row 3/4 Requirement Derivation §5.5), not a ledger schema change.

- Rationale for v2.14 amendment: F90 established the Data Dictionary as the controlled vocabulary that behavioural requirements bind to (entity identity + aliases, attribute existence, value-sets where behaviour references the values, relationships — IN; representation/bounds/types/storage — OUT, those remain row requirements). F90/F85 design items were closed before this amendment (per validate-then-canonicalise discipline): aliases are stored ONLY as `synonym` entries (single source of truth — a canonical entry carries no alias array; its aliases are the set of synonym entries whose `resolves_to` points at it, found by query); value-sets are a field on the attribute with named-addressable values (a requirement references `Task.status.available` as a resolvable dotted path) and are NOT separate elements (the state-model machinery over those values is deferred, F91); relationships are their own directed entries (from→to + cardinality); provenance is a stored `provenance_ref` pointer into existing ledger traceability (so the synonym register is auditable at a glance without a graph traversal). The Data Dictionary is a STANDALONE CROSS-ROW SERVICE (F93) populated incrementally as rows produce requirements — not a numbered pass; its build/resolution/gating mechanism is specified in the Data Dictionary Mechanism Spec (Row 3 logical / Row 4 physical), authored alongside this amendment per Path A discipline (canonical type lands WITH its mechanism spec, as Concern did in v2.12).

Updates (v2.13):

- F82 / F89 resolved (Requirement element amended): two changes to the `Requirement` payload. (1) New OPTIONAL attribute `refines_refs` (array of `Requirement.requirement_id`) carrying the cross-row UML «refine» relationship — child Requirement at row n points up to the parent Requirement(s) at row n−1 it refines; many-to-many; empty/absent permitted at every row (an empty `refines_refs` at row n>1 is the novel-requirement gap signal consumed by the gap→question→answer mechanism, per finding F86, not an error). (2) `requirement_type` enum COLLAPSED from five values (Functional|Constraint|Performance|Suitability|Non-Functional) to three (Functional|Constraint|Structural) per finding F89 — former Performance/Suitability fold into Constraint with the measured-vs-inspected distinction carried by `verification_method` (which gains the value `Measurement`); former Non-Functional renamed Structural (the Behaviour-Structure-binding structural form). The Performance-specific `fit_criteria` SHOULD-rule is replaced by a `verification_method == Measurement` SHOULD-rule, preserving the measurable-criteria discipline under the collapsed taxonomy.

- Rationale for v2.13 amendment: the cross-row/structural design phase (findings F82–F93, June 2026) established that Phase 4 Requirement Quality Analysis scores higher-level requirements as satisfied by *linked* lower-level requirements — requiring a persisted Requirement-to-Requirement «refine» link that the v2.12 payload did not provide (its absence was the schema fingerprint of the former req→source→cci→req plan). The `refines_refs` link is also where the cross-row requirement traceability formerly carried diffusely by stream-2 Signal provenance relocates (findings F83 / Domain Derivation MD-6); the dual-stream Pass-3a model that carried that provenance is rejected (F83), and the Row-Lens Source Re-Analysis specs revert to single-stream — that revert is sequenced AFTER this ledger amendment so the traceability is never lost in the gap. The type collapse (F89) follows the principle that a requirement type earns its place only by doing real work (selecting the structural slot pattern per F88, and routing the verification method); the five-value set carried distinctions now better carried by `verification_method`.

- Data Dictionary NOT added in v2.13: the project Data Dictionary (finding F90 — single project-wide controlled-vocabulary artefact) is conceptually settled but its attribute-level schema has open design items (alias storage, value-set/enumeration representation, relationship representation, provenance). Per the spec's Path A discipline (canonical amendment lands *with* the mechanism specification, as for Concern in v2.12), the `DataDictionaryEntry` / `DataDictionaryRegister` element types will be added when the Data Dictionary mechanism spec is authored and those items are closed — not pre-emptively reserved here.
...
| CELLCONTENTITEM_REG | CellContentItemRegister | `^CELLCONTENTITEM_REG\\d{3}$` |
| D | Domain | `^D\\d{3}$` |
| R | Requirement | `^R\\d{3}$` |
...
- `DataDictionaryEntry` has no row context. It is a project-wide controlled-vocabulary artefact (finding F90) referenced by row-scoped elements (requirement Object slots) across all rows; an entity's identity is row-independent, while the per-row detail of its attributes lives in the row requirements that reference it.
...
- `requirement_type: string` (REQUIRED; Functional|Constraint|Structural)

  Description: Classification of the requirement into one of three fundamental kinds. **Functional** — a behaviour the system performs (Condition-Subject-Predicate-Object form; verified by Test/Demonstration). **Constraint** — a bound on behaviour, solution, quality dimension, or operating context (absorbs the former Performance, Suitability, and Environmental kinds; verified by Inspection for conformance bounds or Measurement for threshold bounds — the distinction is carried by `verification_method`). **Structural** — a composition / attribute / relationship assertion about what an entity *is* (the structural requirement form; verified by schema/composition conformance).

  Notes: Collapsed from the former five-value set (Functional|Constraint|Performance|Suitability|Non-Functional) in v2.13 per finding F89. Former Performance/Suitability → Constraint (the measured-vs-inspected distinction moves to `verification_method`, which is why that field exists). Former Non-Functional → Structural (renamed to de-overload the term and to name the Behaviour-Structure-binding structural form). The type SELECTS the requirement's structural slot pattern per finding F88.
...
- `cci_refs: string[]` (CONDITIONAL; references `CellContentItem.ci_id`)
...
- `refines_refs: string[]` (CONDITIONAL; references `Requirement.requirement_id`)

  Description: The Requirement(s) at the adjacent higher abstraction level (row_target − 1) that this Requirement refines. Carries the cross-row UML «refine» relationship (client = this more-refined Requirement; supplier = the more-abstract Requirement it points to). Many-to-many: a Requirement MAY refine more than one parent (cross-cutting), and a parent MAY be refined by many children (decomposition). Set by Pass 3d derivation for a Requirement elaborated from a row n−1 parent (the link is known at derivation); set by the Requirement Matching service for a row-native Requirement (parent discovered by cross-row match, or left empty as the F86 upward-gap signal). Empty/absent permitted (an empty `refines_refs` at row n>1 with non-empty `cci_refs` is row-native content, possibly an upward gap; both `cci_refs` and `refines_refs` empty is invalid — see rules).

  Notes: Added in v2.13 per finding F82. **Empty/absent is permitted at every row** — a Row 1 Requirement refines nothing (it is top); a Row n>1 Requirement with no parent is a legitimately novel requirement that emerged from this row's own sources, and its emptiness is the gap signal consumed by the gap→question→answer mechanism (finding F86), NOT an error. Each referenced Requirement, when present, MUST have `row_target` exactly one less than this Requirement's `row_target`. This link replaces the diffuse cross-row traceability formerly carried by stream-2 Signal provenance (finding F83 / Domain Derivation MD-6).

- `class_model: object` (CONDITIONAL; applies to `requirement_type == Structural`)

  Description: The authoritative, structured data-model representation of one entity at this Requirement's row — the What/Data-column primitive model (finding F105). When present, the prose `statement` is a generated projection of this payload. Sub-attributes: `entity_ref` (REQUIRED; references a `canonical` `DataDictionaryEntry.dd_id` — binds the model to the DD name; identity lives in the DD, the model lives here); `entity` (REQUIRED; denormalised canonical name); `tier` (REQUIRED; one of `conceptual`, `logical`, `physical`, `detailed`, mirroring `row_target` 2–5); `refinement_kind` (one of `identity`, `decompose`, `realise_relationship`, `introduce`, `merge` — types how this entity relates to its `refines_refs` parent(s), finding OQ-105-01); `attributes` (array; each: `attr_name` REQUIRED, `attr_ref` OPTIONAL dotted reference to the DD/parent attribute, `type`, `domain`, `key` ∈ {PK,FK}, `null`, `unique`, `default`, `aggregatable`, `derived` {`rule`,`inputs`}, `composite`, `origin` ∈ {`refines`,`realises`,`introduced`}); `keys` (array of {`kind`,`attrs`,`target_ref`}); `relationships` (array of {`rel_ref`,`kind`,`target`,`via`,`cardinality`}).

  Notes: Per-row (one per entity per row, chained by `refines_refs`); the payload schema is uniform across rows while a per-row population profile (a Requirement Derivation F105 mechanism concern) governs which fields a row populates. The conjoined-predicate / compound-object decomposition machinery does NOT apply to a `class_model`-bearing Structural; structural-validity rules replace it (entity_ref resolves; ≤1 PK; FK `target_ref` resolves; attribute domains well-formed). A canonical entity's definition is the set of Structural Requirements whose `class_model.entity_ref` references it (by query). Scope is Rows 2–5 (finding D1).

- `object_refs: string[]` (CONDITIONAL; applies to behavioural requirements — `Functional` / `Constraint`)

  Description: The data-model element(s) this behavioural requirement references, each a dotted path resolved through the DD canonical name to the owning `class_model` (finding F107). Forms: `<Entity>` (an entity), `<Entity>.<attr>` (an attribute), `<Entity>.<attr>.<value>` (a named value within an attribute domain, e.g. `Task.status.available`). The value authority is the `class_model` attribute domain; the DD supplies only name / synonym resolution. Materialises the former prose-only Object-slot binding.

  Notes: Each entry MUST resolve — its leading `<Entity>` segment to a `canonical` `DataDictionaryEntry` (directly or via a `synonym`), and the remaining segments to an `attr_name` / domain value in a `class_model` whose `entity_ref` is that canonical entry. A `<Entity>.<attr>.<value>` reference whose value is absent from every defining `class_model` domain is a dangling binding (a Quality-pass finding, finding F107-T1), not a ledger-schema error.
...
- `verification_method: string` (OPTIONAL; Test|Analysis|Inspection|Demonstration|Measurement)

  Description: How satisfaction is verified. Carries the distinction that the collapsed `requirement_type` no longer encodes (finding F89): a Constraint verified by conformance uses **Inspection**; a Constraint verified against a numeric threshold uses **Measurement**. Functional requirements typically use **Test** or **Demonstration**; Structural requirements typically use **Inspection** (schema/composition conformance).

  Notes: `Measurement` added in v2.13. Include only when a natural method exists; omit otherwise (an abstract enterprise constraint may have no natural verification method — omission is correct, not a defect).
...
- `cci_refs` is CONDITIONAL: it MAY be empty for a Requirement (row > 1) produced by elaborating a row n−1 parent (such a Requirement traces via `refines_refs`); it MUST be non-empty for a row-native Requirement and for every Row 1 Requirement.

- **Provenance invariant (not-both-empty):** at least one of `cci_refs` / `refines_refs` MUST be non-empty. A Requirement with BOTH empty is invalid (it traces to nothing). Which one is populated depends on the input's abstraction profile and is not otherwise constrained.

- If `fit_criteria` is present, it MUST NOT be empty.

- `requirement_type` MUST be one of the defined enumeration values (Functional, Constraint, Structural).

- If `verification_method == Measurement`, `fit_criteria` SHOULD be present (a measured Constraint requires a measurable acceptance threshold — this preserves, under the collapsed taxonomy, the discipline formerly attached to the Performance type).

- `domain_refs` MUST contain at least one entry referencing an existing `Domain.domain_id`.

- If `refines_refs` is present, every entry MUST reference an existing `Requirement.requirement_id` whose `row_target` is exactly one less than this Requirement's `row_target`. `refines_refs` MAY be empty or absent at any row (see attribute Notes; finding F86).

- `class_model` MAY be present ONLY when `requirement_type == Structural`; it MUST be absent on `Functional` and `Constraint` Requirements. When present: `entity_ref` MUST reference an existing `DataDictionaryEntry` whose `entry_kind == canonical`; `tier` MUST be one of `conceptual`, `logical`, `physical`, `detailed`; `entity`, `entity_ref`, and `tier` MUST be present. At most one attribute MAY carry `key == PK`. Every `relationships[].target` and every `keys[].target_ref` MUST resolve to a known entity. A data-model Structural Requirement at row_target 2–5 SHOULD carry a `class_model` (prose-only Structural is a migration residue, finding F105).

- `object_refs` MAY be present ONLY on `Functional` / `Constraint` Requirements; it MUST be absent on `Structural` Requirements (a Structural Requirement *is* the model, it does not bind to one). Each entry's leading `<Entity>` segment MUST resolve to a `canonical` `DataDictionaryEntry` (directly or via a `synonym`'s `resolves_to`). Referential resolution of the trailing `.<attr>` / `.<value>` segments into a `class_model` domain is a Quality-pass obligation, not a ledger-schema constraint (see attribute Notes).
...
# Element Type — DataDictionaryEntry [NEW v2.14]

## Purpose (NORMATIVE)

- Represent one entry in the project Data Dictionary — the single project-wide controlled vocabulary: a **naming / disambiguation index** that resolves author surface terms to canonical entity names (finding F90; scope corrected v2.17, finding F107).

- Hold, across the whole project and all Zachman rows, the canonical entity **names** and their author-facing **synonyms** — the vocabulary authority for term resolution / matching and for the leading `<Entity>` segment of a behavioural Requirement's `object_refs`.

## Non-Purpose (NORMATIVE)

- Hold **specification content**: the DD MUST NOT carry attributes, attribute value-sets / domains, types, bounds, storage, keys, or relationships. These are claims about what the data model *is* — they are the **`class_model` payload of Structural Requirements** (the authoritative model), which the DD *references*, not contains (finding F107, reference vs specification). The DD answers "what is this thing called and what does the name resolve to"; `class_model` answers "what is the thing." A canonical entity's definition is the set of Structural Requirements whose `class_model.entity_ref` references it (found by query, mirroring the synonym pattern).

- Be row-stratified. A DataDictionaryEntry is project-wide and has NO `row_target`; an entity's *name* is row-independent even though its per-row *model* lives in row Structural Requirements (`class_model`) that reference it.

- Duplicate cross-row traceability. A `provenance_ref` is a single pointer into existing ledger traceability, not a copy of it.

## Discriminator (NORMATIVE)

A DataDictionaryEntry carries an `entry_kind` taking exactly one of two values (the `relationship` kind was retired in v2.17 — relationships are `class_model.relationships`), which determines the meaningful attribute set:

- `canonical` — a project entity **name** (e.g. Task, Inventory, EarningsRecord). Its model lives in the Structural Requirements whose `class_model.entity_ref` references it.
- `synonym` — an author surface term that resolves to a canonical entry (the cross-reference register; e.g. "work opportunity" → Task). Preserves authored terminology permanently (vocabulary Non-Loss) and is the audit surface for canonicalisation decisions.

## Attributes (with descriptions)

- `dd_id: string` (REQUIRED, format `DD###`)

  Description: Stable identifier for this Data Dictionary entry.

- `entry_kind: string` (REQUIRED; one of `canonical`, `synonym`)

  Description: The kind of entry; determines which of the following attributes apply.

- `name: string` (REQUIRED for `canonical`; OPTIONAL/absent otherwise)

  Description: The canonical entity name (e.g. "Task"). The controlled term that requirement Object slots bind to.

  Notes: SHOULD be distinct within the DD. For `synonym` entries, `name` is not used (the surface term carries the meaning).

- `description: string` (REQUIRED for `canonical`; OPTIONAL otherwise)

  Description: Human-readable definition of what the entity is, at the abstraction-stable / implementation-free level (e.g. "an atomic item of work within the inventory; has a name, monetary value, and status").

- `surface_term: string` (REQUIRED for `synonym`; absent otherwise)

  Description: The author's original term that resolves to a canonical entry (e.g. "compensated work opportunity").

- `resolves_to: string` (REQUIRED for `synonym`; references a `canonical` `DataDictionaryEntry.dd_id`)

  Description: The canonical entry this synonym resolves to.

- `provenance_ref: string` (OPTIONAL; references the ledger element that introduced this entry/term)

  Description: A pointer into existing ledger traceability — the Requirement or CellContentItem that first asserted this entity, introduced this surface term, or asserted this relationship. Makes the synonym register (and the entry set) auditable at a glance without a graph traversal; it does not duplicate the trace.

  Notes: For `synonym` entries this is the element that introduced the surface term (so a wrong merge is attributable to its originating input); for `canonical` it is the first-asserting element.

- `confidence: number` (REQUIRED, 0.0..1.0)

  Description: For `synonym` entries, the resolution confidence (the basis for auto-resolution gating in the Data Dictionary mechanism). For `canonical`/`relationship`, the confidence that the entry/assertion is correct.

## Normative Rules

- `dd_id` MUST be unique and MUST match regex `^DD\\d{3}$`.

- `entry_kind` MUST be one of `canonical`, `synonym`. (The `relationship` kind was retired in v2.17; relationships are `class_model.relationships` on Structural Requirements.)

- If `entry_kind == canonical`: `name` and `description` MUST be present and non-empty; `surface_term` and `resolves_to` MUST be absent. A canonical entry carries NO model content — no attributes, value-sets, types, keys, or relationships (those are the `class_model` payload of Structural Requirements). Its **definition** is exactly the set of Structural Requirements whose `class_model.entity_ref` references this `dd_id` (found by query), mirroring the synonym pattern; the canonical entry itself holds no back-reference array.

- If `entry_kind == synonym`: `surface_term` MUST be present and non-empty; `resolves_to` MUST be present and reference an existing `DataDictionaryEntry` whose `entry_kind == canonical`. A canonical entry holds NO alias array — its aliases are exactly the set of synonym entries whose `resolves_to` references it (found by query).

- A DataDictionaryEntry MUST NOT carry a `row_target` (the DD is project-wide, not row-stratified).
...
# Element Type — DataDictionaryRegister [NEW v2.14]

**Specialisation of:** `Register`

## Purpose (NORMATIVE)

- Declare the authoritative membership set for `DataDictionaryEntry` elements.

- Enable completeness validation and prevent orphaned Data Dictionary entries.

## Non-Purpose (NORMATIVE)

- Duplicate member element content.

- Filter entries by `entry_kind`. All DataDictionaryEntry elements are members regardless of kind; kind-based filtering is a projection concern.
...
- Exactly ONE DataDictionaryRegister SHALL exist when any DataDictionaryEntry elements are present in the ledger.

- `register_type` SHALL be `"DataDictionaryEntry"`.

- `member_ids` SHALL contain ALL `DataDictionaryEntry.dd_id` values present in the ledger, regardless of `entry_kind`.

- Canonical completeness rule:

  > This register SHALL contain the identifiers of ALL DataDictionaryEntry elements present
  > in the ledger, regardless of entry_kind.
...
- `sysengage_ledger_version` MUST equal the specification version of this ledger model (e.g., "2.14").
...

{
   ...
   "title":"SysEngage Ledger Instance (v2.14) — Canonical Serialization",
...
"Stakeholder",
         ...
         "description":"Permitted element types (ledger spec v2.14)."
...
"type":"object",
"additionalProperties":false,
"required":["entity_ref","entity","tier"],
"properties":{
"entity_ref":{"type":"string","pattern":"^DD\\d{3}$"},
"entity":{"type":"string","minLength":1},
"tier":{"type":"string","enum":["conceptual","logical","physical","detailed"]},
"refinement_kind":{"type":"string","enum":["identity","decompose","realise_relationship","introduce","merge"]},
...
"attr_ref":{"type":"string","minLength":1},
"type":{"type":["string","null"]},
"domain":{"type":["string","null"]},
"key":{"type":"string","enum":["PK","FK"]},
"null":{"type":"boolean"},
"unique":{"type":"boolean"},
"default":{"type":["string","null"]},
"aggregatable":{"type":"boolean"},
"derived":{"type":["object","null"]},
"composite":{"type":["object","null"]},
"origin":{"type":"string","enum":["refines","realises","introduced"]}
}}},
...
"refines_refs":{"type":"array", "items":{"$ref":"#/$defs/id_R"}, "uniqueItems":true},

"class_model":{"$ref":"#/$defs/ClassModel"},

"object_refs":{"type":"array", "items":{"type":"string", "minLength":1}, "uniqueItems":true},
...
"DataDictionaryEntryPayload":{

"type":"object",

"additionalProperties":false,

"required":["dd_id","entry_kind","confidence"],

"properties":{

"dd_id":{"type":"string","pattern":"^DD\\d{3}$"},

"entry_kind":{"type":"string","enum":["canonical","synonym"]},

"name":{"type":"string","minLength":1},

"description":{"type":"string","minLength":1},

"surface_term":{"type":"string","minLength":1},

"resolves_to":{"type":"string","pattern":"^DD\\d{3}$"},
...
{"if":{"properties":{"entry_kind":{"const":"canonical"}},"required":["entry_kind"]},

"then":{"required":["name","description"]}},

{"if":{"properties":{"entry_kind":{"const":"synonym"}},"required":["entry_kind"]},

"then":{"required":["surface_term","resolves_to"]}}
...
"register_type":{"const":"DataDictionaryEntry"},

"member_ids":{

"type":"array",

"items":{"type":"string","pattern":"^DD\\d{3}$"},
...

{
                  ...
                  "if":{
                     ...
                     "properties":{"element_type":{"const":"DataDictionaryEntry"}},
                  ...
                  "then":{
                     ...
                     "properties":{
                        ...
                        "payload":{"$ref":"#/$defs/DataDictionaryEntryPayload"},

                        "element_id":{"type":"string","pattern":"^DD\\d{3}$"}
...

{
                  ...
                  "if":{
                     ...
                     "properties":{"element_type":{"const":"DataDictionaryRegister"}},
                  ...
                  "then":{
                     ...
                     "properties":{
                        ...
                        "payload":{"$ref":"#/$defs/DataDictionaryRegisterPayload"}
...
## v2.16 → v2.17 Migration

Data Dictionary scope correction (F107), `class_model` payload (F105), and `object_refs` binding (F107).

**`DataDictionaryEntry` (breaking for `canonical` and `relationship` entries):** the `canonical` `attributes` array (and its `value_set`s) and the entire `relationship` `entry_kind` are removed. A pre-v2.17 ledger's DD model content does not simply drop — it **migrates** to `class_model`: (a) each canonical entry's `attributes` become attributes on the `class_model` of the Structural Requirement(s) defining that entity (de-duplicated and typed against the already-correct row Structural reqs — the DD copies were noisy/duplicative); (b) each `value_set` becomes the `domain` of the corresponding `class_model` attribute (the authoritative domain is the row Structural CHECK, not the DD copy); (c) each `relationship` entry becomes a `class_model.relationships` entry on the appropriate row(s). After migration the `canonical` entry retains only `name`, `description`, `provenance_ref`, `confidence`; a canonical entity's definition is thereafter found by query (Structural Requirements whose `class_model.entity_ref` references it). Tools reading a pre-v2.17 ledger MUST treat residual DD `attributes`/`relationship` content as un-migrated, not as authoritative.

**`Requirement` (additive, non-breaking):** new OPTIONAL `class_model` (Structural only) and `object_refs` (behavioural only). Absent in pre-v2.17 Requirements — a Structural Requirement without `class_model` is prose-only (a migration residue, valid but un-promoted); a behavioural Requirement without `object_refs` has only the former prose-implicit Object-slot binding (no stored value binding — there were none to preserve, finding F107 pin 1). Promotion of prose Structurals to `class_model` and establishment of `object_refs` is the one-time migration; it is not required for a v2.17 ledger to validate, but the Quality pass (F107-T1) reports un-promoted Structurals and dangling `object_refs`.

**Path A:** this amendment lands with the Data Dictionary mechanism spec and the Object-slot binding revisions (Requirement Derivation Row 3/4 §5.5; finding F107-T1); the `class_model` per-row population profile and the concept-coverage refinement check are specified in the Requirement Derivation F105 mechanism spec.

## v2.13 → v2.14 Migration

**Additive only. No existing element types or attributes removed or changed.**

New element types: `DataDictionaryEntry` (DD###) and `DataDictionaryRegister`. Absent in pre-v2.14 ledger instances — tools reading earlier instances treat absence as "the Data Dictionary service has not yet populated the ledger" rather than as an error. The DD is a standalone cross-row service (F93) populated incrementally as rows produce requirements; an empty or absent DD is normal early in a project.

`DataDictionaryEntry.entry_kind` discriminates `canonical` / `synonym` / `relationship`. Aliases are represented ONLY as `synonym` entries (single source of truth); a `canonical` entry carries no alias array, and "an entity's aliases" is a query over synonym entries whose `resolves_to` references it. Value-sets are a field on a canonical entry's attribute with named-addressable values (referenced as `<Entity>.<attr>.<value>`), NOT separate elements; transition/state-model behaviour over those values is NOT represented in v2.14 (deferred, F91). Relationships are directed `relationship` entries (`from_ref` → `to_ref` + `cardinality`).

The canonical `Requirement` element is UNCHANGED in v2.14. Object-slot binding to DD entries is enforced by the Requirement Derivation / Data Dictionary mechanisms, not by the ledger schema — a v2.13 Requirement is a valid v2.14 Requirement with no migration.

**Identifier prefix:** DD### for DataDictionaryEntry. No legacy remap concern (the type is new in v2.14).

## v2.12 → v2.13 Migration

**Additive and value-collapsing changes to the `Requirement` element only. No element types added or removed.**

New attribute on `Requirement`: `refines_refs` (OPTIONAL, array of `R###`). Absent in pre-v2.13 Requirement elements — treat as empty (the Requirement has no recorded parent link; this is indistinguishable from a legitimately unparented requirement and is not an error). Tools performing cross-row «refine» traversal on a pre-v2.13 ledger will find no links and should treat the ledger as not-yet-matched rather than as having no relationships.

`requirement_type` enum collapsed: `Performance` and `Suitability` → `Constraint`; `Non-Functional` → `Structural`. **Migration of existing instances:** a pre-v2.13 Requirement with `requirement_type` in {Performance, Suitability} re-types to `Constraint` and SHOULD set `verification_method = Measurement` where a measurable threshold applies (else `Inspection`); a Requirement with `requirement_type = Non-Functional` re-types to `Structural` (or to `Constraint` if it is in fact a quality-attribute bound rather than a composition assertion — a judgement per requirement). Validators enforcing the v2.13 three-value enum will REJECT instances still carrying the retired values; such instances require migration before v2.13 conformance.

New `verification_method` value: `Measurement`. Absent-meaning unchanged for existing records.

**Data Dictionary not present:** the `DataDictionaryEntry` / `DataDictionaryRegister` element types referenced in design findings F90/F93 are deliberately NOT defined in v2.13 (open attribute-schema items; deferred to the Data Dictionary mechanism spec under Path A discipline). Ledger instances MUST NOT contain Data-Dictionary-prefixed elements under v2.13; any such elements from sandbox/prototype work are unrecognised.
