...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_18.md

Version: 0.18 (column_interrogatives.py — named-instance instruction for Where column at Rows 4 and 5)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). v0.18 adds the `split_protected` flag to Stage 4a Step 1 and splits the Stage 4b warning emission into two distinct types: `stage4a_named_instance_routed` (Step 1 — enumeration split groups) and `stage4a_pairwise_routed` (Step 2 — dissimilar-description routing). Resolves warning conflation identified in NQPS Run 2.
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_17.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.17** | All sections | Architectural spec including split_protected flag and stage4a_pairwise_routed warning type |
...
**Step 1 — Identify and protect enumeration split groups:**

Before evaluating any pairs, scan the new candidate set for sub-signals from Stage 3a-pre splits. A candidate is a confirmed split sub-signal if and only if BOTH conditions are true:
- Its `signal_refs` contains exactly one Signal id, AND
- That Signal id appears in the current run's `enumeration_splits` buffer (i.e. Stage 3a-pre split that Signal in this run)

The second condition is essential. Candidates where `len(signal_refs)==1` but the Signal is NOT in `enumeration_splits` are ordinary AI-derived candidates — the AI routinely produces multiple CCIs from a single Signal without pre-processor involvement. These must pass through normal pairwise evaluation. Only confirmed sub-signals from an actual Stage 3a-pre split receive protection.

Implementation check: `candidate.signal_refs[0] in {s['original_signal_id'] for s in enumeration_splits_buffer}`.

Group all confirmed split sub-signals sharing the same `original_signal_id` into a protected group. Set **both** `stage4a_routed=True` **and** `split_protected=True` on every member. The `split_protected` flag is the discriminator at Stage 4b — it must only be set here in Step 1, never by Step 2. Step 2 sets only `stage4a_routed=True`.

**Step 2 — Pairwise pre-filter for non-protected candidates:**

For each pair of non-protected candidates, evaluate three conditions:

1. `classification_type` match: string equality
2. `signal_refs` match: set equality (`set(a.signal_refs) == set(b.signal_refs)`)
3. Description similarity: Jaccard token overlap ≥ `ProjectProfile.stage4a_similarity_threshold` (default 0.60)

**Routing rules:**
- All three conditions hold → structural duplicate. Apply merge rule (§4.4.1).
- Conditions 1 and 2 hold, condition 3 does not → route both to Stage 4b with `stage4a_routed=True`.
- Condition 1 alone holds → Stage 4b handles normally.

Process all non-protected pairs exhaustively before proceeding.
...
- If **any member** carries `stage4a_routed=True`: use the named-instance preservation directive for the entire group. Before recording the execution warning, check the `split_protected` flag:
  - If any member carries `split_protected=True`: record `{warning_type: "stage4a_named_instance_routed", detail: {cell_id, classification_type, member_count}}` — Step 1 enumeration split group.
  - If no member carries `split_protected=True`: record `{warning_type: "stage4a_pairwise_routed", detail: {cell_id, classification_type, member_count}}` — Step 2 pairwise routing, normal analytical behaviour.
...
Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_17.md
...
*v0.16 replaces the above Document End. End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.16.*

*Changes from v0.15: Stage 4a enumeration split group protection added (Step 1 before pairwise loop); Stage 4b diagnostic logging requirement removed (OQ-3b-11 resolved); Row 3 spec reference updated to v0.15.*

*Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_17.md*

---

*v0.17 replaces the above Document End. End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.17.*

*Changes from v0.16: Stage 4a Step 1 tightened — explicit enumeration_splits buffer check required before protecting a group; implementation check pattern added; Row 3 spec reference updated to v0.16.*

*Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_17.md*

---

*v0.18 replaces the above Document End. End of SysEngage Row 4 Mechanism: CellContentItem Construction v0.18.*

*Changes from v0.17: Stage 4a Step 1 — `split_protected=True` added to enumeration split group candidates; Stage 4b — warning type split: `stage4a_named_instance_routed` for Step 1 groups (split_protected=True), `stage4a_pairwise_routed` for Step 2 groups; Row 3 spec reference updated to v0.17.*
