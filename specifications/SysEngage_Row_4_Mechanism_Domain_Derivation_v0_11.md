...
Filename: SysEngage_Row_4_Mechanism_Domain_Derivation_v0_11.md

Version: 0.11 (internal consistency pass — full spec read before any external review; all internal contradictions and gaps resolved: §4.4.6 execution_status rule made explicit; §7 execution_warnings enumeration completed; §4.4.1 retired_at comment corrected; §4.3 ADVC-3c-01 variable naming corrected; §3.1 stage3 comment updated. Understanding references updated to v0.15; tracker reference updated to v0.37)

Date: 26 May 2026

**Purpose.** Implementation specification for the Domain Derivation mechanism (Pass 3c). v0.11 is produced after a complete front-to-back read of the full spec to identify all internal inconsistencies before external review. It is the designated handoff version — further spec iteration after this point is driven by implementation evidence only. Supersedes v0.10.
...
| **Row 4 Understanding v0.15** | §13 | Pass 3c implementation framework: module structure (row_abstraction_vocabulary.py absent), re-run scenarios (prose), ProjectProfile parameters (three), mode discipline, VER criteria → pytest mapping, 7-fixture table, Replit Agent handoff. |
...
| **Tracker v0.37** | F43, F53, F55 | F43: Action-Required (pass catalogue mode label). F53: Action-Required (Row 3 Understanding §E.12.1 amendment). F55: seventh-review corrections (Resolved). |
...
Write outside the main ledger transaction (same pattern as Pass 3b Step 6). Populate all fields of `mechanism_data` per §7. AnalysisPass `execution_status` is determined as follows:
...
- `"CompletedWithWarnings"` — if, and only if, one or more of these conditions hold: (a) persistent orphaned CCIs remain in `mechanism_data.orphaned_ccis` after the repair attempt; (b) IncrementalRerun AI parse failed and the run fell back to FullRerun (`incremental_fallback_to_fullrerun` in `execution_warnings`); (c) mode violations are recorded in `mechanism_data.mode_violations`.
- `"Completed"` — all other outcomes, including: IdempotentRerun (distinguished by `mechanism_data.idempotent = true`); successful repair (all orphans recovered); informational advisories only (`large_cci_set_advisory`, `domain_count_advisory`, `duplicate_domain_name_merged`, `repair_assign_name_not_found`, `incremental_assign_invalid_domain_id`, `no_cci_input`, `cci_referential_integrity_violation`). Entries in `execution_warnings` alone do NOT change `execution_status` to `CompletedWithWarnings`.
...
**`execution_warnings` — placement clarification:** `execution_warnings` is a **standard top-level AnalysisPass field**, not a sub-field of `mechanism_data`. It is defined at the AnalysisPass model level (same pattern as Pass 3b and all other mechanisms) and is populated by the AnalysisPass writer utility with any advisory entries accumulated during the run. Entries in `execution_warnings` do **not** change `execution_status` — see §4.4.6 for the precise `CompletedWithWarnings` trigger conditions.

Pass 3c writes to `execution_warnings` in the following cases:

| Advisory type | Source | When |
|---|---|---|
| `no_cci_input` | §4.1 | Zero-CCI early exit — no CCIs found for this row |
| `cci_referential_integrity_violation` | §4.1 | CCI references a non-existent ZachmanCell; CCI excluded from eligible set |
| `incremental_fallback_to_fullrerun` | §4.2 | IncrementalRerun AI parse failure; run fell back to FullRerun |
| `duplicate_domain_name_merged` | §4.3 CHK-3c-03 | Two or more proposals had the same name; merged |
| `domain_count_advisory` | §4.3 ADVC-3c-01 | Domain count outside soft bounds (`< 1 + ceil(cci/15)` or `> cci/2`) |
| `repair_assign_name_not_found` | §4.3 repair | Repair assign action domain_name not matched; treated as new |
| `incremental_assign_invalid_domain_id` | §5.2 | IncrementalRerun assign action domain_id not found; treated as new |
...
| **Phase 2 — Mechanism Activation** | `project_profile` row — provides three domain-specific parameters (§13.9 of Row 4 Understanding v0.15). | Soft prerequisite — ProjectProfile must exist. If any parameter is NULL, default value is substituted. |
...
- This spec (Row 4 Mechanism Spec — Domain Derivation v0.11) — implementation authority (all detail including DDL in §5.1)
...
- Row 4 Understanding v0.15 §13 — framework (module structure, prose descriptions, ProjectProfile parameters, VER criteria mapping, fixture table)
...
End of SysEngage Row 4 Mechanism: Domain Derivation v0.11.

Changes from v0.10 (internal consistency pass — full front-to-back read before handoff):
- **§4.4.6 (Significant — from reviewer, fix #1):** `execution_status` determination rule rewritten. "CompletedWithWarnings if orphans, advisories, or IncrementalRerun fallback" replaced with an explicit three-condition rule: (a) persistent orphaned CCIs; (b) IncrementalRerun→FullRerun fallback; (c) mode violations. All informational advisory types explicitly listed as NOT triggering CompletedWithWarnings. Eliminates the contradiction with ADVC-3c-01 ("does not affect execution_status") and §3.3 ("informational only").
- **§7 (Noted — from reviewer + internal read, fix #2):** `execution_warnings` note rewritten. "Three places" claim removed (was wrong — seven types across five sections). Complete table of all seven advisory types added with source sections. Two missing §4.1 types added: `no_cci_input` and `cci_referential_integrity_violation`. §13.7 Understanding updated to match.
- **§4.4.1 (Internal read, fix #3):** Backwards `retired_at` comment corrected. "includes retired Domains — `retired_at IS NOT NULL`" was backwards — the filter `IS NOT NULL` would return only retired rows. Correct statement: no `retired_at` filter; query returns all Domains (active and retired).
- **§4.3 ADVC-3c-01 (Internal read, fix #4):** `cci_count_input` renamed to `len(eligible_ccis)` / `eligible_count` — `cci_count_input` is the `mechanism_data` field name, not a Stage 3 local variable. Stage 3 has `eligible_ccis` from Stage 1.
- **§3.1 (Internal read, fix #5):** stage3 comment updated to include ADVC-3c-01.
- **§13.13 (Noted — from reviewer, fix #6):** Self-reference "v0.13" → "v0.15" in Understanding v0.15.
- **Cross-references:** Understanding v0.14 → v0.15; tracker v0.36 → v0.37; self-reference v0.10 → v0.11.

Changes from v0.9: four noted findings (cci_refs in repair fallback; invalid domain_id fallback; ADVC-3c-01; execution_warnings placement).
...
- SysEngage_Row_4_Understanding_v0_15.md — implementation framework
- SysEngage_Issues_Tracker_v0_37.md — finding disposition

**This is the designated handoff version.** Further spec iteration after v0.11 is driven by implementation evidence only — not by additional pre-build review rounds.
