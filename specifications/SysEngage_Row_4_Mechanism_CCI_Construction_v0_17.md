...
Filename: SysEngage_Row_4_Mechanism_CCI_Construction_v0_17.md

Version: 0.17 (column_interrogatives.py — named-instance instruction for Where column at Rows 4 and 5)

Date: 16 May 2026

**Purpose.** Implementation specification for the CellContentItem Construction mechanism (Pass 3b). v0.17 tightens Stage 4a Step 1 group protection: a candidate is confirmed as a split sub-signal only if its single Signal id appears in the current run's `enumeration_splits` record. Ordinary AI-derived candidates sharing a Signal id (not from a pre-processor split) pass through normal pairwise evaluation.
...
| --- | --- |
| **Row 3 Mechanism Spec reference** | SysEngage_Row_3_Mechanism_CCI_Construction_v0_16.md — all sections |
...
| --- | --- | --- |
| **Row 3 Mechanism Spec v0.16** | All sections | Architectural spec including tightened Stage 4a enumeration_splits scope check |
...
Group all confirmed split sub-signals sharing the same `original_signal_id` into a protected group. Set `stage4a_routed=True` on all members. Remove them from the pairwise candidate pool. Pass them directly to Stage 4b as a pre-formed named-instance group. Record `stage4a_named_instance_routed` in `execution_warnings` once per protected group.
...
Companion artefact: SysEngage_Row_3_Mechanism_CCI_Construction_v0_16.md
