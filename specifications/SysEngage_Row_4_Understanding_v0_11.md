...
**Filename:** SysEngage_Row_4_Understanding_v0_11.md (§13 addendum)

**Version:** 0.11 — §13.8 DDL pointer corrected: points to Mechanism Spec v0.7 §5.1 (which contains the authoritative CREATE TABLE DDL); circular reference eliminated. Supersedes v0.10.

**Date:** 26 May 2026

**Purpose.** §13 carries structural guidance only. The full CREATE TABLE DDL now lives in Mechanism Spec v0.7 §5.1. v0.11 replaces v0.10 in full; all §1–§12 content from v0.5 remains authoritative.

**Scope note.** This document records §13 only.

**Precedence rule.** Where this artefact appears to differ from canonical ledger spec v2.12 or Row 4 Mechanism Spec (Domain Derivation) v0.7, the Mechanism Spec takes precedence.

---

## §13 Pass 3c — Domain Derivation Implementation Framework

This section establishes the Row 4 implementation framework for Pass 3c. The architectural authority is **SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md**. The implementation specification is **SysEngage_Row_4_Mechanism_Domain_Derivation_v0_7.md**. This section provides structural framework guidance — module structure, parameter tables, VER criteria mapping, fixture table, and Replit Agent handoff notes. Implementation detail (stage-by-stage behaviour, pseudocode, full DDL, Pydantic schemas) lives in the Mechanism Spec §5.1 and is not duplicated here.
...
mechanisms/domain_derivation/
  ...
  stage4_entity_production.py              # DM: domain_id allocation; domain_qualifier mapping; entity construction;
                                           #     upstream_domain_ref heuristic (Row 3 only); ledger transaction; DomainRegister
  prompts/
    ...
    row_abstraction_vocabulary.py          # Row-target → abstraction level phrase + Domain qualifier label mapping
  schemas/
...
- The prior-pass query (`query_most_recent_completed_domain_derivation_pass`) excludes both Failed and Skipped runs — a Failed run committed no Domains and must be treated as if no prior run exists.
- The CCI delta calculation for IncrementalRerun uses `query_committed_cci_ids_for_row(row_ref, project_id)` — a live DB query on `domain_cci_membership`, not a stored field in `domain_data`.
...
| `abstraction_level_phrase` | `row_abstraction_vocabulary.py[row_ref]` | e.g., "business conceptual level — business processes, entities, roles, events, and rules" |
| `domain_qualifier_label` | `row_abstraction_vocabulary.py[row_ref]` | e.g., "Conceptual Domain" |
...
**`row_abstraction_vocabulary.py`** is the single source of truth for the row-target → abstraction level phrase and Domain qualifier label mapping (Row 3 Mechanism Spec §4.1.1). It is a simple dict constant, not an AI call. The Row 4 Mechanism Spec will define the concrete module content.
...
- **domain_qualifier** is assigned deterministically from `row_target` via `QUALIFIER_MAP`. The concrete dict lives in `row_abstraction_vocabulary.py` (Mechanism Spec §5.4).
- **upstream_domain_ref** (Row 3 only) is set by Jaccard name-similarity heuristic — DM, no AI call. Threshold: `domain_upstream_match_threshold` (default 0.50).
...
- **Transaction (Mechanism Spec §4.4.4):** Single atomic transaction containing: FullRerun retirement UPDATE; new Domain entity INSERTs; new Domain membership INSERTs (step 3a); IncrementalRerun `assign_membership_inserts` INSERTs (step 3b — existing Domain memberships for new CCIs); project-wide DomainRegister UPDATE (no `row_target` filter).
- **DomainRegister update** uses `query_all_active_domain_ids(project_id)` — the project-wide active domain set. This applies to **all paths including the zero-CCI early exit in §4.1** — never hardcode `member_ids = []`.

---

### §13.7 AnalysisPass Population

The `domain_data` sub-structure in `AnalysisPass.outputs` is defined in Row 3 Mechanism Spec §7. The implementation must populate ALL fields; zero-value arrays MUST be `[]` not null. The AnalysisPass is written outside the main ledger transaction (same discipline as Pass 3b Step 6).

**`domain_data` field population responsibilities by Stage:**
...
| `domain_count_produced` | Stage 4 | `len(domain_entities)` |
...
| `unmatched_upstream_refs` | Stage 4 | domain_ids of Row 3 Domains with no upstream match found |
...
Pass 3c requires the `domain` and `domain_cci_membership` tables and a DomainRegister seed row. The authoritative CREATE TABLE DDL (including `retired_at TIMESTAMPTZ`, `CONSTRAINT fk_upstream_domain FK … ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED`, CHECK constraints, migration author notes, and DomainRegister seed INSERT) is in **Mechanism Spec v0.7 §5.1**. The DDL is not duplicated here.

Key schema decisions to be aware of when reading the Understanding:
...
Four new ProjectProfile parameters are introduced by Pass 3c. These must be added to the `project_profile` table alongside the existing Pass 3b parameters:
...
| `domain_upstream_match_threshold` | float 0.0–1.0 | 0.50 | Jaccard token overlap threshold for `upstream_domain_ref` name-similarity heuristic (Row 3 only) |

**Migration:** Add four columns to the `project_profile` table with the default values above. All four are nullable — `NULL` means "use default". The mechanism reads the value, substitutes the default if NULL.
...
**Mode violations** (a DM stage making an AI call, or an IM stage making no AI call when one was expected) are recorded in `AnalysisPass.outputs.domain_data.mode_violations` and set `execution_status = CompletedWithWarnings`.
...
| VER-3c-08 | Assert `domain_data` structure present with all required fields non-null | `tests/test_domain_derivation.py` |
| VER-3c-09 | Assert `domain_qualifier` matches deterministic mapping for `row_target` | `tests/test_domain_derivation.py` |
| VER-3c-10 | Assert IdempotentRerun leaves domain set unchanged (same domain_ids, same memberships) | `tests/test_domain_derivation.py` |
...
|---|---|---|
| Fixture 1 — PMT Row 2 happy path | `test_pmt_row2_firstrun` | VER-3c-05 (Non-Loss), VER-3c-09 (Conceptual qualifier), ≥2 Domains |
| Fixture 2 — NQPS Row 3 narrow | `test_nqps_row3_firstrun` | VER-3c-09 (Logical qualifier), PLB-3c-05 (logical-design vocabulary spot check) |
| Fixture 3 — IdempotentRerun | `test_pmt_row2_idempotent_rerun` | VER-3c-10, execution_status=Skipped, Stage 2 AI stub not called |
...
The orchestrator (`core/orchestrator.py`) must enforce this. The check is: query AnalysisPass for `mechanism="DomainDerivation"` and `row_ref=current_row`; assert `execution_status ∈ {Completed, CompletedWithWarnings}`. `Skipped` counts as satisfied (idempotent re-run; Domains unchanged). `Failed` blocks Pass 3d.
...
- Row 3 Mechanism Spec (Domain Derivation) v0.1 — primary architectural reference
- This section (Row 4 Understanding §13) — implementation framework
- Row 4 Applied v0.2 — common architectural commitments (stack, transactional discipline, mode decorator)
- Row 4 Understanding §12 (v0.5) — reference implementation patterns from Pass 3b (batch pattern, Pydantic boundary, AnalysisPass population, transactional discipline)
- Canonical Ledger v2.12 — Domain and DomainRegister entity schemas
...
- `upstream_domain_ref` heuristic (Row 3 only, DM, no AI call)
...
- `migrations/XXXX_add_domain_tables.py` — creates `domain` and `domain_cci_membership` tables and seeds DomainRegister row
- `migrations/XXXX_add_domain_profile_params.py` — adds four new ProjectProfile columns

**Build task — ProjectProfile model update:**
- `schemas/project_profile.py` (or equivalent Pydantic model) — add four new optional fields with defaults
...
| OQ ID | Question | Row 4 decision required |
|---|---|---|
| **OQ-3c-01** | Domain count calibration: no formula or hard bounds specified | The Replit Agent should implement PLB-3c-03 as a post-run advisory check in `stage3_structural_validation.py`. Advisory fires if `domain_count_produced < 1 + ceil(cci_count / 15)` or `domain_count_produced > cci_count / 2`. These are soft bounds — AnalysisPass advisory only, not a failure. The bounds should be reviewed against empirical PMT and NQPS production runs and refined in a subsequent tracker finding. |
| **OQ-3c-02** | upstream_domain_ref threshold: 0.5 Jaccard overlap is an initial estimate | Implement as `domain_upstream_match_threshold` ProjectProfile parameter (default 0.5). The Row 4 Mechanism Spec should validate this against the first Row 3 production run where both Row 2 and Row 3 Domains exist in the same project. |
| **OQ-3c-03** | FullRerun retirement: soft-delete flag vs archive table | Decision: **soft-delete flag** (`retired_at TIMESTAMPTZ`, nullable; null = active). Simpler than an archive table; preserves referential integrity for any existing `Requirement.domain_refs` (a retired domain_id still resolves); supports audit trail. The `query_max_domain_id` function queries all Domains including retired ones to ensure no id reuse. Active Domains are those with `retired_at IS NULL`. |

---

## Document End

End of SysEngage Row 4 Understanding v0.11 — §13 addendum.

**Changes from v0.10:**
- §13.1: mechanism spec reference updated v0.6 → v0.7; "full DDL in Mechanism Spec §5.1" made explicit
- §13.8: DDL pointer corrected — now references Mechanism Spec v0.7 §5.1 (which contains the full CREATE TABLE DDL); circular reference with v0.10 §13.8 ↔ v0.6 §5.1 eliminated

§13 content from v0.10 is superseded by v0.11. All §1–§12 content from v0.5 unchanged.

Companion artefacts:
- SysEngage_Row_3_Mechanism_Domain_Derivation_v0_1.md — architectural authority for Pass 3c
- SysEngage_Row_4_Mechanism_Domain_Derivation_v0_7.md — implementation spec (full DDL in §5.1)
